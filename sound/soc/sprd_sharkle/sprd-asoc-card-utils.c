				       ...
				       char *prefix, int idx)
{
	...
	if (strlen(prefix) &&
	    !(daifmt & SND_SOC_DAIFMT_CLOCK_PROVIDER_MASK)) {
		/*
					   ...
					   struct snd_soc_dai_link *dai_link)
{
	...
	if (ret) {
		...
		if (val[0] >= SND_SOC_DPCM_TRIGGER_PRE &&
		    val[0] <= SND_SOC_DPCM_TRIGGER_POST)
			dai_link->trigger[0] = val[0];
		if (val[1] >= SND_SOC_DPCM_TRIGGER_PRE &&
		    val[1] <= SND_SOC_DPCM_TRIGGER_POST)
					  ...
					  struct snd_soc_dai_link *dai_link)
{
	...
	if (val[1] == 1) {	/* dummy playback-only codec */
		dai_link->codecs[0].dai_name = "sprd-dummy-playback-dai";
		dai_link->codecs[0].name = "sprd-dummy-playback";
	} else {
		dai_link->codecs[0].dai_name = "snd-soc-dummy-dai";
		dai_link->codecs[0].name = "snd-soc-dummy";
	}
				      ...
				      int idx, bool is_top_level_node)
{
	...
	ret = asoc_sprd_card_sub_parse_of(cpu, &dai_props->cpu_dai,
					  &dai_props->cpus[0].of_node,
					  &dai_props->cpus[0].dai_name,
					  &cpu_args);
	if (ret < 0)
		goto dai_link_of_err;

	ret = asoc_sprd_card_sub_parse_of(codec, &dai_props->codec_dai,
					  &dai_props->codecs[0].of_node,
					  &dai_props->codecs[0].dai_name,
					  NULL);
	if (ret < 0) {
		if (-ENOENT == ret) {
	...
	if (plat) {
		...
		ret = of_parse_phandle_with_args(plat, "sound-dai",
						 "#sound-dai-cells", 0, &args);
		dai_props->platforms[0].of_node = args.np;
	} else {
		/* Assumes platform == cpu */
		dai_props->platforms[0].of_node = dai_props->cpus[0].of_node;
	}

	/* DAI link name is created from CPU/CODEC dai name */
	name = devm_kzalloc(dev,
			    strlen(dai_props->cpus[0].dai_name) +
			    strlen(dai_props->codecs[0].dai_name) + 2,
			    GFP_KERNEL);
	if (!name) {
		ret = -ENOMEM;
		goto dai_link_of_err;
	...
	sprintf(name, "%s-%s", dai_props->cpus[0].dai_name,
		dai_props->codecs[0].dai_name);
	...
	dev_dbg(dev, "\tcpu : %s / %d\n",
		dai_props->cpus[0].dai_name, dai_props->cpu_dai.sysclk);
	dev_dbg(dev, "\tcodec : %s / %d\n",
		dai_props->codecs[0].dai_name, dai_props->codec_dai.sysclk);
	 ...
	 */
	if (!cpu_args)
		dai_props->cpus[0].dai_name = NULL;
...
{
	struct device *dev = sprd_priv_to_dev(priv);
	struct gpio_desc *hp_desc, *mic_desc;
	u32 val;
	int ret;
	...
	hp_desc = gpiod_get_index(dev, "sprd-audio-card,hp-det", 0,
				   GPIOD_IN);
	if (IS_ERR(hp_desc)) {
		priv->gpio_hp_det = PTR_ERR(hp_desc);
		if (priv->gpio_hp_det == -EPROBE_DEFER)
			return -EPROBE_DEFER;
	} else {
		priv->gpio_hp_det = desc_to_gpio(hp_desc);
		gpiod_put(hp_desc);
	}
	priv->gpio_hp_det_invert = 0;

	mic_desc = gpiod_get_index(dev, "sprd-audio-card,mic-det", 0,
				   GPIOD_IN);
	if (IS_ERR(mic_desc)) {
		priv->gpio_mic_det = PTR_ERR(mic_desc);
		if (priv->gpio_mic_det == -EPROBE_DEFER)
			return -EPROBE_DEFER;
	} else {
		priv->gpio_mic_det = desc_to_gpio(mic_desc);
		gpiod_put(mic_desc);
	}
	priv->gpio_mic_det_invert = 0;
...
static int asoc_sprd_card_unref(struct snd_soc_card *card)
{
	...
	for (num_links = 0, dai_link = card->dai_link;
		...
		of_node_put(dai_link->platforms[0].of_node);
