...
struct sprd_asoc_hook_spk_priv {
	struct gpio_desc *gpio[SPK_CNT_MAX];
...
static void hook_gpio_pulse_control(struct gpio_desc *gpio, unsigned int mode)
{
	int i = 1;
					 ...
					 struct sprd_asoc_ext_hook *ext_hook)
{
	...
	for (i = 0; gpio_map[i].name && i < spk_cnt; i++) {
		...
		hook_spk_priv.gpio[type] = devm_gpiod_get_index(dev,
					"sprd,spk-ext-pa", index,
					speaker_hook[hook_sel].en_level ?
					GPIOD_OUT_HIGH : GPIOD_OUT_LOW);
		if (IS_ERR(hook_spk_priv.gpio[type])) {
			ret = PTR_ERR(hook_spk_priv.gpio[type]);
			dev_err(dev, "Get gpio failed:%d!\n", ret);
			ext_hook->ext_ctrl[ext_ctrl_type] = NULL;
			return ret;
		}

		pr_info("ext_ctrl_type %d hook_sel %d priv_data %d",
