...
static void *sprd_pcm_component_priv(struct snd_soc_pcm_runtime *srtd)
{
...
static bool is_use_resddr(struct snd_pcm_substream *substream)
{
	...
	priv_data = sprd_pcm_component_priv(srtd);
	if (!priv_data) {
...
static int alloc_linklist_cfg_resddr(struct snd_pcm_substream *substream)
{
	...
	struct snd_soc_pcm_runtime *srtd = substream->private_data;
	struct snd_soc_dai *cpu_dai = snd_soc_rtd_to_cpu(srtd, 0);
	struct platform_pcm_priv *priv_data;
	u32 resddr_phy_base;
...
static bool is_use_2stage_dma(struct snd_soc_pcm_runtime *srtd, int stream)
{
	...
	if (snd_soc_rtd_to_cpu(srtd, 0)->id == VBC_DAI_NORMAL &&
	    substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
		ret = (cases & TWO_STAGE_NORMAL) ? true:false;
	else if (snd_soc_rtd_to_cpu(srtd, 0)->id == VBC_DAI_DEEP_BUF)
			...
			u32 buffer_size)
{
	...
	dma_buffer->private_data = dma_buffer;
	dma_buffer->area =
		dma_alloc_attrs(substream->pcm->card->dev,
			      ...
			      struct snd_dma_buffer *dma_buffer)
{
	...
	if (dma_buffer->area && (dma_buffer->dev.type == SNDRV_DMA_TYPE_DEV)) {
		dma_free_attrs(substream->pcm->card->dev,
	...
	struct snd_pcm_hardware *hw)
{
	...
	/* default config */
	hw->periods_min = 1;
	if (sprd_is_i2s(snd_soc_rtd_to_cpu(srtd, 0))) {
	...
	if (substream->stream == SNDRV_PCM_STREAM_CAPTURE) {
		...
		if (snd_soc_rtd_to_cpu(srtd, 0)->id == VBC_DAI_DEEP_BUF) {
			...
			hw->buffer_bytes_max =
				DEEPBUFFER_PLAYBACK_BUFFER_BYTES_MAX;
		} else if (snd_soc_rtd_to_cpu(srtd, 0)->id == VBC_DAI_NORMAL) {
			hw->period_bytes_max =
			hw->buffer_bytes_max =
				NORMAL_PLAYBACK_BUFFER_BYTES_MAX;
		} else if (sprd_is_i2s(snd_soc_rtd_to_cpu(srtd, 0))) {
			    ...
			    struct snd_pcm_substream *substream)
{
	...
	if (sprd_is_i2s(snd_soc_rtd_to_cpu(srtd, 0))) {
		config = sprd_i2s_dai_to_config(snd_soc_rtd_to_cpu(srtd, 0));
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
	...
	if (sprd_is_dfm(snd_soc_rtd_to_cpu(srtd, 0)) || sprd_is_vaudio(snd_soc_rtd_to_cpu(srtd, 0))) {
		runtime->private_data = rtd;
		ret = 0;
		goto out;
	}
			    ...
			    struct snd_pcm_hw_params *params)
{
	 ...
	 */
	if (params_count == 2) {
		switch (snd_soc_rtd_to_cpu(srtd, 0)->id) {
					 ...
					 struct snd_pcm_hw_params *params)
{
	...
	*/
	...
	sp_asoc_pr_info("(pcm) %s, cpudai_id=%d\n", __func__,
			snd_soc_rtd_to_cpu(srtd, 0)->id);
	dma_data = snd_soc_dai_get_dma_data(snd_soc_rtd_to_cpu(srtd, 0), substream);
	if (!dma_data)
		goto no_dma;
	...
	if (rtd->params == NULL) {
		...
		ret =
		    sprd_pcm_request_dma_channel(substream,
					 snd_soc_rtdcom_lookup(srtd, "sprd-pcm-platform")->dev->of_node,
					 rtd, rtd->hw_chan);
	if (ret)
		goto hw_param_err;
	}
		...
		if (i == 0) {
			if (snd_soc_rtd_to_cpu(srtd, 0)->id == VBC_DAI_NORMAL) {
				...
				pr_err("%s %d failed unknown cpu_dai_id=%d\n",
				       __func__, __LINE__, snd_soc_rtd_to_cpu(srtd, 0)->id);
				goto hw_param_err;
			}
		} else
			      ...
			      struct snd_pcm_substream *substream, int cmd)
{
	...
	sp_asoc_pr_info("%s, %s cpu_dai->id = %d Trigger %s cmd:%d\n", __func__,
			sprd_dai_pcm_name(snd_soc_rtd_to_cpu(srtd, 0)), snd_soc_rtd_to_cpu(srtd, 0)->id,
...
static int sprd_pcm_component_ioctl(struct snd_soc_component *component,
				   struct snd_pcm_substream *substream,
...
static void sprd_pcm_platform_proc_init(struct snd_soc_component *component)
{
	struct snd_info_entry *entry;

	if (!snd_card_proc_new(component->card->snd_card,
			       "sprd-dmaengine", &entry))
		snd_info_set_text_ops(entry, component,
...
#else /* !CONFIG_PROC_FS */
static inline void
sprd_pcm_platform_proc_init(struct snd_soc_component *component)
{
}
#endif
...
static int aud_init_iram_addr(struct snd_soc_component *platform)
{
	...
	if (priv_data->iram_size > 0) {
		priv_data->iram_virt_addr =
		    (char *)devm_ioremap(platform->dev,
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	platform = snd_kcontrol_chip(kcontrol);
	if (!platform) {
...
static int sprd_soc_platform_probe(struct platform_device *pdev)
{
	...
	return devm_snd_soc_register_component(&pdev->dev, &sprd_soc_platform,
					       NULL, 0);
}

static void sprd_soc_platform_remove(struct platform_device *pdev)
{
	int i;
	struct platform_pcm_priv *priv_data = NULL;
