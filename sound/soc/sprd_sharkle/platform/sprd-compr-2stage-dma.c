...
struct sprd_compr_rtd {
	...
	uint32_t received_stage0;
	uint32_t received_stage1;
	uint32_t avail_total;
...
static void *sprd_compr_get_priv(struct snd_soc_pcm_runtime *srtd)
{
	struct snd_soc_component *component;

	component = snd_soc_rtdcom_lookup(srtd, "sprd-pcm-platform");
	if (!component)
		return NULL;
...
/* extern uint32_t audio_smem_alloc(uint32_t size);
 * extern void audio_smem_free(uint32_t addr, uint32_t size);
 */
...
static void sprd_compr_dma_buf_done(void *data)
{
	...
	if (srtd->copied_total > srtd->avail_total)
		srtd->copied_total = srtd->avail_total;
	sp_asoc_pr_info("%s, DMA copied totol=%d, avail_total=%d, buf_done_count=%d\n",
							__func__,
							srtd->copied_total,
							srtd->avail_total,
							s_buf_done_count);
	...
	unsigned int flag)
{
	...
	dma_tx_des =
			dma_chn->device->device_prep_dma_memcpy(
					dma_chn, 0, 0, 0,
...
int compr_stream_config_dma1(struct snd_compr_stream *substream,
			      uint32_t frag_size, uint32_t frag_cnt)
{
	struct snd_compr_runtime *runtime = substream->runtime;
	struct sprd_compr_rtd *srtd = runtime->private_data;
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct sprd_compr_pdata *pdata =
		sprd_compr_get_priv(rtd);
	...
	dma_chn_request = of_dma_request_slave_channel(
					pdata->dev->of_node,
					"compr_dma_stage1");
	...
	dma_config_ptr = kzalloc(
		sizeof(struct sprd_dma_cfg) * period_cnt,
		GFP_KERNEL);
	...
	for (j = 0; j < 2; j++) {
		...
		cfg_ptr->datawidth = 2;
		cfg_ptr->fragmens_len = period;
		cfg_ptr->block_len = period;
		cfg_ptr->transcation_len = srtd->params.tran_len;
		cfg_ptr->req_mode = FRAG_REQ_MODE;
#ifdef CONFIG_SPRD_COMPR_CM4_WAKE_UP
		cfg_ptr->irq_mode = TRANS_BLOCK_DONE;
#else
		cfg_ptr->irq_mode = TRANS_DONE;
#endif
		...
		if (substream->direction == SND_COMPRESS_PLAYBACK) {
			cfg_ptr->src_addr = dma_src_phys;
			cfg_ptr->des_addr = audio_addr_ap2dsp(IRAM_OFFLOAD,
							      dma_des_phys, 0);
		} else {
			cfg_ptr->src_addr = audio_addr_ap2dsp(IRAM_OFFLOAD,
							     dma_des_phys, 0);
			cfg_ptr->des_addr = dma_src_phys;
		}
		cfg_ptr->wrap_ptr = cfg_ptr->des_addr + cfg_ptr->fragmens_len;
		cfg_ptr->wrap_to = cfg_ptr->des_addr;

		cfg_ptr->link_cfg_v = (unsigned long)(srtd->dma_cfg_virt[s]);
		cfg_ptr->link_cfg_p = (unsigned long)(srtd->dma_cfg_phy[s]);
		cfg_ptr->dev_id = 0;
		/* none-wrap */
		/* dma_src_phys += srtd->params.tran_len; */
	}
	...
	sp_asoc_pr_info(
			"srtd frag_len=%d, block_len=%d, tran_len=%d\n",
...
hw_param_err:
	kfree(dma_config_ptr);
	sp_asoc_pr_info("%s, hw_param_err,ret=%d\n", __func__, ret);
			      ...
			      uint32_t frag_size)
{
	...
	/*for (j = 0; j < period_cnt; j++) {*/
	for (j = 0; j < 2; j++) {
		cfg_ptr = dma_config_ptr + j;
		cfg_ptr->datawidth = 2;
		cfg_ptr->fragmens_len = period;
		cfg_ptr->block_len = period;
		cfg_ptr->transcation_len = totsize;
		cfg_ptr->req_mode = FRAG_REQ_MODE;
#ifdef CONFIG_SPRD_COMPR_CM4_WAKE_UP
		cfg_ptr->irq_mode = TRANS_DONE;
#endif
		...
		/*dma_src_phys += cfg_ptr->block_len;*/
	}
	...
	/* get dma desc from dma config */
	srtd->dma_tx_des[s] =
			dma_chn_request->device->device_prep_dma_memcpy(
					dma_chn_request, 0, 0, 0,
					DMA_CFG_FLAG | DMA_HARDWARE_FLAG |
	...
	sp_asoc_pr_info(
			"totsize=%zu, period=%zu, hw_req_id=%d, srtd->dma_tx_des[0]=0x%p\n",
			totsize, period,
			hw_req_id,
			srtd->dma_tx_des[0]);
				    ...
				    struct snd_compr_stream *cstream)
{
	...
	sp_asoc_pr_info(
		"sprd_platform_compr_open E,pdata=0x%p, platform name=%s\n",
		pdata, snd_soc_rtdcom_lookup(rtd, "sprd-pcm-platform")->name);
	...
	sp_asoc_pr_info(
		"sprd_platform_compr_open,srtd=0x%p,cstream->direction=%d\n",
		srtd,
		cstream->direction);
	...
	atomic_set(&srtd->pause, 0);
	srtd->wake_lock = wakeup_source_register(NULL, "compr write");
				    ...
				    struct snd_compr_stream *cstream)
{
	...
	wakeup_source_unregister(srtd->wake_lock);
					...
					struct snd_compr_tstamp64 *arg)
{
	...
	memset(&tstamp, 0x0, sizeof(struct snd_compr_tstamp64));
	...
	memcpy(arg, &tstamp, sizeof(struct snd_compr_tstamp64));
	sp_asoc_pr_info(
...
{
	struct snd_soc_component *component = entry->private_data;
...
static const struct snd_compress_ops sprd_platform_compr_ops = {
...
static const struct snd_soc_component_driver sprd_platform_compr_drv = {
	.name		= "sprd-pcm-compr",
...
static int sprd_platform_compr_probe(struct platform_device *pdev)
{
	...
	ret = devm_snd_soc_register_component(&pdev->dev,
			&sprd_platform_compr_drv,
			sprd_compr_dai, ARRAY_SIZE(sprd_compr_dai));
	if (ret)
		sp_asoc_pr_info("registering compr component failed\n");
	ADEBUG();

	return ret;
}

static void sprd_platform_compr_remove(struct platform_device *pdev)
{
	struct sprd_compr_pdata *pdata = NULL;
	...
	ADEBUG();
	compr_ch_close(AUD_IPC_AGDSP);
}

static int sprd_platform_compr_suspend(struct device *dev)
{
	struct sprd_compr_pdata *pdata = NULL;

	ADEBUG();

	pdata = (struct sprd_compr_pdata *)dev_get_drvdata(dev);
	if (pdata)
		pdata->suspend = true;
...
static int sprd_platform_compr_resume(struct device *dev)
{
	struct sprd_compr_pdata *pdata = NULL;
...
static const struct dev_pm_ops sprd_compr_pm_ops = {
