				 ...
				 struct snd_kcontrol *k, int event)
{
	...
	board_ext_enable(snd_soc_dapm_to_card(w->dapm), on, BOARD_FUNC_HP);
				...
				struct snd_kcontrol *k, int event)
{
	...
	board_ext_enable(snd_soc_dapm_to_card(w->dapm), on, BOARD_FUNC_EAR);
			       ...
			       struct snd_kcontrol *k, int event)
{
	...
	board_ext_enable(snd_soc_dapm_to_card(w->dapm), on, BOARD_FUNC_SPK);
				...
				struct snd_kcontrol *k, int event)
{
	...
	board_ext_enable(snd_soc_dapm_to_card(w->dapm), on, BOARD_FUNC_SPK1);
			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	gpiod_set_value(spec_data->gpios[SMARTAMP_BOOST_GPIO_MADE], !!item);
				  ...
				  struct smartamp_boost_data *boost_data)
{
	...
	if (of_property_read_bool(np, smartamp_boost_name)) {
		...
		for (i = 0; i < ARRAY_SIZE(gpio_maps); i++) {
			...
			boost_data->gpios[gpio_maps[i].index] =
				devm_gpiod_get_index(dev, "sprd,smartamp-boost",
						     index,
						     gpio_maps[i].index ==
						     SMARTAMP_BOOST_GPIO_ENABLE ?
						     GPIOD_OUT_HIGH :
						     GPIOD_OUT_LOW);
			if (IS_ERR(boost_data->gpios[gpio_maps[i].index])) {
				ret = PTR_ERR(boost_data->gpios[gpio_maps[i].index]);
