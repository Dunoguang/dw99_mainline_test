...
#define widget_kcontrol_to_codec_drvdata(kctl) \
	snd_soc_component_get_drvdata(\
		snd_soc_dapm_kcontrol_to_component((kctl)))

/* Returns the driver data of the codec associated to @kctl which
 * belongs to a codec.
 */
#define codec_kcontrol_to_codec_drvdata(kctl) \
	snd_soc_component_get_drvdata(snd_kcontrol_chip((kctl)))
			     ...
			     int vbc_eq_idx)
{
	...
	if (vbc_codec == NULL) {
		pr_err("%s %d snd_soc_component_get_drvdata failed\n",
		       __func__, __LINE__);
		return;
	}
...
static int vbc_codec_soc_probe(struct snd_soc_component *component)
{
	struct vbc_codec_priv *vbc_codec = snd_soc_component_get_drvdata(component);
	struct vbc_equ *p_eq_setting = &vbc_codec->vbc_eq_setting;
	...
	vbc_codec->codec = component;
	p_eq_setting->codec = component;

	vbc_proc_init(component);
...
/* power down chip */
static void vbc_codec_soc_remove(struct snd_soc_component *component)
{
}
...
static int sprd_vbc_codec_probe(struct platform_device *pdev)
{
	...
	vbc_codec->wake_lock =
		wakeup_source_register(NULL, "vbc-eq-loading");
...
static int sprd_vbc_codec_remove(struct platform_device *pdev)
{
	...
	wakeup_source_unregister(vbc_codec->wake_lock);
