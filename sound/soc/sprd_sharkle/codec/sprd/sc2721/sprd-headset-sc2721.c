	  ...
	  (hdst->re_detect == true && 1 == gpio_detect_value)) {
		...
		if (hdst->headphone) {
			...
			if (hdst->re_detect == false)
				...
				__pm_wakeup_event(hdst->micbias_wakelock, 2000);
...
static irqreturn_t headset_button_irq_handler(int irq, void *dev)
{
	...
	headset_irq_button_enable(0, hdst->irq_button);
	__pm_wakeup_event(hdst->btn_wakelock, 2000);
...
static irqreturn_t headset_detect_all_irq_handler(int irq, void *dev)
{
	...
	__pm_wakeup_event(hdst->det_all_wakelock, 2000);
	hdst->gpio_det_val_last =
		gpio_get_value(pdata->gpios[HDST_GPIO_DET_ALL]);
	headset_reg_read(ANA_STS7, &val);
...
int sprd_headset_soc_probe(struct snd_soc_component *codec)
{
	...
	ret = snd_soc_card_jack_new(card, "Headset Jack",
		SPRD_HEADSET_JACK_MASK, &hdst->hdst_jack);
	if (ret) {
	...
	ret = snd_soc_card_jack_new(card, "Headset Keyboard",
		SPRD_BUTTON_JACK_MASK, &hdst->btn_jack);
	if (ret) {
	...
	hdst->det_all_wakelock = wakeup_source_register(NULL,
					"headset_detect_wakelock");
	hdst->btn_wakelock = wakeup_source_register(NULL,
					"headset_button_wakelock");
	hdst->micbias_wakelock = wakeup_source_register(NULL,
					"headset_micbias_wakelock");
