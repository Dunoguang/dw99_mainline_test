...
static int sprd_codec_auto_ldo_volt(struct snd_soc_component *codec,
...
static int sprd_codec_pga_spk_set(struct snd_soc_component *codec, int pgaval)
{
	int reg, val, mask;
			  ...
			  struct snd_info_buffer *buffer)
{
	...
	aud_glb_reg = devm_kzalloc(sprd_codec->codec->card->dev,
...
free:
	devm_kfree(sprd_codec->codec->card->dev, aud_glb_reg);
}
...
static void sprd_codec_remove(struct platform_device *pdev)
{
	struct sprd_codec_priv *sprd_codec = platform_get_drvdata(pdev);
