...
static int sprd_audio_power_probe(struct platform_device *pdev)
{
	...
	rdev = regulator_register(&pdev->dev, &info->desc, &config);
	if (IS_ERR(rdev)) {
...
static void sprd_audio_power_remove(struct platform_device *pdev)
{
...
static int __init sprd_audio_power_init(void)
{
	...
	ret = sprd_audio_power_add_devices();
	if (ret)
		return ret;

	return sprd_audio_power_regulator_init();
}

module_init(sprd_audio_power_init);
