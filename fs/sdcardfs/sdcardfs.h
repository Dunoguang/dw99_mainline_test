...
static inline int get_gid(struct super_block *sb,
		struct sdcardfs_inode_data *data)
{
	struct sdcardfs_sb_info *sbi = SDCARDFS_SB(sb);

	if (sbi->gid == AID_SDCARD_RW && !sbi->options.default_normal)
		...
		return multiuser_get_uid(data->userid, sbi->gid);
}

static inline int get_mode(struct super_block *sb,
		struct sdcardfs_inode_info *info,
		struct sdcardfs_inode_data *data)
{
	int owner_mode;
	int filtered_mode;
	struct sdcardfs_sb_info *sbi = SDCARDFS_SB(sb);
	int visible_mode = 0775 & ~sbi->mask;
