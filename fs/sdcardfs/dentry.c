 ...
 *          1: dentry is valid
 */
static int sdcardfs_d_revalidate(struct inode *dir, const struct qstr *qstr,
			struct dentry *dentry, unsigned int flags)
{
	...
	if ((lower_dentry->d_flags & DCACHE_OP_REVALIDATE)) {
		err = lower_dentry->d_op->d_revalidate(d_inode(lower_dentry->d_parent), NULL, lower_dentry, flags);
		if (err == 0) {
			goto out;
		}
	}
