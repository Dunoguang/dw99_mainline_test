	...
	struct fs_context *fc)
{
	...
	sb_info->gid = mntopts->gid;
	sb_info->mask = mntopts->mask;
	if (opts->debug) {
	...
	set_default_d_op(sb, &sdcardfs_ci_dops);
...
{
	return get_tree_nodev(fc, __sdcardfs_fill_super);
}
...
static int sdcardfs_reconfigure_context(struct fs_context *fc)
{
	...
	sbi->gid = fc_opts->vfsopts.gid;
	sbi->mask = fc_opts->vfsopts.mask;
...
static struct file_system_type sdcardfs_fs_type = {
	.owner		= THIS_MODULE,
	.name		= SDCARDFS_NAME,
	.parameters	= sdcardfs_param_specs,
