...
static unsigned long pressure_min = 1; /* VMPRESSURE_MEDIUM */
static unsigned long pressure_max = 2; /* VMPRESSURE_CRITICAL */
...
module_param_named(queue_work_time, queue_work_time, ulong, S_IRUGO);

static unsigned long swap_total_scan;
static unsigned long swap_total_reclaim;
module_param_named(swap_total_scan, swap_total_scan, ulong, S_IRUGO);
module_param_named(swap_total_reclaim, swap_total_reclaim, ulong, S_IRUGO);
...
module_param_named(swap_high_ratio, swap_high_ratio, int, S_IRUGO | S_IWUSR);

static int swap_low_ratio = DEF_SWAP_LOW_RATION;
module_param_named(swap_low_ratio, swap_low_ratio, int, S_IRUGO | S_IWUSR);
...
static void swap_fn(struct work_struct *work)
{
	...
	if (info.totalswap) {
		...
		if (swap_used_ratio > swap_high_ratio) {
			pr_info("process reclaim, swap free is low,"
				" skip reclaim\n");
			return;
			...
			now_per_swap_size =
				per_swap_size * (swap_high_ratio - swap_used_ratio);
			...
			unsigned long action, void *data)
{
	...
	if ((pressure >= pressure_min) && (pressure < pressure_max))
		if (!work_pending(&swap_work)) {
			if (pr_debug)
				pr_info("pr queue work at vmpressure %lu\n",
					pressure);
