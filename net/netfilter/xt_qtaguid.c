			    ...
			    const struct net_device **el_dev)
{
	...
	if (par->state->in) {
		*el_dev = par->state->in;
		*direction = IFS_RX;
	} else if (par->state->out) {
		*el_dev = par->state->out;
		*direction = IFS_TX;
	} else {
	...
	if (skb->dev && *el_dev != skb->dev) {
		MT_DEBUG("qtaguid[%d]: skb->dev=%p %s vs par->%s=%p %s\n",
			 par->state->hook, skb->dev, skb->dev->name,
				       ...
				       struct xt_action_param *par)
{
	...
	MT_DEBUG("qtaguid[%d]: iface_stat: %s(%s): "
		 ...
		 par->state->hook, __func__, el_dev->name, el_dev->type,
	...
	if (entry == NULL) {
		IF_DEBUG("qtaguid[%d]: iface_stat: %s(%s): not tracked\n",
			 par->state->hook, __func__, el_dev->name);
		spin_unlock_bh(&iface_stat_list_lock);
		return;
	}
				    ...
				    struct xt_action_param *par)
{
	...
	MT_DEBUG("qtaguid[%d]: find_sk(skb=%p) family=%d\n",
		 par->state->hook, skb, par->state->pf);
	...
	if (sk) {
		MT_DEBUG("qtaguid[%d]: %p->sk_proto=%u->sk_state=%d\n",
			 par->state->hook, sk, sk->sk_protocol, sk->sk_state);
		/*
...
static bool qtaguid_mt(const struct sk_buff *skb, struct xt_action_param *par)
{
	...
	if (info->match & XT_QTAGUID_GID) {
		...
		MT_DEBUG("qtaguid[%d]: sk=%p->sk_socket=%p->file=%p\n",
			 par->state->hook, sk, sk->sk_socket,
...
static int ctrl_cmd_untag(const char *input)
{
	...
	CT_DEBUG("qtaguid: ctrl_untag(%s): socket->...->f_count=%ld ->sk=%p\n",
		 input, file_count(el_socket->file),
		 el_socket->sk);
