	...
	for (i = 0; i < ARRAY_SIZE(imsbr_msglists); i++) {
		...
		if (!strcmp(cmd, list->name)) {
			if (list->req_len > msghdr->imsbr_paylen) {
				...
				pr_info("req_len is %d, but paylen is %d, padding...\n",
...
static const struct proc_ops imsbr_flow_fops = {
	.proc_open    = imsbr_flow_open,
	.proc_read    = seq_read,
...
static const struct proc_ops imsbr_stat_fops = {
	.proc_open    = imsbr_stat_open,
	.proc_read    = seq_read,
	.proc_lseek   = seq_lseek,
