...
static const struct proc_ops xfrm_dec_tcpdump_fops = {
...
static int __init iptable_vowifi_ipsec_dec_dump_init(void)
{
	...
	err = nf_register_net_hooks(&init_net, nf_ipv4_ipsec_dec_dump_ops,
...
{
	nf_unregister_net_hooks(&init_net, nf_ipv4_ipsec_dec_dump_ops,
			    ARRAY_SIZE(nf_ipv4_ipsec_dec_dump_ops));
