...
static int nf_xfrm6_input_decode_cap_log(struct sk_buff *skb)
{
	...
	cr_xfrm_depth = skb_sec_path(skb)->len - 1;
	if (unlikely(cr_xfrm_depth < 0))
		return ret;

	x = skb_sec_path(skb)->xvec[cr_xfrm_depth];
					      ...
					      const struct nf_hook_state *state)
{
	...
	if (list_empty(&state->net->ptype_all))
		return NF_ACCEPT;
...
static const struct proc_ops xfrm6_dec_tcpdump_fops = {
	.proc_open  = tcpdump_opt_proc_open,
	.proc_read  = seq_read,
	.proc_write  = tcpdump_opt_proc_write,
...
static int __init ip6table_vowifi_ipsec_dec_dump_init(void)
{
	...
	err = nf_register_net_hooks(&init_net, nf_ipv6_ipsec_dec_dump_ops,
...
{
	nf_unregister_net_hooks(&init_net, nf_ipv6_ipsec_dec_dump_ops,
			    ARRAY_SIZE(nf_ipv6_ipsec_dec_dump_ops));
