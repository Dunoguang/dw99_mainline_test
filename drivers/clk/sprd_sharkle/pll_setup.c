...
static int sprd_adjustable_pll_determine_rate(struct clk_hw *hw,
					       struct clk_rate_request *req)
{
	req->rate = sprd_adjustable_pll_round_rate(hw, req->rate,
						   &req->best_parent_rate);
	return 0;
}

const struct clk_ops sprd_clk_sc9838_adjustable_pll_ops = {
	.determine_rate = sprd_adjustable_pll_determine_rate,
