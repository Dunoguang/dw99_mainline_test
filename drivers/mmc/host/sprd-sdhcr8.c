...
static void sprd_sdhc_timeout(struct timer_list *t)
{
	struct sprd_sdhc_host *host = container_of(t, struct sprd_sdhc_host, timer);
	unsigned long flags;
...
static int sprd_sdhc_execute_tuning(struct mmc_host *mmc, u32 opcode)
{
	...
	pr_info("%s(%s): the best step %d, phase 0x%08x, delay value 0x%08x\n",
...
static const struct mmc_host_ops sprd_sdhc_ops = {
	...
	.card_hw_reset = sprd_sdhc_emmc_hw_reset,
...
static int sprd_get_ext_resource(struct sprd_sdhc_host *host)
{
	...
	/* init timer */
	timer_setup(&host->timer, sprd_sdhc_timeout, 0);
				...
				struct mmc_host *mmc)
{
	...
	mmc_of_parse(mmc);
	mmc_of_parse_voltage(mmc, &host->ocr_mask);
    /*
...
static void sprd_sdhc_remove(struct platform_device *pdev)
{
	struct sprd_sdhc_host *host = platform_get_drvdata(pdev);
	struct mmc_host *mmc = host->mmc;
