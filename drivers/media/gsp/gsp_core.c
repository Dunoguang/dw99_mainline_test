...
int gsp_core_verify(struct gsp_core *core)
{
	...
	ret = (core->id > gsp->core_cnt) || (0 > core->id) ? 1 : 0;
...
void gsp_core_recover(struct kthread_work *work)
{
	...
	GSP_INFO("enter gsp_core_recover, with state: %d\n",
...
void gsp_core_hang_handler(struct timer_list *t)
{
	struct gsp_core *core = container_of(t, struct gsp_core, timer);
	gsp_core_state_set(core, CORE_STATE_HW_HANG_ERR);
	kthread_queue_work(&core->kworker, &core->recover);
}
