...
#include "../gsp_layer.h"
#include "gsp_r4p0_core.h"
#include "gsp_r4p0_reg.h"
#include "gsp_r4p0_coef_cal.h"
#include "../gsp_interface.h"
#include "../gsp_interface/gsp_interface_iwhale2.h"

static int gsp_r4p0_core_cfg_is_err(struct gsp_core *core)
{
	struct R4P0_GSP_GLB_CFG_REG gsp_glb_cfg_value;

	gsp_glb_cfg_value.value =
		gsp_core_reg_read(R4P0_GSP_GLB_CFG(core->base));
	return gsp_glb_cfg_value.ERR_FLG;
}

static void gsp_r4p0_core_err_code_print(struct gsp_core *core)
{
	struct R4P0_GSP_GLB_CFG_REG gsp_glb_cfg_value;
...
static void print_image_layer_cfg(struct gsp_r4p0_img_layer *layer)
{
	struct gsp_r4p0_img_layer_params *params = NULL;
	...
	GSP_DEBUG("img_format[%d,%d], scaling_en[%d], pitch[%d, %d]\n",
		params->img_format, params->vfbc_mode, params->scaling_en,
...
static void print_osd_layer_cfg(struct gsp_r4p0_osd_layer *layer)
{
	struct gsp_r4p0_osd_layer_params *params = NULL;
	...
	GSP_DEBUG("osd_format[%d,%d], zorder[%d], pitch[%d, %d]\n",
		params->osd_format, params->vfbc_mode, params->zorder,
		params->pitch, params->height);
...
static void print_des_layer_cfg(struct gsp_r4p0_des_layer *layer)
{
	struct gsp_r4p0_des_layer_params *params = NULL;

	params = &layer->params;
	gsp_layer_common_print(&layer->common);

	GSP_DEBUG("des_format[%d, %d], cr8_en[%d], pitch[%d, %d]\n",
		params->img_format, params->fbc_mod, params->compress_r8_en,
		params->pitch, params->height);
...
static void gsp_r4p0_core_cfg_print(struct gsp_r4p0_cfg *cfg)
{
	int icnt = 0;
...
void gsp_r4p0_core_dump(struct gsp_core *c)
{
	int icnt = 0;
	struct R4P0_GSP_CTL_REG_T reg_struct;

	memset(&reg_struct, 0, sizeof(struct R4P0_GSP_CTL_REG_T));

	reg_struct.glb_cfg.value = gsp_core_reg_read(R4P0_GSP_GLB_CFG(c->base));
	reg_struct.int_cfg.value = gsp_core_reg_read(R4P0_GSP_INT(c->base));
	reg_struct.cmd_sts.value = gsp_core_reg_read(R4P0_GSP_CMD_STS(c->base));
	reg_struct.cmd_coef1_addr.value =
		gsp_core_reg_read(R4P0_GSP_CMD_COEF1_ADDR(c->base));
	reg_struct.cmd_coef2_addr.value =
		gsp_core_reg_read(R4P0_GSP_CMD_COEF2_ADDR(c->base));
	reg_struct.cmd_addr.value =
	gsp_core_reg_read(R4P0_GSP_CMD_ADDR(c->base));
	reg_struct.mod_cfg.value = gsp_core_reg_read(R4P0_GSP_MOD_CFG(c->base));
	reg_struct.secure_cfg.value =
		gsp_core_reg_read(R4P0_GSP_SECURE_CFG(c->base));

	reg_struct.des1_data_cfg.value =
		gsp_core_reg_read(R4P0_DES_DATA_CFG(c->base));
	reg_struct.des1_y_addr.value =
		gsp_core_reg_read(R4P0_DES_Y_ADDR(c->base));
	reg_struct.des1_u_addr.value =
		gsp_core_reg_read(R4P0_DES_U_ADDR(c->base));
	reg_struct.des1_v_addr.value =
		gsp_core_reg_read(R4P0_DES_V_ADDR(c->base));
	reg_struct.des1_pitch.value =
		gsp_core_reg_read(R4P0_DES_PITCH(c->base));
	reg_struct.back_rgb.value =
		gsp_core_reg_read(R4P0_BACK_RGB(c->base));
	reg_struct.des_scl_size1.value =
		gsp_core_reg_read(R4P0_DES_SCL_SIZE1(c->base));
	reg_struct.des_scl_size2.value =
		gsp_core_reg_read(R4P0_DES_SCL_SIZE2(c->base));
	reg_struct.work_area_size1.value =
		gsp_core_reg_read(R4P0_WORK_AREA_SIZE1(c->base));
	reg_struct.work_area_xy1.value =
		gsp_core_reg_read(R4P0_WORK_AREA_XY1(c->base));
	reg_struct.work_area_size2.value =
		gsp_core_reg_read(R4P0_WORK_AREA_SIZE2(c->base));
	reg_struct.work_area_sxy2.value =
		gsp_core_reg_read(R4P0_WORK_AREA_SXY2(c->base));
	reg_struct.work_area_dxy2.value =
		gsp_core_reg_read(R4P0_WORK_AREA_DXY2(c->base));

	for (icnt = 0; icnt < R4P0_IMGL_NUM; icnt++) {
		reg_struct.limg_cfg[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_CFG(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		if (reg_struct.limg_cfg[icnt].Limg_en == 0)
			continue;
		reg_struct.limg_y_addr[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_Y_ADDR(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_u_addr[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_U_ADDR(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_v_addr[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_V_ADDR(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_pitch[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_PITCH(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_clip_start[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_CLIP_START(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_clip_size[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_CLIP_SIZE(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_des_start[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_DES_START(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_pallet_rgb[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_PALLET_RGB(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.limg_ck[icnt].value =
			gsp_core_reg_read(R4P0_LIMG_CK(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.y2y_y_param[icnt].value =
			gsp_core_reg_read(R4P0_Y2Y_Y_PARAM(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.y2y_u_param[icnt].value =
			gsp_core_reg_read(R4P0_Y2Y_U_PARAM(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
		reg_struct.y2y_v_param[icnt].value =
			gsp_core_reg_read(R4P0_Y2Y_V_PARAM(
				(c->base + icnt * R4P0_LIMG_OFFSET)));
	}

	for (icnt = 0; icnt < R4P0_OSDL_NUM; icnt++) {
		reg_struct.losd_cfg[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_CFG(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		if (reg_struct.losd_cfg[icnt].Losd_en == 0)
			continue;
		reg_struct.losd_r_addr[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_R_ADDR(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		reg_struct.losd_pitch[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_PITCH(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		reg_struct.losd_clip_start[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_CLIP_START(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		reg_struct.losd_clip_size[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_CLIP_SIZE(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		reg_struct.losd_des_start[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_DES_START(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		reg_struct.losd_pallet_rgb[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_PALLET_RGB(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
		reg_struct.losd_ck[icnt].value =
			gsp_core_reg_read(R4P0_LOSD_CK(
				(c->base + icnt * R4P0_LOSD_OFFSET)));
	}

	reg_struct.ip_rev.value =
		gsp_core_reg_read(R4P0_GSP_IP_REV(c->base));
	reg_struct.axi_sts.value =
		gsp_core_reg_read(R4P0_GSP_AXI_STS(c->base));
	reg_struct.debug_cfg.value =
		gsp_core_reg_read(R4P0_GSP_DEBUG_CFG(c->base));
	reg_struct.debug1_cfg.value =
		gsp_core_reg_read(R4P0_GSP_DEBUG1(c->base));
	reg_struct.debug2_cfg.value =
		gsp_core_reg_read(R4P0_GSP_DEBUG2(c->base));
	...
	GSP_DEBUG("ARQOSH[0x%x], ARQOSL[0x%x], AWQOS[0x%x]\n",
		reg_struct.mod_cfg.ARQOSH, reg_struct.mod_cfg.ARQOSL,
		reg_struct.mod_cfg.AWQOS);

	GSP_DEBUG("RAND_ID_MOD[0x%x], GAP_RB[0x%x], GAP_WB[0x%x]\n",
		reg_struct.glb_cfg.RAND_ID_MOD, reg_struct.glb_cfg.GAP_RB,
		reg_struct.glb_cfg.GAP_WB);

	GSP_DEBUG("BLK_TIMER[0x%x], GSP_AXI_STS[0x%x], GSP_DEBUG_CFG[0x%x]\n",
		reg_struct.mod_cfg.BLK_TIMER, reg_struct.axi_sts.value,
		reg_struct.debug_cfg.value);

	GSP_DEBUG("GSP_DEBUG1[0x%x], GSP_DEBUG2[0x%x]\n",
	...
	if (reg_struct.glb_cfg.CMD_EN == 1) {
		...
		GSP_DUMP("cmd_coef1_addr[0x%x], cmd_coef2_addr[0x%x]\n",
			reg_struct.cmd_coef1_addr.value,
			reg_struct.cmd_coef2_addr.value);
		...
		GSP_DUMP("CR8[%d, %d], rot_mod[%d], des_format[%d, %d]\n",
			...
			reg_struct.des1_data_cfg.BCOMP_MOD);
		...
		GSP_DUMP("des_scl_size2[%d, %d, %d, %d, %d]\n",
			reg_struct.mod_cfg.SCALE2_EN,
			reg_struct.des_scl_size2.VTAP_MOD,
			reg_struct.des_scl_size2.DES_SCL_H,
			reg_struct.des_scl_size2.HTAP_MOD,
			reg_struct.des_scl_size2.DES_SCL_W);
		...
		for (icnt = 0; icnt < R4P0_IMGL_NUM; icnt++) {
			...
			reg_struct.limg_cfg[icnt].VFBC_MOD,
			...
			GSP_DUMP("ZNUM_L[%d, %d], des_rect[%d, %d]\n",
			reg_struct.limg_cfg[icnt].ZNUM_L, icnt,
		...
		for (icnt = 0; icnt < R4P0_OSDL_NUM; icnt++) {
			...
			reg_struct.losd_cfg[icnt].VFBC_MOD,
			...
			GSP_DUMP("ZNUM_L[%d, %d], des_rect[%d, %d]\n",
			reg_struct.losd_cfg[icnt].ZNUM_L, icnt,
...
static void gsp_r4p0_core_cfg_reinit(struct gsp_r4p0_cfg *cfg)
{
	...
	GSP_DEBUG("gsp_r4p0 cfg parent kcfg[%d]\n", gsp_kcfg_to_tag(kcfg));
	if (cfg->common.init != 1) {
		GSP_ERR("gsp_r4p0 cfg[%d] has not been initialized\n",
			cfg->common.tag);
		return;
	}
	...
	for (icnt = 0; icnt < R4P0_IMGL_NUM; icnt++)
		memset(&cfg->limg[icnt].params, 0,
			sizeof(struct gsp_r4p0_img_layer_params));
	for (icnt = 0; icnt < R4P0_OSDL_NUM; icnt++)
		memset(&cfg->losd[icnt].params, 0,
			sizeof(struct gsp_r4p0_osd_layer_params));
	memset(&cfg->ld1.params, 0, sizeof(struct gsp_r4p0_des_layer_params));
}

static int gsp_r4p0_core_capa_init(struct gsp_core *core)
{
	struct gsp_r4p0_capability *capa = NULL;
	struct gsp_rect max = {0, 0, 8191, 8191};
	struct gsp_rect min = {0, 0, 4, 4};

	capa = (struct gsp_r4p0_capability *)core->capa;

	/* common information initialize */
	capa->common.magic = GSP_CAPABILITY_MAGIC;
	strcpy(capa->common.version, "R4P0");
	capa->common.capa_size = sizeof(struct gsp_r4p0_capability);

	capa->common.max_layer = R4P0_IMGL_NUM + R4P0_OSDL_NUM;
	capa->common.max_img_layer = R4P0_IMGL_NUM;
...
static void gsp_r4p0_int_clear_and_disable(struct gsp_core *core)
{
	struct R4P0_GSP_INT_REG gsp_int_value;
	struct R4P0_GSP_INT_REG gsp_int_mask;

	if (core == NULL) {
		GSP_ERR("r4p0 interrupt clear with null core\n");
		return;
	}
	...
	gsp_core_reg_update(R4P0_GSP_INT(core->base),
			gsp_int_value.value, gsp_int_mask.value);

}

static void gsp_r4p0_coef_cache_init(struct gsp_r4p0_core *core)
{
	uint32_t i = 0;
	...
	if (core->cache_coef_init_flag == 0) {
		...
		while (i < R4P0_GSP_COEF_CACHE_MAX) {
...
static void gsp_r4p0_core_cfg_init(struct gsp_r4p0_cfg *cfg,
					struct gsp_kcfg *kcfg)
{
	...
	cfg->common.layer_num = R4P0_IMGL_NUM + R4P0_OSDL_NUM;
	cfg->common.init = 1;
...
int gsp_r4p0_core_init(struct gsp_core *core)
{
	int ret = 0;
	struct gsp_kcfg *kcfg = NULL;
	struct gsp_r4p0_core *c = (struct gsp_r4p0_core *)core;
	...
	gsp_r4p0_core_capa_init(core);

	gsp_r4p0_coef_cache_init(c);

	/* to do some specific initialize operations here */
	list_for_each_entry(kcfg, &core->kcfgs, sibling) {
		gsp_r4p0_core_cfg_init((struct gsp_r4p0_cfg *)kcfg->cfg, kcfg);
	}

	return ret;
}

int gsp_r4p0_core_alloc(struct gsp_core **core, struct device_node *node)
{
	void *capa = NULL;
	struct platform_device *pdev = NULL;

	if (IS_ERR_OR_NULL(core)) {
		GSP_ERR("r4p0 core alloc params error\n");
		return -1;
	}
	...
	if (IS_ERR_OR_NULL(pdev)) {
		GSP_ERR("find gsp_r4p0 device by child node failed\n");
		return -1;
	}
	*core = devm_kzalloc(&pdev->dev, sizeof(struct gsp_r4p0_core),
						GFP_KERNEL);
	if (IS_ERR_OR_NULL(*core)) {
	...
	memset(*core, 0, sizeof(struct gsp_r4p0_core));
	(*core)->cfg_size = sizeof(struct gsp_r4p0_cfg);
	(*core)->dev = &pdev->dev;
	if (dev_name(&pdev->dev))
		GSP_INFO("core[%d] device name: %s\n", (*core)->id,
			 dev_name(&pdev->dev));

	capa = kzalloc(sizeof(struct gsp_r4p0_capability), GFP_KERNEL);
	if (IS_ERR_OR_NULL(capa)) {
		GSP_ERR("gsp_r4p0 capability memory allocated failed\n");
		devm_kfree(&pdev->dev, *core);
		return -1;
	}
	(*core)->capa = capa;
	(*core)->capa->capa_size = sizeof(struct gsp_r4p0_capability);
	(*core)->node = node;
	GSP_ERR("gsp_r4p0_cap_size = %d\n", (int)(*core)->capa->capa_size);
	GSP_INFO("gsp_r4p0 core allocate success\n");
	return 0;
}

static void gsp_r4p0_core_irq_enable(struct gsp_core *core)
{
	struct R4P0_GSP_INT_REG gsp_int_value;
	struct R4P0_GSP_INT_REG gsp_int_mask;
...
static void gsp_r4p0_core_irq_disable(struct gsp_core *core)
{
	struct R4P0_GSP_INT_REG gsp_int_value;
	struct R4P0_GSP_INT_REG gsp_int_mask;
...
static irqreturn_t gsp_r4p0_core_irq_handler(int irq, void *data)
{
	...
	gsp_r4p0_int_clear_and_disable(core);
...
int gsp_r4p0_core_enable(struct gsp_core *c)
{
	int ret = -1;
	struct gsp_r4p0_core *core = NULL;

	core = (struct gsp_r4p0_core *)c;
...
void gsp_r4p0_core_disable(struct gsp_core *c)
{
	struct gsp_r4p0_core *core = NULL;

	core = (struct gsp_r4p0_core *)c;
	gsp_r4p0_core_irq_disable(c);

	clk_disable_unprepare(core->gsp_eb_clk);
	clk_disable_unprepare(core->gsp_clk);
}

static int gsp_r4p0_core_parse_clk(struct gsp_r4p0_core *core)
{
	int status = 0;

	if (core->common.id == 0) {
		core->gsp_clk = of_clk_get_by_name(core->common.node,
				R4P0_GSP0_CLOCK_NAME);
		core->gsp_eb_clk = of_clk_get_by_name(core->common.node,
				R4P0_GSP0_EB_CLOCK_NAME);
	} else {
		core->gsp_clk = of_clk_get_by_name(core->common.node,
				R4P0_GSP1_CLOCK_NAME);
		core->gsp_eb_clk = of_clk_get_by_name(core->common.node,
				R4P0_GSP1_EB_CLOCK_NAME);
	}
	core->gsp_clk_parent = of_clk_get_by_name(core->common.node,
				R4P0_GSP_CLOCK_PARENT);
...
static int gsp_r4p0_core_parse_irq(struct gsp_core *core)
{
	int ret = -1;
	struct device *dev = NULL;
	static const char * const r4p0_irq_name[] = {
		"GSP0", "GSP1"
	};
	...
	if (core->irq)
		ret = devm_request_irq(dev, core->irq,
				gsp_r4p0_core_irq_handler,
				IRQF_SHARED, r4p0_irq_name[core->id], core);

	if (ret)
		GSP_ERR("r4p0 core[%d] request irq failed! %d\n",
		core->id, core->irq);

	return ret;
}

int gsp_r4p0_core_parse_dt(struct gsp_core *core)
{
	int ret = -1;
	struct device *dev = NULL;
	struct gsp_r4p0_core *r4p0_core = NULL;
	struct resource r;

	dev = container_of(&core->node, struct device, of_node);
	r4p0_core = (struct gsp_r4p0_core *)core;

	r4p0_core->gsp_ctl_reg_base = core->base;
	GSP_INFO("gsp_ctl_reg_base: 0x%p\n", core->base);

	if (of_address_to_resource(core->node, 1, &r)) {
		GSP_ERR("map vsp sys domain reg failed,Line(%d).\n", __LINE__);
		ret |= -1;
		return ret;
	}
	r4p0_core->vsp_sys_regmap =
		ioremap(r.start, resource_size(&r));
	GSP_INFO("vsp_sys_regmap(0xD1100000): 0x%p\n",
		r4p0_core->vsp_sys_regmap);
	if (IS_ERR(r4p0_core->vsp_sys_regmap)) {
		GSP_ERR("map vsp sys domain reg failed,Line(%d).\n", __LINE__);
		ret |= -1;
	}

	ret = gsp_r4p0_core_parse_irq(core);
	if (ret) {
		GSP_ERR("core[%d] parse irq failed\n", core->id);
		return ret;
	}

	gsp_r4p0_core_parse_clk(r4p0_core);

	return ret;
}


static void gsp_r4p0_core_misc_reg_set(struct gsp_core *core,
			struct gsp_r4p0_cfg *cfg)
{
	void __iomem *base = NULL;
	struct R4P0_DES_SCL_SIZE1_REG des_scl_size_value;
	struct R4P0_DES_SCL_SIZE1_REG des_scl_size_mask;
	struct R4P0_WORK_AREA_XY1_REG  work_area_xy1_value;
	struct R4P0_WORK_AREA_XY1_REG work_area_xy1_mask;
	struct R4P0_WORK_AREA_SIZE1_REG work_area_size1_value;
	struct R4P0_WORK_AREA_SIZE1_REG work_area_size1_mask;
	struct R4P0_WORK_AREA_SXY2_REG  work_area_sxy2_value;
	struct R4P0_WORK_AREA_SXY2_REG work_area_sxy2_mask;
	struct R4P0_WORK_AREA_SIZE2_REG work_area_size2_value;
	struct R4P0_WORK_AREA_SIZE2_REG work_area_size2_mask;
	struct R4P0_WORK_AREA_DXY2_REG work_area_dxy2_value;
	struct R4P0_WORK_AREA_DXY2_REG work_area_dxy2_mask;
	struct R4P0_GSP_MOD_CFG_REG gsp_mod_cfg_value;
	struct R4P0_GSP_MOD_CFG_REG gsp_mod_cfg_mask;
	int icnt = 0;
	...
	for (icnt = 0; icnt < R4P0_IMGL_NUM; icnt++) {
		...
		des_scl_size_value.VTAP_MOD =
			cfg->misc.scale_para[icnt].vtap_mod;
		des_scl_size_value.DES_SCL_H =
			cfg->misc.scale_para[icnt].scale_rect_out.rect_h;
		des_scl_size_value.HTAP_MOD =
			cfg->misc.scale_para[icnt].htap_mod;
		des_scl_size_value.DES_SCL_W =
			cfg->misc.scale_para[icnt].scale_rect_out.rect_w;
		...
		gsp_core_reg_update(R4P0_DES_SCL_SIZE1(
			(base + icnt * R4P0_DES_SCL_SIZE_OFFSET)),
	...
	gsp_core_reg_update(R4P0_WORK_AREA_XY1(base),
	...
	gsp_core_reg_update(R4P0_WORK_AREA_SIZE1(base),
	...
	gsp_core_reg_update(R4P0_WORK_AREA_SXY2(base),
	...
	gsp_core_reg_update(R4P0_WORK_AREA_SIZE2(base),
	...
	gsp_core_reg_update(R4P0_WORK_AREA_DXY2(base),
	...
	gsp_mod_cfg_value.SCALE1_EN = cfg->misc.scale_para[0].scale_en;
	gsp_mod_cfg_value.SCALE2_EN = cfg->misc.scale_para[1].scale_en;
	...
	gsp_mod_cfg_mask.SCALE2_EN = 0x1;
	gsp_mod_cfg_mask.PMARGB_EN = 0x1;
	gsp_core_reg_update(R4P0_GSP_MOD_CFG(base),
		gsp_mod_cfg_value.value, gsp_mod_cfg_mask.value);

}

static void gsp_r4p0_core_limg_reg_set(void __iomem *base,
			   struct gsp_r4p0_img_layer *layer, int layer_index)
{
	struct R4P0_LAYERIMG_CFG_REG limg_cfg_value;
	struct R4P0_LAYERIMG_CFG_REG limg_cfg_mask;
	struct R4P0_LAYERIMG_Y_ADDR_REG limg_y_addr_value;
	struct R4P0_LAYERIMG_Y_ADDR_REG limg_y_addr_mask;
	struct R4P0_LAYERIMG_U_ADDR_REG limg_u_addr_value;
	struct R4P0_LAYERIMG_U_ADDR_REG limg_u_addr_mask;
	struct R4P0_LAYERIMG_V_ADDR_REG limg_v_addr_value;
	struct R4P0_LAYERIMG_V_ADDR_REG limg_v_addr_mask;
	struct R4P0_LAYERIMG_PITCH_REG limg_pitch_value;
	struct R4P0_LAYERIMG_PITCH_REG limg_pitch_mask;
	struct R4P0_LAYERIMG_CLIP_SIZE_REG limg_clip_size_value;
	struct R4P0_LAYERIMG_CLIP_SIZE_REG limg_clip_size_mask;
	struct R4P0_LAYERIMG_CLIP_START_REG limg_clip_start_value;
	struct R4P0_LAYERIMG_CLIP_START_REG limg_clip_start_mask;
	struct R4P0_LAYERIMG_DES_START_REG limg_des_start_value;
	struct R4P0_LAYERIMG_DES_START_REG limg_des_start_mask;
	struct R4P0_LAYERIMG_CK_REG limg_ck_value;
	struct R4P0_LAYERIMG_CK_REG limg_ck_mask;
	struct gsp_r4p0_img_layer_params	*limg_params = NULL;
	...
	if (layer->common.enable != 1) {
		...
		gsp_core_reg_update(R4P0_LIMG_CFG(
			(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_Y_ADDR(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_U_ADDR(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_V_ADDR(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_PITCH(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_CLIP_SIZE(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_CLIP_START(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_DES_START(
		(base + layer_index * R4P0_LIMG_OFFSET)),
		limg_des_start_value.value, limg_des_start_mask.value);

	/* limg pallet rgb set */
	if (limg_params->pallet_en) {
		struct R4P0_LAYERIMG_PALLET_RGB_REG limg_pallet_value;
		struct R4P0_LAYERIMG_PALLET_RGB_REG limg_pallet_mask;
		...
		gsp_core_reg_update(R4P0_LIMG_PALLET_RGB(
			(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LIMG_CK(
		(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	if (limg_params->y2y_mod == 0) {
		struct R4P0_Y2Y_Y_PARAM_REG y2y_y_para_value;
		struct R4P0_Y2Y_Y_PARAM_REG y2y_y_para_mask;
		struct R4P0_Y2Y_U_PARAM_REG y2y_u_para_value;
		struct R4P0_Y2Y_U_PARAM_REG y2y_u_para_mask;
		struct R4P0_Y2Y_V_PARAM_REG y2y_v_para_value;
		struct R4P0_Y2Y_V_PARAM_REG y2y_v_para_mask;
		...
		gsp_core_reg_update(R4P0_Y2Y_Y_PARAM(
			(base + layer_index * R4P0_LIMG_OFFSET)),
		...
		gsp_core_reg_update(R4P0_Y2Y_U_PARAM(
			(base + layer_index * R4P0_LIMG_OFFSET)),
		...
		gsp_core_reg_update(R4P0_Y2Y_V_PARAM(
			(base + layer_index * R4P0_LIMG_OFFSET)),
	...
	limg_cfg_value.VFBC_MOD = limg_params->vfbc_mode;
	...
	limg_cfg_mask.VFBC_MOD = 0x3;
	limg_cfg_mask.Y2R_MOD = 0x7;
	limg_cfg_mask.Y2Y_MOD = 0x1;
	limg_cfg_mask.ZNUM_L = 0x7;
	limg_cfg_mask.Limg_en = 0x1;
	gsp_core_reg_update(R4P0_LIMG_CFG(
		(base + layer_index * R4P0_LIMG_OFFSET)),
		limg_cfg_value.value, limg_cfg_mask.value);
...
static void gsp_r4p0_core_losd_reg_set(void __iomem *base,
			   struct gsp_r4p0_osd_layer *layer, int layer_index)
{
	struct R4P0_LAYEROSD_CFG_REG losd_cfg_value;
	struct R4P0_LAYEROSD_CFG_REG losd_cfg_mask;
	struct R4P0_LAYEROSD_R_ADDR_REG losd_r_addr_value;
	struct R4P0_LAYEROSD_R_ADDR_REG losd_r_addr_mask;
	struct R4P0_LAYEROSD_PITCH_REG losd_pitch_value;
	struct R4P0_LAYEROSD_PITCH_REG losd_pitch_mask;
	struct R4P0_LAYEROSD_CLIP_SIZE_REG losd_clip_size_value;
	struct R4P0_LAYEROSD_CLIP_SIZE_REG losd_clip_size_mask;
	struct R4P0_LAYEROSD_CLIP_START_REG losd_clip_start_value;
	struct R4P0_LAYEROSD_CLIP_START_REG losd_clip_start_mask;
	struct R4P0_LAYEROSD_DES_START_REG losd_des_start_value;
	struct R4P0_LAYEROSD_DES_START_REG losd_des_start_mask;
	struct R4P0_LAYEROSD_CK_REG losd_ck_value;
	struct R4P0_LAYEROSD_CK_REG losd_ck_mask;
	struct gsp_r4p0_osd_layer_params	*losd_params = NULL;
	...
	if (layer->common.enable != 1) {
		...
		gsp_core_reg_update(R4P0_LOSD_CFG(
			(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LOSD_R_ADDR(
		(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LOSD_PITCH(
		(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LOSD_CLIP_START(
		(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LOSD_CLIP_SIZE(
		(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LOSD_DES_START(
		(base + layer_index * R4P0_LOSD_OFFSET)),
		losd_des_start_value.value, losd_des_start_mask.value);

	/* losd pallet set */
	if (losd_params->pallet_en) {
		struct R4P0_LAYEROSD_PALLET_RGB_REG losd_pallet_value;
		struct R4P0_LAYEROSD_PALLET_RGB_REG losd_pallet_mask;
		...
		gsp_core_reg_update(R4P0_LOSD_PALLET_RGB(
			(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	gsp_core_reg_update(R4P0_LOSD_CK(
		(base + layer_index * R4P0_LOSD_OFFSET)),
	...
	losd_cfg_value.VFBC_MOD = losd_params->vfbc_mode;
	...
	losd_cfg_mask.VFBC_MOD = 0x1;
	losd_cfg_mask.ZNUM_L = 0x7;
	losd_cfg_mask.Losd_en = 0x1;
	gsp_core_reg_update(R4P0_LOSD_CFG(
		(base + layer_index * R4P0_LOSD_OFFSET)),
		losd_cfg_value.value, losd_cfg_mask.value);
...
static void gsp_r4p0_core_ld1_reg_set(void __iomem *base,
			   struct gsp_r4p0_des_layer *layer)
{
	struct R4P0_DES_DATA_CFG1_REG des_cfg_value;
	struct R4P0_DES_DATA_CFG1_REG des_cfg_mask;
	struct R4P0_DES_Y_ADDR1_REG des_y_addr_value;
	struct R4P0_DES_Y_ADDR1_REG des_y_addr_mask;
	struct R4P0_DES_U_ADDR1_REG des_u_addr_value;
	struct R4P0_DES_U_ADDR1_REG des_u_addr_mask;
	struct R4P0_DES_V_ADDR1_REG des_v_addr_value;
	struct R4P0_DES_V_ADDR1_REG des_v_addr_mask;
	struct R4P0_DES_PITCH1_REG des_pitch_value;
	struct R4P0_DES_PITCH1_REG des_pitch_mask;
	struct gsp_r4p0_des_layer_params	*ld1_params = NULL;
	...
	gsp_core_reg_update(R4P0_DES_Y_ADDR(base),
	...
	gsp_core_reg_update(R4P0_DES_U_ADDR(base),
	...
	gsp_core_reg_update(R4P0_DES_V_ADDR(base),
	...
	gsp_core_reg_update(R4P0_DES_PITCH(base),
		des_pitch_value.value, des_pitch_mask.value);

	if (ld1_params->bk_para.bk_enable) {
		struct R4P0_BACK_RGB_REG back_rgb_value;
		struct R4P0_BACK_RGB_REG back_rgb_mask;
		...
		gsp_core_reg_update(R4P0_BACK_RGB(base),
	...
	if (ld1_params->img_format == GSP_R4P0_DST_FMT_RGB565) {
		des_cfg_value.R5_SWAP_MOD = ld1_params->endian.rgb_swap_mode;
	} else if ((ld1_params->img_format == GSP_R4P0_DST_FMT_ARGB888) ||
			(ld1_params->img_format == GSP_R4P0_DST_FMT_RGB888)) {
		des_cfg_value.R8_SWAP_MOD =
			ld1_params->endian.rgb_swap_mode;
	}
	des_cfg_value.BCOMP_MOD = ld1_params->fbc_mod;
	...
	des_cfg_mask.BCOMP_MOD = 0x3;
	des_cfg_mask.DITHER_EN = 0x1;
	des_cfg_mask.BK_EN = 0x1;
	des_cfg_mask.BK_BLD = 0x1;
	gsp_core_reg_update(R4P0_DES_DATA_CFG(base),
		des_cfg_value.value, des_cfg_mask.value);
}

static void gsp_r4p0_coef_gen_and_cfg(struct gsp_r4p0_core *core,
						struct gsp_r4p0_cfg *cmd)
{
	int i = 0;
	int jcnt = 0;
	...
	for (jcnt = 0; jcnt < R4P0_IMGL_NUM; jcnt++)
		if (cmd->misc.scale_para[jcnt].scale_en) {
			src_w = cmd->misc.scale_para[jcnt].scale_rect_in.rect_w;
			src_h = cmd->misc.scale_para[jcnt].scale_rect_in.rect_h;
			dst_w =
			cmd->misc.scale_para[jcnt].scale_rect_out.rect_w;
			dst_h =
			cmd->misc.scale_para[jcnt].scale_rect_out.rect_h;
			hor_tap = cmd->misc.scale_para[jcnt].htap_mod;
			ver_tap = cmd->misc.scale_para[jcnt].vtap_mod;

			ret_coef = gsp_r4p0_gen_block_scaler_coef(core,
								src_w, src_h,
								dst_w, dst_h,
								hor_tap,
								ver_tap);
			...
			if (ret_coef) {
				for (i = 0; i < 128; i++)
					gsp_core_reg_update(
						R4P0_SCALE_COEF_ADDR(
						(core->gsp_ctl_reg_base +
						jcnt * R4P0_SCALE_COEF_OFFSET
						+ i * 4)),
						*ret_coef++, 0xffffffff);
...
static void gsp_r4p0_core_run(struct gsp_core *core)
{
	struct R4P0_GSP_GLB_CFG_REG gsp_mod1_cfg_value;
	struct R4P0_GSP_GLB_CFG_REG gsp_mod1_cfg_mask;

	gsp_mod1_cfg_value.value = 0;
	gsp_mod1_cfg_value.GSP_RUN = 1;
	gsp_mod1_cfg_mask.value = 0;
	gsp_mod1_cfg_mask.GSP_RUN = 0x1;
	gsp_core_reg_update(R4P0_GSP_GLB_CFG(core->base),
		gsp_mod1_cfg_value.value, gsp_mod1_cfg_mask.value);
}

static int gsp_r4p0_core_run_precheck(struct gsp_core *c)
{
	struct gsp_r4p0_core *core = (struct gsp_r4p0_core *)c;
	struct IWHALE2_AHB_EB_REG iwhale2_ahb_reg;
	struct IWHALE2_AON_PWU_VSP_REG iwhale2_aon_pwu_vsp_reg;
	struct IWHALE2_AON_APB_EB1_REG iwhale2_aon_apb_eb1_reg;
	int ret = 0;
	int err_code = 0;
	...
	uint32_t gsp0_clk, gsp_mtx_clk;

	iwhale2_ahb_reg.value =
		gsp_core_reg_read((core->vsp_sys_regmap + 0x0));

	if (c->id == 0) {
		if (iwhale2_ahb_reg.GSP0_EB == 0)
			err_code = 0x1;

		if (iwhale2_ahb_reg.GSP_MMU_EB == 0)
			err_code |= 0x2;

		if (iwhale2_ahb_reg.GSP_MTX_EB == 0)
			err_code |= 0x4;

		if (iwhale2_ahb_reg.SYS_MTX_EB == 0)
			err_code |= 0x8;

		if (iwhale2_ahb_reg.CKG_EB == 0)
			err_code |= 0x10;
	} else {
		if (iwhale2_ahb_reg.GSP1_EB == 0)
			err_code = 0x100;
	...
	if (err_code) {
		ret = -1;
		GSP_ERR("err_code: 0x%x, 0xD1100000: 0x%x\n",
			 err_code, iwhale2_ahb_reg.value);
	} else {
		ret = 0;
	}

	gsp = gsp_core_to_parent(c);
	interface = gsp_dev_to_interface(gsp);
	gsp_interface = (struct gsp_interface_iwhale2 *)interface;

	if (IS_ERR_OR_NULL(gsp_interface->aon_pwu_regmap)) {
		GSP_ERR("Line(%d)\n", __LINE__);
	} else {
		regmap_read(gsp_interface->aon_pwu_regmap,
					 0x4C, &iwhale2_aon_pwu_vsp_reg.value);
		if (iwhale2_aon_pwu_vsp_reg.PD_VSP_SYS_FORCE_SHUTDOWN)
			GSP_ERR("0xE42B004C[0x%x]\n",
				iwhale2_aon_pwu_vsp_reg.value);
	}

	if (IS_ERR_OR_NULL(gsp_interface->aon_apb_regmap)) {
		GSP_ERR("Line(%d)\n", __LINE__);
	} else {
		regmap_read(gsp_interface->aon_apb_regmap,
					 0x4, &iwhale2_aon_apb_eb1_reg.value);
		if ((iwhale2_aon_apb_eb1_reg.GSP_EMC_EB == 0) ||
			(iwhale2_aon_apb_eb1_reg.AON_VSP_EB == 0))
			GSP_ERR("0xE42E0004[0x%x]\n",
				iwhale2_aon_apb_eb1_reg.value);
	}

	if (IS_ERR_OR_NULL(gsp_interface->vsp_clk_regmap)) {
		GSP_ERR("Line(%d)\n", __LINE__);
	} else {
		gsp0_clk = gsp_core_reg_read(
			(gsp_interface->vsp_clk_regmap + 0x30));
		gsp_mtx_clk = gsp_core_reg_read(
			(gsp_interface->vsp_clk_regmap + 0x38));
		if ((gsp0_clk != 0x3) || (gsp_mtx_clk != 0x3))
			GSP_ERR("0xD1000030[0x%x],0xD1000038[0x%x]\n",
				gsp0_clk, gsp_mtx_clk);
	}

	return ret;
}

int gsp_r4p0_core_trigger(struct gsp_core *c)
{
	int ret = -1;
	int icnt = 0;
	void __iomem *base = NULL;
	struct gsp_kcfg *kcfg = NULL;
	struct gsp_r4p0_cfg *cfg = NULL;
	struct gsp_r4p0_core *core = (struct gsp_r4p0_core *)c;
	struct R4P0_GSP_GLB_CFG_REG gsp_mod1_cfg_value;

	if (gsp_core_verify(c)) {
		GSP_ERR("gsp_r4p0 core trigger params error\n");
		return ret;
	}

	kcfg = c->current_kcfg;
	if (gsp_kcfg_verify(kcfg)) {
		GSP_ERR("gsp_r4p0 trigger invalidate kcfg\n");
		return ret;
	}

	if (gsp_r4p0_core_run_precheck(c)) {
		GSP_ERR("r4p0 core run precheck fail !\n");
		return GSP_K_CLK_CHK_ERR;
	}

	/*hardware status check*/
	gsp_mod1_cfg_value.value =
		gsp_core_reg_read(R4P0_GSP_GLB_CFG(c->base));
	if (gsp_mod1_cfg_value.GSP_BUSY) {
		GSP_ERR("core is still busy(0x%x), can't trigger\n",
			gsp_mod1_cfg_value.value);
		return GSP_K_HW_BUSY_ERR;
	}
	base = c->base;
	cfg = (struct gsp_r4p0_cfg *)kcfg->cfg;

	gsp_r4p0_coef_gen_and_cfg(core, cfg);

	gsp_r4p0_core_misc_reg_set(c, cfg);
	for (icnt = 0; icnt < R4P0_IMGL_NUM; icnt++)
		gsp_r4p0_core_limg_reg_set(base, &cfg->limg[icnt], icnt);
	for (icnt = 0; icnt < R4P0_OSDL_NUM; icnt++)
		gsp_r4p0_core_losd_reg_set(base, &cfg->losd[icnt], icnt);
	gsp_r4p0_core_ld1_reg_set(base, &cfg->ld1);

	if (gsp_r4p0_core_cfg_is_err(c)) {
		gsp_r4p0_core_err_code_print(c);
...
int gsp_r4p0_core_release(struct gsp_core *c)
{
	struct gsp_r4p0_core *core = NULL;
...
int gsp_r4p0_core_copy_cfg(struct gsp_kcfg *kcfg,
			void *arg, int index)
{
	struct gsp_r4p0_cfg_user *cfg_user_arr = NULL;
	struct gsp_r4p0_cfg_user *cfg_user;
	struct gsp_r4p0_cfg *cfg = NULL;
	int icnt = 0;
	...
	memcpy(&cfg->ld1.params, &cfg_user->ld1.params,
		   sizeof(struct gsp_r4p0_des_layer_params));
	gsp_layer_set_filled(&cfg->ld1.common);

	memcpy(&cfg->misc, &cfg_user->misc,
		   sizeof(struct gsp_r4p0_misc_cfg));

	gsp_r4p0_core_cfg_print(cfg);

	return 0;

}

int __user *gsp_r4p0_core_intercept(void __user *arg, int index)
{
	struct gsp_r4p0_cfg_user  __user *cfgs = NULL;

	cfgs = (struct gsp_r4p0_cfg_user __user *)arg;
	return &cfgs[index].ld1.common.sig_fd;
}

void gsp_r4p0_core_reset(struct gsp_core *core)
{
/*
	struct gsp_r4p0_core *c = (struct gsp_r4p0_core *)core;
	struct IWHALE2_AHB_RST_REG iwhale2_reset_reg;

	iwhale2_reset_reg.value = 0;
	if (core->id == 0) {
		iwhale2_reset_reg.value =
			gsp_core_reg_read((c->vsp_sys_regmap + 0x4));
		iwhale2_reset_reg.GSP0_SOFT_RST = 1;
		iwhale2_reset_reg.GSP_MTX_SOFT_RST = 1;
		gsp_core_reg_write((c->vsp_sys_regmap + 0x4),
			iwhale2_reset_reg.value);

		mdelay(1);

		iwhale2_reset_reg.GSP0_SOFT_RST = 0;
		iwhale2_reset_reg.GSP_MTX_SOFT_RST = 0;
		gsp_core_reg_write((c->vsp_sys_regmap + 0x4),
			iwhale2_reset_reg.value);
	} else {
		iwhale2_reset_reg.value =
			gsp_core_reg_read((c->vsp_sys_regmap + 0x4));
		iwhale2_reset_reg.GSP1_SOFT_RST = 1;
		iwhale2_reset_reg.GSP_MTX_SOFT_RST = 1;
		gsp_core_reg_write((c->vsp_sys_regmap + 0x4),
			iwhale2_reset_reg.value);
		...
		iwhale2_reset_reg.GSP1_SOFT_RST = 0;
		iwhale2_reset_reg.GSP_MTX_SOFT_RST = 0;
		gsp_core_reg_write((c->vsp_sys_regmap + 0x4),
			iwhale2_reset_reg.value);
	}
