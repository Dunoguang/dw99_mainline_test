...
#include <video/gsp_lite_r3p0_cfg.h>
#include "../gsp_core.h"
#include "../gsp_kcfg.h"
#include "../gsp_debug.h"
#include "../gsp_dev.h"
...
void gsp_lite_r3p0_core_dump(struct gsp_core *c)
{
	...
	reg_struct.glb_cfg.value = gsp_core_reg_read(LITE_R3P0_GSP_GLB_CFG(c->base));
	reg_struct.int_cfg.value = gsp_core_reg_read(LITE_R3P0_GSP_INT(c->base));
	reg_struct.mod_cfg.value = gsp_core_reg_read(LITE_R3P0_GSP_MOD_CFG(c->base));
...
static void gsp_lite_r3p0_core_cfg_reinit(struct gsp_lite_r3p0_cfg *cfg)
{
	...
	memset(&cfg->ld1.params, 0, sizeof(struct gsp_lite_r3p0_des_layer_params));
}
...
int gsp_lite_r3p0_core_init(struct gsp_core *core)
{
	...
	list_for_each_entry(kcfg, &core->kcfgs, sibling) {
		gsp_lite_r3p0_core_cfg_init((struct gsp_lite_r3p0_cfg *)kcfg->cfg, kcfg);
	}
...
static int gsp_lite_r3p0_core_parse_irq(struct gsp_core *core)
{
	...
	if (core->irq)
		ret = devm_request_irq(dev, core->irq,
				...
				IRQF_SHARED, lite_r3p0_irq_name[core->id], core);
...
static void gsp_lite_r3p0_core_limg_reg_set(void __iomem *base,
			   struct gsp_lite_r3p0_img_layer *layer, int layer_index)
{
...
static void gsp_lite_r3p0_core_losd_reg_set(void __iomem *base,
			   struct gsp_lite_r3p0_osd_layer *layer, int layer_index)
{
			...
			if (ret_coef) {
				for (i = 0; i < 128; i++)
					gsp_core_reg_update(
						...
						jcnt * LITE_R3P0_SCALE_COEF_OFFSET
						+ i * 4)),
						*ret_coef++, 0xffffffff);
