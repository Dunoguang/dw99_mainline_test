...
#include <video/gsp_lite_r2p0_cfg.h>
#include "../gsp_core.h"
#include "../gsp_kcfg.h"
#include "../gsp_debug.h"
#include "../gsp_dev.h"
...
static bool gsp_lite_r2p0_core_checkpower(struct gsp_core *core)
{
	int ret = -1;
	unsigned int val = 0;
	...
	if (IS_ERR_OR_NULL(core)) {
		GSP_ERR("gsp_lite_r2p0 core params error\n");
		return false;
	}

	if (IS_ERR_OR_NULL(core->node)) {
		GSP_ERR("gsp_lite_r2p0 core node parameters error\n");
		return false;
	}

	gpr = syscon_regmap_lookup_by_phandle(core->node, "sprd,sys-ap-ahb");
	ret = regmap_read(gpr, REG_AP_AHB_AHB_EB, &val);
	if (ret) {
		GSP_ERR("gsp_lite_r2p0 read failed\n");
		return false;
	}

	return (val & BIT_AP_AHB_GSP_EB) ? true : false;
}
...
static irqreturn_t gsp_lite_r2p0_core_irq_handler(int irq, void *data)
{
	...
	if (gsp_lite_r2p0_core_checkpower(core) == false) {
		GSP_ERR("core irq handler powerdown\n");
		return IRQ_NONE;
	}
