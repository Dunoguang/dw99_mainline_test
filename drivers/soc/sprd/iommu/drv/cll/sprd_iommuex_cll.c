...
#ifdef CONFIG_64BIT
#ifdef CONFIG_ARM64
	arch_sync_dma_for_device(virt_to_phys((void *)(ppn_base + start_entry * 4)),
		(unsigned long)(ppn_base + end_entry * 4) - (unsigned long)(ppn_base + start_entry * 4),
		DMA_TO_DEVICE);
#else
	/*x86*/
				...
				struct sprd_iommu_map_param *p_map_param)
{
	...
	for_each_sg(p_map_param->p_sg_table->sgl, sg,
		...
		for (entry_index = 0; entry_index < valid_page_entrys;
			...
			if (likely(p_iommu_priv->pagt_base_phy_ddr == 0))
				((u32 *)p_iommu_priv->ppn_base_addr)
					[total_page_entrys + entry_index] =
					(u32)phy_addr;
			else
				mmu_ex_write_pate_totable(
					p_iommu_priv->ppn_base_addr,
					total_page_entrys + entry_index,
					(u32)phy_addr);
		}
