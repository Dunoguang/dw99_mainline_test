				...
				sprd_iommu_hdl  p_iommupf_hdl)
{
	...
	if (p_init_param->pagt_base_ddr > 0) {
		...
		priv->pagt_base_ddr = (ulong)ioremap(
...
#ifdef CONFIG_ARM64
		arch_sync_dma_for_device(virt_to_phys((void *)priv->ch[ch_num].pgt_base_addr),
					(unsigned long)(priv->ch[ch_num].pgt_base_addr + total_page_entrys * 4) - (unsigned long)priv->ch[ch_num].pgt_base_addr,
					DMA_TO_DEVICE);
