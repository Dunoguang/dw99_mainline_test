...
static const struct of_device_id sprd_ptm_of_match[] = {
	{ .compatible = "sprd,ptm-sharkle", .data = &ptm_v1_data},
	{ .compatible = "sprd,ptm-sharkl3", .data = &ptm_v2_data},
	{ },
};
...
static void sprd_ptm_set_msterid(struct sprd_ptm_dev *sdev)
{
	...
	master_val = ((msterid_mask & PTM_MASTERID_MSK) << 16) |
				(msterid & PTM_MASTERID_MSK);
...
static int sprd_ptm_trace_enable(struct sprd_ptm_dev *sdev)
{
	...
	clk_prepare_enable(sdev->clk_cs);
...
static int sprd_ptm_legacy_start(struct sprd_ptm_dev *sdev, u32 cir)
{
	...
	if (!sdev->mode_info.legacy.per_buf)
		return -ENOMEM;
	hrtimer_setup(&sdev->mode_info.legacy.timer,
		       sprd_ptm_legacy_time_handler, CLOCK_REALTIME,
		       HRTIMER_MODE_REL);
...
static void sprd_ptm_init(struct device *dev)
{
	...
	struct regmap *syscon = sdev->pub_base;

	if (!IS_ERR(sdev->pub_base))
		regmap_update_bits(syscon,
			REG_PUB_AHB_WRAP_BUSMON_CLK_EN,
			BIT_PUB_AHB_WRAP_BUSMON_EB_REG |
			BIT_PUB_AHB_WRAP_BUSMON_EB_AXI,
			BIT_PUB_AHB_WRAP_BUSMON_EB_REG |
			BIT_PUB_AHB_WRAP_BUSMON_EB_AXI);
...
static int sprd_ptm_probe(struct platform_device *pdev)
{
	...
	struct ptm_pvt_para *ptm_ver_info;
	const struct of_device_id *lock_of_id;
	void __iomem *funnel_base, *tmc_base, *tpiu_base;
	int i, ret;

	if (!pdev->dev.of_node) {
		dev_err(&pdev->dev, "Can't find %s ptm node!\n", pdev->name);
		return -ENODEV;
	}

	lock_of_id = of_match_node(sprd_ptm_of_match,
				   pdev->dev.of_node);
	if (!lock_of_id) {
		dev_err(&pdev->dev, "Get the ptm of device id failed!\n");
		return -ENODEV;
	}

	ptm_ver_info = (struct ptm_pvt_para *)lock_of_id->data;
	...
	/* some boards have not pub base */
	sdev->pub_base = syscon_regmap_lookup_by_phandle(pdev->dev.of_node,
				"sprd,syscon-pub-ahb");
	if (IS_ERR(sdev->pub_base))
		dev_warn(&pdev->dev, "There has not pub gloabl syscon!\n");
	...
	dev_info(&pdev->dev, "sprd ptm probe done!\n");

	return 0;
}

static void sprd_ptm_remove(struct platform_device *pdev)
{
...
static int sprd_ptm_suspend(struct platform_device *pdev, pm_message_t state)
{
	return 0;
}

static int sprd_ptm_resume(struct platform_device *pdev)
{
	struct sprd_ptm_dev *sdev = dev_get_drvdata(&pdev->dev);
...
static struct platform_driver sprd_ptm_driver = {
	...
	.suspend  = sprd_ptm_suspend,
	.resume   = sprd_ptm_resume,
	.driver = {
		.owner = THIS_MODULE,
		.name = "sprd_ptm",
