...
#define tick_2_ms(time) (1000*(time)/CONFIG_HZ)
 ...
 *   persistence[] : ringbuffer --> size according to buffer_size;
 * */
 ...
 * ----------------------------------------------
 * */
static void div64_compute_ratio(const unsigned long long dividend,
...
static u64 get_idle_time(int cpu)
{
	...
	/* FIXME: the idle time from get_cpu_idle_time_us() is reset
	 *        while CPU is hot-pluged. Using cpustat[CPUTIME_IDLE]
	 *        to get idle. It isn't very accurate, but stable */
#if 0
	if (cpu_online(cpu))
	...
	if (idle_time == -1ULL)
		...
		idle = (u64)(idle_time) * NSEC_PER_USEC;

	/*FIXME: this idle function has bug on our shark platform:
	   not monotone increasing */
	if (DEBUG_PRINT)
		pr_debug("acedebug: get_idle_time: cpu=%d, idle=%llu\n",
			   cpu, idle);
...
static u64 get_iowait_time(int cpu)
{
	...
	/* FIXME: the iowait time from get_cpu_iowait_time_us() is reset
	 *        while CPU is hot-pluged. Using cpustat[CPUTIME_IOWAIT]
	 *        to get iowait. It isn't very accurate, but stable */
#if 0
	if (cpu_online(cpu))
	...
	if (iowait_time == -1ULL)
		...
		iowait = (u64)(iowait_time) * NSEC_PER_USEC;

	return iowait;
}
#endif

static void get_each_cpu_data(struct cpu_usage_info *new, int cpuid)
{
#ifdef CONFIG_VM_EVENT_COUNTERS
...
static void update_cpu_record(struct cpu_recorder *record,
			      struct cpu_usage_info *prev,
			      struct cpu_usage_info *next, int cpuid)
{
	record->total.user += record->percpu[cpuid].user =
	    next->user - prev->user;
	record->total.system += record->percpu[cpuid].system =
	    next->system - prev->system;
	record->total.nice += record->percpu[cpuid].nice =
	    next->nice - prev->nice;
	/*
	...
	if (next->idle < prev->idle) {
		...
		prev->sum =
		    prev->user + prev->system + prev->nice + prev->idle +
		    prev->iowait + prev->irq + prev->softirq + prev->steal;
	}

	record->total.idle += record->percpu[cpuid].idle =
	    next->idle - prev->idle;
	record->total.iowait += record->percpu[cpuid].iowait =
	    next->iowait - prev->iowait;
	record->total.irq += record->percpu[cpuid].irq = next->irq - prev->irq;
	record->total.softirq += record->percpu[cpuid].softirq =
	    next->softirq - prev->softirq;
	record->total.steal += record->percpu[cpuid].steal =
	    next->steal - prev->steal;
	 ...
	 *       SO: here we walk-around, but need modify in future!!!
	 */
	if (next->context_switch < prev->context_switch)
		prev->context_switch = next->context_switch;
	if (next->pagefault < prev->pagefault)
		prev->pagefault = next->pagefault;
	if (next->pagemajfault < prev->pagemajfault)
		prev->pagemajfault = next->pagemajfault;

	record->total.context_switch += record->percpu[cpuid].context_switch =
	    next->context_switch - prev->context_switch;
	record->total.pagefault += record->percpu[cpuid].pagefault =
	    next->pagefault - prev->pagefault;
	record->total.pagemajfault += record->percpu[cpuid].pagemajfault =
	    next->pagemajfault - prev->pagemajfault;
#endif
}
...
static void cpu_threadinfo_clean(struct thread_cpuinfo *buffer)
{
	...
	if (begin < end + 1) {
		for (i = begin; i < end + 1; i++) {
			buffer->persistence[i].utime = 0;
			buffer->persistence[i].stime = 0;
		}
	} else {
...
static void update_prev(struct task_struct *prev)
{
	...
	buffer =
	    (struct thread_cpuinfo *)(prev->stack + sizeof(struct thread_info) +
						8);
	if (!buffer)
		return;
	...
	if (buffer->index == global_index) {
		/*update utime stime */
		buffer->persistence[bufferid(buffer->index)].utime +=
		    prev->utime - buffer->start.utime;
		buffer->persistence[bufferid(buffer->index)].stime +=
		    prev->stime - buffer->start.stime;
	} else {
		/*clean buffer */
...
static void update_next(struct task_struct *next)
{
	...
	/*thread buffer. Skip SCHED_STACK_END_CHECK by end add 8 */
	buffer =
	    (struct thread_cpuinfo *)(next->stack + sizeof(struct thread_info) +
						8);
	if (!buffer)
		return;
...
{
	int i, pos;
	struct cpu_usage_info next;

	/*init */
	pos = bufferid(global_index);
	memset(&next, 0, sizeof(struct cpu_usage_info));
	memset(&recorder[pos], 0, sizeof(struct cpu_recorder));

	/*record start time */
	recorder[pos].timestamp.start = each_time;
	memcpy(&recorder[pos].timestamp.ts_start, &ts, sizeof(struct timespec64));
	rtc_time64_to_tm(recorder[pos].timestamp.ts_start.tv_sec,
		       &recorder[pos].timestamp.tm_start);

	/*record end time */
	recorder[pos].timestamp.end = each_time = sched_clock();
	ktime_get_real_ts64(&ts);
	memcpy(&recorder[pos].timestamp.ts_end, &ts, sizeof(struct timespec64));
	rtc_time64_to_tm(recorder[pos].timestamp.ts_end.tv_sec,
		       &recorder[pos].timestamp.tm_end);
	...
	for_each_possible_cpu(i) {
		...
		/*update prev */
		memcpy(&prev_value[i], &next, sizeof(struct cpu_usage_info));
	}
...
static void compute_print_cpu_rate(struct seq_file *p, void *v,
				   struct cpu_recorder *record)
{
	...
	for_each_possible_cpu(i) {
		/*compute each cpu */
		div64_compute_ratio(record->percpu[i].idle,
				    record->percpu[i].sum, idle_ratio);
		div64_compute_ratio(record->percpu[i].user,
				    record->percpu[i].sum, user_ratio);
		div64_compute_ratio(record->percpu[i].system,
				    record->percpu[i].sum, system_ratio);
		div64_compute_ratio(record->percpu[i].nice,
				    record->percpu[i].sum, nice_ratio);
		div64_compute_ratio(record->percpu[i].iowait,
				    record->percpu[i].sum, iowait_ratio);
		div64_compute_ratio(record->percpu[i].irq,
				    record->percpu[i].sum, irq_ratio);
		div64_compute_ratio(record->percpu[i].softirq,
				    record->percpu[i].sum, softirq_ratio);
		div64_compute_ratio(record->percpu[i].steal,
				    record->percpu[i].sum, steal_ratio);
		div64_compute_ratio(record->percpu[i].sum,
				    record->percpu[i].sum, sum_ratio);
		/*print each cpu */
		seq_printf(p,
			   " cpu%d(%d): %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% | %15lu %15lu %15lu\n",
			   ...
			   sum_ratio[1], record->percpu[i].context_switch,
			   record->percpu[i].pagefault,
			   record->percpu[i].pagemajfault);
	}

	if (num_possible_cpus() > 1) {
		/*compute total */
		div64_compute_ratio(record->total.idle, record->total.sum,
				    idle_ratio);
		div64_compute_ratio(record->total.user, record->total.sum,
				    user_ratio);
		div64_compute_ratio(record->total.system, record->total.sum,
				    system_ratio);
		div64_compute_ratio(record->total.nice, record->total.sum,
				    nice_ratio);
		div64_compute_ratio(record->total.iowait, record->total.sum,
				    iowait_ratio);
		div64_compute_ratio(record->total.irq, record->total.sum,
				    irq_ratio);
		div64_compute_ratio(record->total.softirq, record->total.sum,
				    softirq_ratio);
		div64_compute_ratio(record->total.steal, record->total.sum,
				    steal_ratio);
		div64_compute_ratio(record->total.sum, record->total.sum,
				    sum_ratio);
		/*print total */
		seq_puts(p, " ------------------\n");
		seq_printf(p,
			   " Total:   %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% %4lu.%02lu%% | %15lu %15lu %15lu\n",
			   ...
			   record->total.context_switch,
			   record->total.pagefault, record->total.pagemajfault);
	}
}

static void compute_print_each_thread_rate(struct seq_file *p, void *v, int pid,
					   char *name, unsigned long u_time,
					   ...
					   unsigned long index)
{
	...
	/*devider */
	timer_circle_long =
	    recorder[index].timestamp.end - recorder[index].timestamp.start;
	...
	if (NULL != name) {
		seq_printf(p,
			   ...
			   pid, utime_ratio[0], utime_ratio[1], stime_ratio[0],
			   stime_ratio[1], ttime_ratio[0], ttime_ratio[1],
			   name);
	} else {
		seq_printf(p,
			   " %-6s  %4lu.%02lu%%  %4lu.%02lu%%  %4lu.%02lu%%\n",
			   "Total:", utime_ratio[0], utime_ratio[1],
			   stime_ratio[0], stime_ratio[1], ttime_ratio[0],
			   ttime_ratio[1]);
	}
}
...
static void print_summary(struct seq_file *p, void *v, unsigned long bufid)
{
	...
	if (bufid == bufferid(global_index)) {
		...
		recorder[bufid].timestamp.end = sched_clock();
		memcpy(&recorder[bufid].timestamp.ts_start, &ts,
		       sizeof(struct timespec64));
		ktime_get_real_ts64(&recorder[bufid].timestamp.ts_end);
		rtc_time64_to_tm(recorder[bufid].timestamp.ts_start.tv_sec,
			       &recorder[bufid].timestamp.tm_start);
		rtc_time64_to_tm(recorder[bufid].timestamp.ts_end.tv_sec,
			       &recorder[bufid].timestamp.tm_end);
	}

	/*compute how long time */
	timelongth =
	    recorder[bufid].timestamp.end - recorder[bufid].timestamp.start;
	...
	seq_printf(p,
		   ...
		   start_time,
		   recorder[bufid].timestamp.tm_start.tm_year + 1900,
		   recorder[bufid].timestamp.tm_start.tm_mon + 1,
		   recorder[bufid].timestamp.tm_start.tm_mday,
		   recorder[bufid].timestamp.tm_start.tm_hour,
		   recorder[bufid].timestamp.tm_start.tm_min,
		   recorder[bufid].timestamp.tm_start.tm_sec,
		   recorder[bufid].timestamp.ts_start.tv_nsec, end_time,
		   recorder[bufid].timestamp.tm_end.tm_year + 1900,
		   recorder[bufid].timestamp.tm_end.tm_mon + 1,
		   recorder[bufid].timestamp.tm_end.tm_mday,
		   recorder[bufid].timestamp.tm_end.tm_hour,
		   recorder[bufid].timestamp.tm_end.tm_min,
		   recorder[bufid].timestamp.tm_end.tm_sec,
		   recorder[bufid].timestamp.ts_end.tv_nsec);
}

static void print_cpu_usage(struct seq_file *p, void *v, unsigned long bufid)
{
	int i;
	struct cpu_usage_info now;

	/*init */
	memset(&now, 0, sizeof(struct cpu_usage_info));

	/*update current record temporarily */
	if (bufid == bufferid(global_index)) {
		memset(&recorder[bufid].percpu, 0,
		       num_possible_cpus() * sizeof(struct cpu_usage_info));
		memset(&recorder[bufid].total, 0,
		       sizeof(struct cpu_usage_info));
		...
		for_each_possible_cpu(i) {
			...
			/*update recorder */
			update_cpu_record(&recorder[bufid], &prev_value[i],
					  &now, i);
		}
	}
	...
	seq_printf(p,
		   ...
		   bufid, "IDLE", "USER", "SYSTEM", "NICE", "IOWAIT", "IRQ",
		   "SOFTIRQ", "STEAL", "TOTAL", "CTXT_SWITCH", "FG_FAULT",
		   "FG_MAJ_FAULT");
...
static void print_each_thread_usage(struct seq_file *p, void *v,
				    unsigned long bufid)
{
	...
	for_each_process(gp)
		for_each_thread(gp, pp) {
		/*get ringbuffer. Skip SCHED_STACK_END_CHECK by end add 8 */
		buffer =
		    (struct thread_cpuinfo *)(pp->stack +
					      sizeof(struct thread_info) + 8);
		if (!buffer)
			continue;
		...
		if (0 !=
		    (buffer->persistence[bufid].utime +
		     buffer->persistence[bufid].stime)) {

			unsigned long u_time =
			    tick_2_ms(buffer->persistence[bufid].utime);
			unsigned long s_time =
			    tick_2_ms(buffer->persistence[bufid].stime);

			/*compute total */
			total_usr += buffer->persistence[bufid].utime;
			total_sys += buffer->persistence[bufid].stime;

			compute_print_each_thread_rate(p, v, pp->pid, pp->comm,
						       u_time, s_time, bufid);
		}
		}
	...
	/*print total */
	seq_puts(p, " ------------------\n");
	compute_print_each_thread_rate(p, v, 0, NULL, (tick_2_ms(total_usr)),
				       (tick_2_ms(total_sys)), bufid);
}
...
static int __init sprd_cpu_usage_init(void)
{
	...
	/*init timer */
	kt = ktime_set(hrtimer_se, hrtimer_ns);
	hrtimer_setup(&timer, hrtimer_handler, CLOCK_MONOTONIC,
		       HRTIMER_MODE_REL);
