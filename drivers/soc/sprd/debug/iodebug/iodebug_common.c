...
asmlinkage int iodebug_trace(const char *fmt, ...)
{
	...
	if (iodebug_trace_enable) {
		va_start(args, fmt);
		r = vprintk_emit(0, -1, NULL, fmt, args);
		va_end(args);
	}
	return r;
}
...
static void show_timer_function(struct timer_list *t)
{
...
static int iodebug_periodic_func_timer_init(void)
{
	if (g_iodebug_vfs_timer_flag == false) {
		timer_setup(&g_show_timer, show_timer_function, 0);
...
static int iodebug_nl_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh, struct netlink_ext_ack *extack)
{
	unsigned short type;
