...
static enum hrtimer_restart scan_burst_irq(struct hrtimer *hr)
{
	...
	if ((save_nr_irqs != irq_get_nr_irqs() || stat == 1) && Processing == false)
					...
					loff_t *ppos)
{
	...
	if (enable == 1) {
		...
		hrtimer_setup(&irq_monitor_timer, scan_burst_irq,
...
static int monitor_irqs_change(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		Processing = true;
		do {
			tmp_nr_irqs = max(save_nr_irqs, irq_get_nr_irqs());
