...
note_buf_t __percpu *sprd_crash_notes;
...
static struct crypto_shash *sysdump_hash_tfm;
...
void crash_note_save_cpu(struct pt_regs *regs, int cpu)
{
	...
	storenote(&notes, (char *)per_cpu_ptr(sprd_crash_notes, cpu));
}
				 ...
				 struct pt_regs *regs)
{
	...
	strscpy(sprd_sysdump_extra.reason, reason,
		sizeof(sprd_sysdump_extra.reason));
...
void sysdump_enter(int enter_id, const char *reason, struct pt_regs *regs)
{
	...
	if (sprd_sysdump_init == 0) {
		...
		sprd_crash_notes = alloc_percpu(note_buf_t);
		if (sprd_crash_notes == NULL) {
	...
	{
		SHASH_DESC_ON_STACK(shash_desc, sysdump_hash_tfm);

		shash_desc->tfm = sysdump_hash_tfm;
		crypto_shash_init(shash_desc);
		crypto_shash_update(shash_desc, sg_virt(&sg),
				    _etext - _stext);
		crypto_shash_final(shash_desc, g_ktxt_hash_data);
	}
...
int sysdump_sysctl_init(void)
{
	...
	sysdump_sysctl_hdr = register_sysctl("kernel", sysdump_sysctl_table);
	...
	sysdump_hash_tfm = crypto_alloc_shash("sha1", 0, 0);
	if (IS_ERR(sysdump_hash_tfm))
		return PTR_ERR(sysdump_hash_tfm);
...
void sysdump_sysctl_exit(void)
{
	...
	crypto_free_shash(sysdump_hash_tfm);
}
