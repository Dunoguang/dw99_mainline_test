...
static struct wakeup_source *messages_wakelock;
#endif
...
static int print_thread(void *data)
{
	...
	while (1) {
		__pm_stay_awake(messages_wakelock);
...
void pm_debug_init(void)
{
	...
	messages_wakelock = wakeup_source_register(NULL,
						   "pm_message_wakelock");
