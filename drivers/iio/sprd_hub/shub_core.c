...
static int get_file_size(struct file *f)
{
	...
	error = vfs_getattr(&f->f_path, &stat, STATX_BASIC_STATS, 0);
	if (error) {
...
static void shub_synctimestamp(struct shub_data *sensor)
{
	...
	if (sensor->mcu_mode != SHUB_NORMAL)
		return;
	ktime_get_boottime_ts64(&kt);
	k_timestamp = timespec64_to_ns(&kt);
...
static int shub_probe_trigger(struct iio_dev *iio_dev)
{
	...
	mcu_data->trig =
	    iio_trigger_alloc(&iio_dev->dev, "%s-dev%d", iio_dev->name, iio_device_id(iio_dev));
	if (!mcu_data->trig) {
		ret = -ENOMEM;
...
static int create_sysfs_interfaces(struct shub_data *mcu_data)
{
	...
	mcu_data->sensor_class = class_create("sprd_sensorhub");
...
static int shub_probe(struct platform_device *pdev)
{
	...
	indio_dev = iio_device_alloc(NULL, sizeof(*mcu));
	if (!indio_dev) {
...
static void shub_remove(struct platform_device *pdev)
{
