...
/* 4.4 vendor struct global_attr for governor sysfs attributes */

/* CPUFREQ governor event constants (ported from 4.4) */
#define CPUFREQ_GOV_START		0x00000001
#define CPUFREQ_GOV_STOP		0x00000002
#define CPUFREQ_GOV_LIMITS		0x00000004
#define CPUFREQ_GOV_POLICY_INIT		0x00000005
#define CPUFREQ_GOV_POLICY_EXIT		0x00000006
		...
		unsigned int loadadjfreq)
{
	...
	do {
		...
		index = cpufreq_table_find_index_al(pcpu->policy, loadadjfreq / tl, false);
		if (index < 0)
			break;
		...
		if (freq > prevfreq) {
			...
			if (freq >= freqmax) {
				 ...
				 * than freqmax.
				 */
				index = cpufreq_table_find_index_dl(pcpu->policy, freqmax - 1, false);
					if (index < 0)
						break;
					freq = pcpu->freq_table[index].frequency;
			...
			if (freq <= freqmin) {
				 ...
				 * than freqmin.
				 */
				index = cpufreq_table_find_index_al(pcpu->policy, freqmin + 1, false);
					if (index < 0)
						break;
					freq = pcpu->freq_table[index].frequency;
...
static void cpufreq_interactive_timer(struct timer_list *t)
{
	...
	int cpu_load;
	struct cpufreq_interactive_cpuinfo *pcpu =
		container_of(t, struct cpufreq_interactive_cpuinfo, cpu_timer);
	unsigned int data = pcpu->cpu;
	struct cpufreq_interactive_tunables *tunables =
		pcpu->policy->governor_data;
	...
	index = cpufreq_table_find_index_al(pcpu->policy, new_freq, false);
	if (index < 0) {
		spin_unlock_irqrestore(&pcpu->target_freq_lock, flags);
		goto rearm;
	}
	...
	struct notifier_block *nb, unsigned long val, void *data)
{
	...
	if (val == CPUFREQ_POSTCHANGE) {
		pcpu = &per_cpu(cpuinfo, freq->policy->cpu);
		if (!down_read_trylock(&pcpu->enable_sem))
			return 0;
		if (!pcpu->governor_enabled) {
			up_read(&pcpu->enable_sem);
	...
	const char *buf, size_t count)
{
	...
	if (IS_ERR(new_target_loads))
		return IS_ERR(new_target_loads) ? PTR_ERR(new_target_loads) : 0;
	...
	const char *buf, size_t count)
{
	...
	if (IS_ERR(new_above_hispeed_delay))
		return IS_ERR(new_above_hispeed_delay) ? PTR_ERR(new_above_hispeed_delay) : 0;
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_POLICY_EXIT:
		if (!--tunables->usage_count) {
			if (cpufreq_interactive_initialized == 1) {
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			timer_delete_sync(&pcpu->cpu_timer);
			timer_delete_sync(&pcpu->cpu_slack_timer);
...
static
#endif
static int cpufreq_interactive_gov_init(struct cpufreq_policy *policy)
{
	return cpufreq_governor_interactive(policy, CPUFREQ_GOV_POLICY_INIT);
}

static void cpufreq_interactive_gov_exit(struct cpufreq_policy *policy)
{
	cpufreq_governor_interactive(policy, CPUFREQ_GOV_POLICY_EXIT);
}

static int cpufreq_interactive_gov_start(struct cpufreq_policy *policy)
{
	return cpufreq_governor_interactive(policy, CPUFREQ_GOV_START);
}

static void cpufreq_interactive_gov_stop(struct cpufreq_policy *policy)
{
	cpufreq_governor_interactive(policy, CPUFREQ_GOV_STOP);
}

static void cpufreq_interactive_gov_limits(struct cpufreq_policy *policy)
{
...
struct cpufreq_governor cpufreq_gov_interactive = {
	.name = "interactive",
	.init = cpufreq_interactive_gov_init,
	.exit = cpufreq_interactive_gov_exit,
	.start = cpufreq_interactive_gov_start,
	.stop = cpufreq_interactive_gov_stop,
	.limits = cpufreq_interactive_gov_limits,
	.owner = THIS_MODULE,
};
...
static int __init cpufreq_interactive_init(void)
{
	...
	for_each_possible_cpu(i) {
		...
		timer_setup(&pcpu->cpu_timer, cpufreq_interactive_timer,
			    TIMER_DEFERRABLE);
		timer_setup(&pcpu->cpu_slack_timer, cpufreq_interactive_nop_timer, 0);
		pcpu->cpu = i;
