...
/* 7.2 removed CPUFREQ_ETERNAL; keep the classic -1 semantics */
...
#define SPRD_CPUFREQ_MAX_FREQ_VOLT 18
...
struct sprd_cpufreq_driver_data {
	...
	unsigned int online;
	struct sprd_cpufreq_bin bin;
	struct sprd_cpufreq_bin bin_low_volt;
...
static int sprd_cpufreq_sync_current_voltage(
	struct sprd_cpufreq_driver_data *cpufreq_data);

DEFINE_SEMAPHORE(cpu_gpu_sync_volt_sem, 1);
	...
	u32 *p_binning_data)
{
	...
	if (c->bin_low_volt.inited == true) {
		*p_binning_data = c->bin_low_volt.bin;
		return c->bin_low_volt.ret;
	}
	...
	if (efuse_blk_binning_data == 0) {
		*p_binning_data = 0;
		c->bin_low_volt.inited = true;
		c->bin_low_volt.bin = *p_binning_data;
		c->bin_low_volt.ret = -1;
		return -1;
	}
	...
	u32 *p_binning_data)
{
	...
	if (c->bin.inited == true) {
		*p_binning_data = c->bin.bin;
		return c->bin.ret;
	}
	...
	struct sprd_cpufreq_driver_data *cpufreq_data)
{
...
#if defined(CONFIG_OTP_SPRD_AP_EFUSE) || defined(CONFIG_OTP_SPRD_AP_IEFUSE)
	if (!efuse_binning_get(dev, np_cpufreq_data,
		cpufreq_data, &p_binning_data))
#elif defined(CONFIG_OTP_SPRD_AP_PUBLIC_EFUSE)
	if (efuse_binning_get(dev, np_cpufreq_data,
		cpufreq_data, &p_binning_data) >= 0)
#endif
	{
	...
	#if defined(CONFIG_OTP_SPRD_AP_PUBLIC_EFUSE)
		efuse_binning_low_volt_get(dev, np_cpufreq_data,
			cpufreq_data, &p_binning_low_volt);
	...
	of_property_read_u32(np_cpufreq_data, "sprd,max-cpufreq",
			     &max_freq_khz);
	if (max_freq_khz)
		P_INF("cluster%d max frequency limited to %uKHz\n",
		      cluster, max_freq_khz);
	...
	while (nr) {
		...
		if (max_freq_khz && freq / 1000 > max_freq_khz) {
			nr -= 2;
			continue;
		}
...
unsigned int sprd_cpufreq_update_opp(int cpu, int temp_now)
{
	...
	c->temp_max > 0) {
		...
		time_after(jiffies, c->temp_fall_time))) {
			...
			temp_now)) {
				c->temp_fall_time = 0;
				ret = dev_pm_opp_of_add_table_binning(
					c->cluster, c->cpu_dev, NULL, c);
				if (!ret) {
					max_freq = c->temp_max_freq;
					ret = sprd_cpufreq_sync_current_voltage(c);
					if (ret) {
						P_ERR("failed to sync voltage after OPP update: %d\n",
							ret);
						max_freq = 0;
					}
				}
...
{
	struct dev_pm_opp *opp;
	unsigned long freq_hz, volt_new, volt_new_req, volt_old;
	unsigned int volt_tol;
	int old_uv, ret;

	if (!cpufreq_data || !cpufreq_data->cpu_dev ||
	    IS_ERR_OR_NULL(cpufreq_data->clk) ||
	    IS_ERR_OR_NULL(cpufreq_data->reg))
		return -ENODEV;

	freq_hz = clk_get_rate(cpufreq_data->clk);
	rcu_read_lock();
	opp = dev_pm_opp_find_freq_ceil(cpufreq_data->cpu_dev, &freq_hz);
	if (IS_ERR(opp)) {
		rcu_read_unlock();
		return PTR_ERR(opp);
	}
	...
	old_uv = regulator_get_voltage(cpufreq_data->reg);
	if (old_uv < 0)
		return old_uv;
	volt_old = old_uv;
	...
	if (cpu_gpu_flag == 1) {
		...
		volt_new = max_t(unsigned long, volt_new,
				get_gpu_target_voltage());
		ret = regulator_set_voltage_tol(cpufreq_data->reg,
				volt_new, volt_tol);
		if (ret)
	...
	P_INF("cluster%u synced %luKHz voltage %lu->%luuV after OPP update\n",
		cpufreq_data->cluster, freq_hz / 1000, volt_old, volt_new);

update_request:
	cpufreq_data->volt_tol_var = volt_tol;
	cpufreq_data->volt_req = volt_new_req;
	cpufreq_data->freq_req = freq_hz;
	return 0;
}
/**
	...
	unsigned int idx, bool force)
{
	...
	if (!cpufreq_data->freqvolts || idx >= cpufreq_data->freqvolts) {
		P_ERR("invalid cpufreq_data/idx%u, returning\n", idx);
		ret = -ENODEV;
		goto EXIT_UNLOCK;
	}
	...
	/* Raise voltage before changing the clock. */
	if (volt_new > volt_old) {
	...
	if (ret) {
		...
		if ((volt_old > 0) && (volt_new > volt_old)) {
	...
	/* Lower voltage after changing the clock. */
	if (volt_new < volt_old) {
...
static int sprd_cpufreq_table_verify(struct cpufreq_policy_data *policy)
{
...
{
	if (sprd_cpufreq_cpu_callback(NULL, CPU_UP_PREPARE,
			(void *)(unsigned long)cpu) != NOTIFY_OK)
		return -EINVAL;
	return 0;
}

static int sprd_cpufreq_cpuhp_dead(unsigned int cpu)
{
	if (sprd_cpufreq_cpu_callback(NULL, CPU_POST_DEAD,
			(void *)(unsigned long)cpu) != NOTIFY_OK)
		return -EINVAL;
	return 0;
}
...
put_np:
	...
	*/
	...
	cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN, "sprd/cpufreq:online",
					sprd_cpufreq_cpuhp_online, sprd_cpufreq_cpuhp_dead);

	return ret;
}

static void sprd_cpufreq_remove(struct platform_device *pdev)
{
}
