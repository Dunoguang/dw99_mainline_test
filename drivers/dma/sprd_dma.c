...
#include <linux/mfd/syscon/sprd/isharkl2/isharkl2_ap_ahb_glb.h>
...
static int sprd_dma_probe(struct platform_device *pdev)
{
	...
	dma_base = devm_ioremap(&pdev->dev, res->start,
					resource_size(res));
	if (!dma_base) {
	...
	dma_cap_set(DMA_MEMCPY | DMA_SLAVE, sdev->dma_dev.cap_mask);
