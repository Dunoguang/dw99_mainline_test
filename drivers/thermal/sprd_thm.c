...
static int sprd_sys_temp_read(struct thermal_zone_device *tz, int *temp)
{
	struct sprd_thermal_zone *pzone = tz->devdata;
	int ret = 0;
...
static const struct thermal_zone_device_ops sprd_of_thermal_ops = {
...
int sprd_thermal_init(struct sprd_thermal_zone *pzone)
{
	...
	pzone->therm_dev =
	    devm_thermal_of_zone_register(pzone->dev, pzone->id, pzone,
					    &sprd_of_thermal_ops);
