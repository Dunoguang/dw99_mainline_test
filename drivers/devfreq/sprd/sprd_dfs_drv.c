...
static int dfs_smsg_thread(void *dmc_data)
{
	...
	if (freq_table[0] >= freq_table[j])
		...
		data->devfreq->scaling_min_freq = freq_table[0];
		min_freq = freq_table[0];
		data->devfreq->scaling_max_freq = freq_table[j];
...
static int dfs_freq_probe(struct platform_device *pdev)
{
	...
	data->devfreq->scaling_min_freq = dfs_min_freq(data);
	data->devfreq->scaling_max_freq = dfs_max_freq(data);
	scenario_data = data;
...
static void dfs_freq_remove(struct platform_device *pdev)
{
