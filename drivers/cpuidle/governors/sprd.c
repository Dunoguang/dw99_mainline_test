			...
			bool *stop_tick)
{
	...
	data->last_state_idx = data->base_idx = 0;
	...
	for (i = 1; i < drv->state_count; i++) {
		...
		data->base_idx = i;
		if ((data->base_idx > 1) && (data->next_timer_us > 100 + 2 * s->target_residency))
			need_recovery = true;
		else
			need_recovery = false;
	}
				...
				struct cpuidle_device *dev)
{
	...
	hrtimer_setup(&data->recovery_htimer, recovery_htimer_fn,
