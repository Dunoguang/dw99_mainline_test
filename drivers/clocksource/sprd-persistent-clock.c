...
static notrace u64 sprd_perclock_read_count(void)
{
	unsigned int val_lo;
...
void read_persistent_clock64(struct timespec64 *ts)
{
	...
	if (sprd_perclock_base) {
		...
		delta = delta * NSEC_PER_SEC;
		delta = div64_u64(delta, clock_freq);
		timespec64_add_ns(&persistent_ts, delta);
		*ts = persistent_ts;
	} else {
		ts->tv_sec = 0;
		ts->tv_nsec = 0;
...
static int __init sprd_persistent_clock_init(struct device_node *np)
{
...
TIMER_OF_DECLARE(sprd_persistent_clock, "sprd,persistent-clock",
