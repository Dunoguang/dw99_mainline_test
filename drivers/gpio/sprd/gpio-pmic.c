...
static struct sprd_gpio_chip pmic_gpio = {
	...
	.chip.set_config = sprd_gpio_set_config,
...
static struct sprd_gpio_chip pmic_eic = {
	...
	.chip.set_config = sprd_eic_set_config,
	.chip.to_irq = sprd_gpio_to_irq,
...
static int pmic_gpio_probe(struct platform_device *pdev)
{
	...
	err = devm_request_threaded_irq(sgc->chip.parent, irq, NULL,
				pmic_irq_handler,
				IRQF_TRIGGER_LOW | IRQF_ONESHOT | IRQF_NO_SUSPEND,
				dev_name(sgc->chip.parent), sgc);
	if (err != 0) {
		dev_err(sgc->chip.parent, "unable to request IRQ#%d: %d\n",
			irq, err);
		goto fail;
	}
...
{
	sprd_gpio_common_remove(pdev, pmic_gpio_match_table);
}
