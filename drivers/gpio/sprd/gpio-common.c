...
int sprd_gpio_set(struct gpio_chip *chip, unsigned offset, int value)
{
	pr_debug("%d+%d %d\n", chip->base, offset, value);
	sprd_gpio_write(chip, offset, REG_GPIO_DATA, value);
	return 0;
...
int sprd_gpio_plus_set(struct gpio_chip *chip, u32 offset, int value)
{
	struct device *dev = chip->parent;
...
int sprd_gpio_set_config(struct gpio_chip *chip, unsigned offset,
			      unsigned long config)
{
	...
	return sprd_gpio_set_debounce(chip, offset, arg);
}

int sprd_eic_set_config(struct gpio_chip *chip, unsigned offset,
			unsigned long config)
{
	unsigned int param = pinconf_to_config_param(config);
	unsigned int arg = pinconf_to_config_argument(config);
...
int sprd_eic_set(struct gpio_chip *chip, unsigned offset, int value)
{
	pr_debug("%d+%d %d\n", chip->base, offset, value);
	return 0;
}
	...
	struct sprd_gpio_chip *sgc)
{
	...
	sgc->chip.parent = &pdev->dev;

	gpiochip_add_data(&sgc->chip, NULL);

	return 0;
}

void sprd_gpio_common_remove(struct platform_device *pdev,
