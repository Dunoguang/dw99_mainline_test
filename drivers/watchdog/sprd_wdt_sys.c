...
static int sprd_wdt_suspend(void *data)
{
	if (!sprd_wdt.wdt_enabled)
		return 0;
...
static void sprd_wdt_resume(void *data)
{
	if (!sprd_wdt.wdt_enabled)
		return;
...
static struct syscore sprd_wdt_syscore = {
	.ops = &sprd_wdt_syscore_ops,
};
...
static int sprd_wdt_probe(struct platform_device *pdev)
{
	...
	sprd_wdt.reg = devm_ioremap(dev, wdt_res->start,
	...
	register_syscore(&sprd_wdt_syscore);

	dev_info(&pdev->dev, "sprd wdt probe OK!\n");
	return 0;
}

static void sprd_wdt_remove(struct platform_device *pdev)
{
	unregister_syscore(&sprd_wdt_syscore);
