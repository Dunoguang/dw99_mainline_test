...
struct sd_dbs_tuners {
	...
	struct sprd_pm_qos_request max_cpu_request[CLUSTER_ALL][CORE_MAX_ALL];
	struct sprd_pm_qos_request min_cpu_request[CLUSTER_ALL][CORE_MIN_ALL];
...
static void dbs_check_cpu_sprd(struct timer_list *t)
{
	...
	/*
	* calculate max_load according to current frequency. (but not very precise)
	* now dynamic hotplug is only used for sprd 4 core ARM platform.
	* if using for other platforms, the implemention should be modified
...
static struct sprd_pm_qos_request boot_min_cpu_hotplug_request[CLUSTER_ALL];
...
static int sd_tuners_init(struct sd_dbs_tuners *tuners)
{
	 ...
	 */
	timer_setup(&tuners->dynamic_load_timer, dbs_check_cpu_sprd,
		    TIMER_DEFERRABLE);
...
static const struct proc_ops sprd_hotplug_proc_ops = {
	.proc_open = sprd_hotplug_proc_open,
	.proc_read = seq_read,
	.proc_write = sprd_hotplug_proc_write,
...
static int sprd_add_proc_files(struct sd_dbs_tuners *tuners)
{
	if (!tuners->cpu_hotplug_disable) {
		...
		if (!tuners->proc_dir)
			return -ENOMEM;
		tuners->proc_entry = proc_create("enabled", 0644, tuners->proc_dir, &sprd_hotplug_proc_ops);
...
static int __init sprd_hotplug_init(void)
{
	...
	ret = kobject_init_and_add(&hotplug_kobj, &hotplug_dir_ktype,
		&bus_get_dev_root(&cpu_subsys)->kobj, "cpuhotplug");
	if (ret) {
...
static void __exit sprd_hotplug_exit(void)
{
	...
	timer_delete_sync(&tuners->dynamic_load_timer);
