...
static int sprd_correct_rtc_time(struct device *dev, const char *from)
{
	...
	secs = mktime64(CONFIG_RTC_START_YEAR, 1, 1, 0, 0, 0);
	rtc_time64_to_tm(secs, &tm);
...
static void sprd_rtc_remove(struct platform_device *pdev)
{
...
static int __init sprd_rtc_init(void)
{
	...
	if (CONFIG_RTC_START_YEAR > 1970)
		secs_start_year_to_1970 = mktime64(CONFIG_RTC_START_YEAR,
			1, 1, 0, 0, 0);
	else
		secs_start_year_to_1970 = mktime64(1970, 1, 1, 0, 0, 0);
