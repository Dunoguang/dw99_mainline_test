...
static void musb_reset_fifo_2_default(struct musb *musb, u8 epnum, u8 is_tx)
{
	...
	if (is_tx) {
		musb_writeb(musb_base, MUSB_TXFIFOSZ,
			musb->context.index_regs[epnum].txfifosz);
		musb_writew(musb_base, MUSB_TXFIFOADD,
			musb->context.index_regs[epnum].txfifoadd);
		hw_ep->tx_double_buffered =
			!!(musb->context.index_regs[epnum].txfifosz
				& MUSB_FIFOSZ_DPB);
	} else {
		musb_writeb(musb_base, MUSB_RXFIFOSZ,
			musb->context.index_regs[epnum].rxfifosz);
		musb_writew(musb_base, MUSB_RXFIFOADD,
			musb->context.index_regs[epnum].rxfifoadd);
...
static void musb_reset_all_fifo_2_default(struct musb *musb)
{
	int i;
...
static int musb_sprd_config_usbid_int(struct sprd_glue *glue)
{
	...
	ret = gpio_request(glue->gpio_host, "musb usb_id detect");
	if (ret < 0)
		return ret;
...
static int musb_sprd_config_vbus_int(struct sprd_glue *glue)
{
	...
	ret = gpio_request(glue->gpio_device, "musb vbus detect");
	if (ret < 0)
...
static int musb_sprd_otg_init(struct sprd_glue *glue)
{
	...
	ret = gpio_request(glue->gpio_otg, "musb otg ext power detect");
	if (ret < 0) {
				   ...
				   const char *buf, size_t size)
{
	...
	((struct musb_hdrc_config *)musb->config)->maximum_speed = max_speed;
	return size;
}
