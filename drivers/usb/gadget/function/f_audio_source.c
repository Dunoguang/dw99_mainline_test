...
static int audio_source_index;

static struct device *create_function_device(char *name)
{
	if (!audio_source_class) {
		audio_source_class = class_create("f_audio_source");
		if (IS_ERR(audio_source_class))
			return ERR_CAST(audio_source_class);
	}
	return device_create(audio_source_class, NULL,
			     MKDEV(0, audio_source_index++), NULL, name);
}
