...
{							\
	.attr = { .name = #_name },			\
	.show_const = dual_role_show_property,		\
	.store_const = dual_role_store_property,	\
}
...
int dual_role_uevent(const struct device *dev, struct kobj_uevent_env *env)
{
	struct dual_role_phy_instance *dual_role = dev_get_drvdata(dev);
	int ret = 0, j;
	char *prop_buf;
	...
	for (j = 0; j < dual_role->desc->num_properties; j++) {
		...
		ret = dual_role_show_property((struct device *)dev, attr, prop_buf);
...
{
	dual_role_class = class_create("dual_role_usb");
