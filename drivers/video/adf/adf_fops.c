		...
		struct adf_overlay_engine_data __user *arg)
{
	...
	strscpy(data.name, eng->base.name, sizeof(data.name));
		...
		struct adf_device_data __user *arg)
{
	...
	strscpy(data.name, dev->base.name, sizeof(data.name));
...
static int adf_intf_get_mode(struct adf_interface *intf, u32 __user *arg)
{
	u32 index;
	int ret;

	ret = adf_interface_get_mode(intf, &index);
	if (ret < 0)
		return ret;
		...
		struct adf_interface_data __user *arg)
{
	...
	strscpy(data.name, intf->base.name, sizeof(data.name));
		...
		struct adf_file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ADF_GET_MODE:
		return adf_intf_get_mode(intf, (u32 __user *)arg);
