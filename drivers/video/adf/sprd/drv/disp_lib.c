			...
			char *val, size_t count)
{
	...
	if (IS_ERR(f)) {
		pr_err("Open file %s result: %ld\n", filename, PTR_ERR(f));
		return PTR_ERR(f);
	}

	ret = vfs_write(f, val, count, &f->f_pos);
	if (ret <= 0)
		pr_err("write %s fail: %zd\n", filename, ret);
	filp_close(f, NULL);
			...
			char *val, size_t count)
{
	...
	f = filp_open(filename, flags, mode);
	if (IS_ERR(f)) {
		pr_err("Open %s result: %ld\n", filename, PTR_ERR(f));
		return PTR_ERR(f);
	}

	ret = vfs_read(f, val, count, &f->f_pos);
	if (ret <= 0)
		pr_err("read %s ret: %zd\n", filename, ret);
...
MODULE_DESCRIPTION("display lib");
