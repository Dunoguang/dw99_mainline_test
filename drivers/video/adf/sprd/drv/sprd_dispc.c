				...
				struct sprd_dispc *dispc, int id)
{
	...
	encoder->ctx.base = (unsigned long)ioremap(r.start,
					resource_size(&r));
...
static int32_t sprd_dispc_resume(struct sprd_dispc *dispc)
{
	struct dispc_context *ctx = &dispc->ctx;
	struct panel_info *panel = ctx->panel;
	u32 saved_dpi_clk_src = ctx->dpi_clk_src;
	/* 保存原始的接口类型 */
	int original_if_type = ctx->if_type;
	...
	pr_info("sprd_dispc_resume: Powering up...\n");
	...
	if (dispc->clk) {
		...
		if (ctx->dpi_clk_src == 0 && saved_dpi_clk_src != 0) {
			pr_info("sprd_dispc_resume: restore dpi_clk_src %d -> %d\n",
				ctx->dpi_clk_src, saved_dpi_clk_src);
			ctx->dpi_clk_src = saved_dpi_clk_src;
		}
	...
	/* 
	 * 3. 复位硬件 
	 * 注意：复位操作可能会清除之前的 Enable 状态和寄存器配置
	 */
	if (dispc->glb) {
		dispc->glb->reset(ctx);
		usleep_range(3000, 4000);
		...
		/* !!! 新增修复 !!! */
		/* 复位后再次使能，防止 Reset 操作把 Enable 位给清掉了 */
		pr_info("sprd_dispc_resume: Re-enable GLB after reset\n");
		dispc->glb->enable(ctx);
	}

	/* 4. 配置像素时钟 (必须在 Reset 之后) */
	if (dispc->clk) {
		if (ctx->if_type == SPRD_DISPC_IF_EDPI) {
			pr_info("sprd_dispc_resume: Force clk update for EDPI\n");
			ctx->if_type = SPRD_DISPC_IF_DPI;
			
			if (dispc_clk_update(dispc, panel->fps, SPRD_FORCE_FPS))
				pr_err("sprd_dispc_resume: dispc_clk_update failed!\n");
				
			ctx->if_type = original_if_type;
		} else {
			if (dispc_clk_update(dispc, panel->fps, SPRD_FORCE_FPS))
				pr_err("sprd_dispc_resume: dispc_clk_update failed!\n");
		}
	...
	pr_info("dispc resume OK (Final Fix V2)\n");
	return 0;
}
