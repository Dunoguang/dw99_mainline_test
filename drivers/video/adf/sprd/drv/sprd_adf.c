...
static bool skip_boot_logo = true;
module_param(skip_boot_logo, bool, 0644);
MODULE_PARM_DESC(skip_boot_logo, "Skip kernel ADF boot logo handoff");
							   ...
							   *post)
{
	...
	for (i = 0; i < post->n_bufs; i++) {
		...
		dst->y2r_coef = cust_src->y2r_coef;
						    ...
						    int index)
{
	...
	if (!np) {
		pr_err("error: sprd,dispc phandle missing\n");
	...
	if (!dispc) {
		pr_info("dispc drvdata is not ready, defer adf probe\n");
		return ERR_PTR(-EPROBE_DEFER);
	}
		...
		struct platform_device *pdev)
{
	...
	case SPRD_SINGLE_DISPC:
		...
		pipe->n_sdev = 1;
		dispc = sprd_device_init(pdev, 0);
		if (IS_ERR(dispc))
			return PTR_ERR(dispc);
		pipe->sdev[0] = dispc;
		break;
	...
	case SPRD_DOUBLE_DISPC_INDEPENDENT_CONTENT:
		pr_info("interface_id = %d\n", interface_id);
		pipe->n_sdev = 1;
		dispc = sprd_device_init(pdev, interface_id);
		if (IS_ERR(dispc))
			return PTR_ERR(dispc);
		pipe->sdev[interface_id] = dispc;
		break;
	default:
		return -1;
	}
...
static int sprd_adf_probe(struct platform_device *pdev)
{
	...
	if (skip_boot_logo) {
		pr_info("skip ADF boot logo handoff\n");
	} else {
