...
static ssize_t gen_read_store(struct device *dev,
	...
	mipi_dsi_set_max_return_size(dsi, input_param[1]);
	mipi_dsi_gen_read(dsi, &input_param[0], 1,
...
static ssize_t gen_write_store(struct device *dev,
	...
	mipi_dsi_gen_write(dsi, input_param, input_len);
...
static ssize_t dcs_read_store(struct device *dev,
	...
	mipi_dsi_set_max_return_size(dsi, input_param[1]);
	sprd_mipi_dsi_dcs_read(dsi, input_param[0],
			  read_buf, input_param[1]);
...
static ssize_t dcs_write_store(struct device *dev,
	...
	sprd_mipi_dsi_dcs_write(dsi, input_param, input_len);
...
static ssize_t work_mode_store(struct device *dev,
	...
	if (!strncmp(buf, "video", strlen("video")))
		...
		mipi_dsi_set_work_mode(dsi, DSI_MODE_CMD);
			       ...
			       char *buf)
{
	...
	mode = mipi_dsi_get_work_mode(dsi);
...
static ssize_t lp_cmd_en_store(struct device *dev,
	...
	lp_cmd_en = enable;
	mipi_dsi_lp_cmd_enable(dsi, enable);
