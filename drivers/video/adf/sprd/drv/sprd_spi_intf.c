...
static int sprd_spi_dev_probe(struct spi_device *spi_dev)
{
	...
	spi_dev->chip_select[0] = panel->spi_cs;
...
static void sprd_spi_dev_remove(struct spi_device *spi_dev)
{
	spi->spi_dev = NULL;
	spi_set_drvdata(spi_dev, NULL);
}

static const struct of_device_id sprd_spi_of_ids[] = {
	{ .compatible = "sprd,spi-panel" },
	{ },
};
MODULE_DEVICE_TABLE(of, sprd_spi_of_ids);

static struct spi_driver spi_drv = {
	.driver = {
		.name = "spi-panel-drv",
		.owner = THIS_MODULE,
		.of_match_table = sprd_spi_of_ids,
	},
