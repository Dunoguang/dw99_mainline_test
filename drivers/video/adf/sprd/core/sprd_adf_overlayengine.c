				...
				size_t n_engs)
{
	...
	for (i = 0; i < n_engs; i++) {
		...
		pr_info("%s: registered %s id=%d minor=%d\n", __func__,
				engs[i].base.base.name, engs[i].base.base.id,
				engs[i].base.base.minor);
	}
...
/**
 * sprd_adf_destroy_overlay_engines - destroy sprd adf overlayengine obj
 *
 * @engs: sprd adf overlay_engine obj
 * @n_engs: the number of sprd adf overlay_engine
 *
 * sprd_adf_destroy_overlay_engines() destroy adf overlay_engine obj and
 * free resource
 */
int sprd_adf_destroy_overlay_engines(
			struct sprd_adf_overlay_engine *engs,
			size_t n_engs)
{
	size_t i;
