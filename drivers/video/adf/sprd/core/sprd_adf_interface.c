...
static int sprd_adf_set_preferred_mode(struct adf_interface *intf)
{
	...
	if (!sprd_adf_intf->ops || !sprd_adf_intf->ops->get_modes) {
		ADF_DEBUG_WARN("ops or ops->get_modes is illegal\n");
		return -EINVAL;
	}

	sprd_adf_intf->ops->get_modes(sprd_adf_intf, NULL, &n_modes);
	if (!n_modes) {
		ADF_DEBUG_WARN("get modelist count is 0\n");
		return -EINVAL;
	}

	modelist = kzalloc(sizeof(modelist[0]) * n_modes, GFP_KERNEL);
	if (!modelist) {
		ADF_DEBUG_WARN("kzalloc modelist failed\n");
		return -ENOMEM;
	}

	sprd_adf_intf->ops->get_modes(sprd_adf_intf, modelist, &n_modes);

	ADF_DEBUG_GENERAL("hdisplay = %d;vdisplay = %d;\n",
			  modelist[0].hdisplay, modelist[0].vdisplay);

	if (intf->flags & ADF_INTF_FLAG_PRIMARY)
		adf_hotplug_notify_connected(intf, modelist, n_modes);

	memcpy(&intf->current_mode, &modelist[0], sizeof(modelist[0]));
					     ...
					     u32 *offset, u32 *pitch)
{
	...
	*pitch = w * adf_format_bpp(format) / 8;
	...
	if (IS_ERR(handle)) {
		handle = ion_alloc(sprd_adf_intf->fb_ion_client,
		(size_t)size, 0, ION_HEAP_ID_MASK_SYSTEM, 0);
		if (IS_ERR(handle)) {
				 ...
				 struct drm_mode_modeinfo *mode)
{
	...
	if (!mode) {
		ADF_DEBUG_WARN("parameter mode is ilegal\n");
		goto err_out;
	}
	...
	if (!sprd_adf_intf->ops || !sprd_adf_intf->ops->set_mode) {
		ADF_DEBUG_WARN("ops or ops->set_mode is illegal\n");
		return -EINVAL;
	}

	return sprd_adf_intf->ops->set_mode(sprd_adf_intf, mode);
...
/**
 * sprd_adf_intf_modeget - get index of interface mode
 *
 * @intf: sprd adf interface obj
 * @index: drm mode index
 *
 * Return 0 on success or error code (<0) if the mode could not be
 * get.
 */
static int sprd_adf_intf_modeget(struct adf_interface *intf, u32 *index)
{
	struct sprd_adf_interface *sprd_adf_intf;
	...
	if (!sprd_adf_intf->ops || !sprd_adf_intf->ops->get_mode) {
		ADF_DEBUG_WARN("ops or ops->get_mode is illegal\n");
		return -EINVAL;
	}


	return sprd_adf_intf->ops->get_mode(sprd_adf_intf, index);
}
			...
			size_t n_intfs)
{
	...
	for (i = 0; i < n_intfs; i++) {
		...
		pr_info("%s: registered %s id=%d minor=%d\n", __func__,
				intfs[i].base.base.name, intfs[i].base.base.id,
				intfs[i].base.base.minor);
...
error_out0:
	for (; i >= 0; i--) {
		...
		sprd_adf_destroy_interface_private_data(intfs[i].data);
	}
...
/**
 * sprd_adf_destroy_interfaces - destroy sprd adf interface obj
 *
 * @intfs: sprd adf interface obj
 * @n_intfs: the number of sprd adf interface
 *
 * sprd_adf_destroy_interfaces() destroy adf interface obj and free resource
 */
int sprd_adf_destroy_interfaces(
				struct sprd_adf_interface *intfs,
				size_t n_intfs)
{
	size_t i;
