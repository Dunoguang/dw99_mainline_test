...
#define ADF_UEVENT_RETRY_COUNT 4
#define ADF_UEVENT_RETRY_DELAY_MS 15000

static void adf_obj_uevent_work(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct adf_obj *obj = container_of(dwork, struct adf_obj, uevent_work);

	pr_info("%s: replay add uevent for %s id=%d minor=%d retry=%u\n",
			__func__, dev_name(&obj->dev), obj->id, obj->minor,
			obj->uevent_retries);
	kobject_uevent(&obj->dev.kobj, KOBJ_ADD);

	obj->uevent_retries++;
	if (obj->uevent_retries < ADF_UEVENT_RETRY_COUNT)
		schedule_delayed_work(&obj->uevent_work,
				msecs_to_jiffies(ADF_UEVENT_RETRY_DELAY_MS));
}
...
int adf_obj_sysfs_init(struct adf_obj *obj, struct device *parent)
{
	...
	pr_info("%s: registered %s id=%d minor=%d\n", __func__,
			dev_name(&obj->dev), obj->id, obj->minor);
	obj->uevent_retries = 0;
	INIT_DELAYED_WORK(&obj->uevent_work, adf_obj_uevent_work);
	kobject_uevent(&obj->dev.kobj, KOBJ_ADD);
	schedule_delayed_work(&obj->uevent_work,
			msecs_to_jiffies(ADF_UEVENT_RETRY_DELAY_MS));
	return 0;
...
static char *adf_device_devnode(const struct device *dev, umode_t *mode,
...
static char *adf_interface_devnode(const struct device *dev, umode_t *mode,
		kuid_t *uid, kgid_t *gid)
{
	struct adf_obj *obj = container_of(dev, struct adf_obj, dev);
...
static char *adf_overlay_engine_devnode(const struct device *dev, umode_t *mode,
		kuid_t *uid, kgid_t *gid)
{
	struct adf_obj *obj = container_of(dev, struct adf_obj, dev);
...
int adf_device_sysfs_init(struct adf_device *dev)
{
	...
	dev_set_name(&dev->base.dev, "adf%d", dev->base.id);
...
int adf_interface_sysfs_init(struct adf_interface *intf)
{
	...
	dev_set_name(&intf->base.dev, "adf-interface%d.%d", parent->base.id,
			intf->base.id);
...
int adf_overlay_engine_sysfs_init(struct adf_overlay_engine *eng)
{
	...
	dev_set_name(&eng->base.dev, "adf-overlay-engine%d.%d",
			parent->base.id, eng->base.id);
...
{
	cancel_delayed_work_sync(&obj->uevent_work);
	idr_remove(&adf_minors, obj->minor);
...
int adf_sysfs_init(void)
{
	...
	adf_class = class;
	adf_major = ret;
	pr_info("%s: registered adf char device major=%d\n", __func__,
			adf_major);
	return 0;
