		...
		struct drm_mode_modeinfo *mode)
{
	...
	if (vmode->name)
		strscpy(mode->name, vmode->name, sizeof(mode->name));
	else
...
static int adf_fb_alloc(struct adf_fbdev *fbdev)
{
	...
	if (dma_buf_vmap(fbdev->dma_buf, &fbdev->map)) {
		ret = -ENOMEM;
		dev_err(fbdev->info->dev, "vmapping fb failed\n");
		goto err_vmap;
	}
	fbdev->vaddr = fbdev->map.vaddr;
...
{
	dma_buf_vunmap(fbdev->dma_buf, &fbdev->map);
	dma_buf_put(fbdev->dma_buf);
}
		...
		struct fb_ops *fbops, const char *fmt, ...)
{
	...
	fbdev->info->flags = 0; /* FBINFO_DEFAULT */
