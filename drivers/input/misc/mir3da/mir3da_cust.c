...
#define DELAY_INTERVAL_MAX               2000
#define DELAY_INTERVAL_MIN               0
#define DELAY_INTERVAL_DEFAULT           50 
...
#ifdef USE_HRTIMER
static enum hrtimer_restart mir3da_acc_work(struct hrtimer *timer)
{
	struct mir3da_data *acc;
	ktime_t poll_delay;

	acc = container_of((struct hrtimer *)timer,
	struct mir3da_data, work_timer);

	complete(&acc->report_complete);
	if (atomic_read(&acc->delay) > 0)
	{
		poll_delay = ktime_set(0, atomic_read(&acc->delay) * NSEC_PER_MSEC );
	}
	else
	{
		poll_delay = ktime_set(0, DELAY_INTERVAL_DEFAULT * NSEC_PER_MSEC);
	}
...
{
	struct mir3da_data *acc = data;
	short x=0,y=0,z=0;
	static short temp_x=0,temp_y=0,temp_z=0;
	int map_para = 1;
	...
	while(1)
	{
		...
		mutex_lock(&acc->enable_mutex);
		if (atomic_read(&acc->enable) <= 0)
		{
			mutex_unlock(&acc->enable_mutex);
			continue;
		}
		if (mir3da_read_data(acc->mir3da_i2c_client, &x, &y, &z) != 0) {
			...
			input_report_abs(acc->input, ABS_X, x);
			input_report_abs(acc->input, ABS_Y, y);
			input_report_abs(acc->input, ABS_Z, z);
			input_sync(acc->input); 

			MI_DATA("input report x=%d, y=%d, z=%d !\n", x, y, z);
		}
		mutex_unlock(&acc->enable_mutex);
	}
	return 0;
...
static long mir3da_misc_ioctl( struct file *file,unsigned int cmd, unsigned long arg)
{
	...
	case MIR3DA_ACC_IOCTL_SET_DELAY:
		if (copy_from_user(&interval, argp, sizeof(interval)))
			return -EFAULT;
		if (interval < DELAY_INTERVAL_MIN || interval > DELAY_INTERVAL_MAX)
			interval = DELAY_INTERVAL_DEFAULT;
...
#ifdef USE_HRTIMER
		if (bEnable) {
			if (hrtimer_try_to_cancel(&mir3da->work_timer) < 0) 
				return -1; 

			mir3da->hrtimer_running = true;
			if (atomic_read(&mir3da->delay)  > 0)
			{
				poll_delay = ktime_set(0, atomic_read(&mir3da->delay)  * NSEC_PER_MSEC);
			}
			else
			{
				poll_delay = ktime_set(0, DELAY_INTERVAL_DEFAULT * NSEC_PER_MSEC);
			...
			if(mir3da_set_enable(mir3da->mir3da_i2c_client, 1) < 0)
				return -EINVAL;		
			hrtimer_start(&mir3da->work_timer, poll_delay, HRTIMER_MODE_REL);		
		}else{
			if (mir3da->hrtimer_running)
			{
				mir3da->hrtimer_running = false;
				hrtimer_cancel(&mir3da->work_timer);
				if(mir3da_set_enable(mir3da->mir3da_i2c_client, 0) < 0)
					return -EINVAL;
			}
		}
#else
...
static ssize_t mir3da_delay_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	MI_MSG("mir3da_delay_store data=: %ld", data);
	if (data < DELAY_INTERVAL_MIN || data > DELAY_INTERVAL_MAX)
	{
		//return -EINVAL;
		data = DELAY_INTERVAL_DEFAULT;
	}
...
static int mir3da_read_chip_id(struct i2c_client *client, char *buf)
{
	return sprintf(buf, "%d\n", 0x0013);  
}

static ssize_t mir3da_chip_id_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);

	return mir3da_read_chip_id(client, buf);
}
...
static DEVICE_ATTR(enable,          S_IRUGO|S_IWUSR|S_IWGRP,  mir3da_enable_show,             mir3da_enable_store);
static DEVICE_ATTR(delay,           S_IRUGO|S_IWUSR|S_IWGRP,  mir3da_delay_show,              mir3da_delay_store);
...
static DEVICE_ATTR(direction_remap, S_IRUGO|S_IWUSR|S_IWGRP,  mir3da_direction_remap_show,    mir3da_direction_remap_store);
#if MIR3DA_OFFSET_TEMP_SOLUTION
static DEVICE_ATTR(offset,          S_IRUGO|S_IWUSR|S_IWGRP,  mir3da_offset_show,             NULL);
...
static DEVICE_ATTR(chip_id,         S_IRUGO,            mir3da_chip_id_show,            NULL);
...
int i2c_smbus_read(PLAT_HANDLE handle, u8 addr, u8 *data)
{
	...
	MI_ERR("i2c_smbus_read: addr = 0x%x!\n", addr);

	*data = i2c_smbus_read_byte_data(client, addr);
	MI_ERR("i2c_smbus_read: data = 0x%x!\n", *data);
...
#if MIR3DA_OFFSET_TEMP_SOLUTION
MIR_GENERAL_OPS_DECLARE(ops_handle, i2c_smbus_read, i2c_smbus_read_block, i2c_smbus_write, sensor_sync_write, sensor_sync_read, check_califolder_exist,get_address,support_fast_auto_cali,msdelay, _printk, sprintf);
#else
MIR_GENERAL_OPS_DECLARE(ops_handle, i2c_smbus_read, i2c_smbus_read_block, i2c_smbus_write, NULL, NULL, NULL,get_address,NULL,msdelay, _printk, sprintf);
#endif
...
static int mir3da_probe(struct i2c_client *client)
{
	...
	struct mir3da_data *mir3da=NULL;
	...
	MI_ERR("enter mir3da_probe!\n");

#ifdef CONFIG_ZYT_GSENSOR_COMPATIBLE
	if(CDC_Gsensor_Device_Id(0)!=0)
	{
		printk("GSensor(0x%x)Exist!\n", CDC_Gsensor_Device_Id(0));
		return -ENODEV;
	}
#endif
...
#if MIR3DA_OFFSET_TEMP_SOLUTION
	MI_ERR("enter mir3da_probe  MIR3DA_OFFSET_TEMP_SOLUTION!\n");
	...
	MI_ERR("mir3da_probe: chip_id = 0x%x!\n", chip_id);
	...
	{
		result = -ENODEV;
		printk("Fail to find mir3da sensor.\n");
		goto exit_check_chip_failed;
	}
#endif

#if defined(ZCFG_GSENSOR_MIR3DA_DIRECTION)
	direction_remap = ZCFG_GSENSOR_MIR3DA_DIRECTION;
#else
	...
	atomic_set(&mir3da->delay, DELAY_INTERVAL_DEFAULT);
...
#ifdef USE_HRTIMER
	hrtimer_init(&mir3da->work_timer, CLOCK_MONOTONIC, HRTIMER_MODE_REL);
	mir3da->work_timer.function = mir3da_acc_work;
	mir3da->hrtimer_running = false;
	init_completion(&mir3da->report_complete);
	mir3da->thread = kthread_run(report_event, mir3da, "sensor_report_event");
	if (IS_ERR(mir3da->thread))
	{
		MI_ERR("unable to create report_event thread\n");
		result = -EINVAL;
		goto err_cancel_hrtimer;
	}
	sched_setscheduler_nocheck(mir3da->thread, SCHED_FIFO, &param);
#endif
...
{
	int result = 0;
	struct mir3da_data *mir3da = i2c_get_clientdata(to_i2c_client(dev));
...
#define MIR3DA_PM_OPS  (&mir3da_pm_ops)
#else /* CONFIG_HAS_EARLYSUSPEND */
#define MIR3DA_PM_OPS NULL
#endif /* CONFIG_HAS_EARLYSUSPEND */
#endif /* KERNEL4 */
		       ...
		       struct i2c_board_info *info)
{
	...
	strscpy(info->type, "da311", I2C_NAME_SIZE);
	//strscpy(info->type, MIR3DA_DRV_NAME, I2C_NAME_SIZE);
