...
static int ts_request_gpio(struct ts_data *pdata)
{
	...
	if (!board->int_gpio) {
		...
		retval = devm_gpio_request_one(&pdata->pdev->dev,
			board->int_gpio, GPIOF_IN, ATS_INT_LABEL);
		if (retval < 0) {
	...
	if (!board->rst_gpio) {
		...
		retval = devm_gpio_request_one(&pdata->pdev->dev,
			board->rst_gpio, GPIOF_IN, ATS_RST_LABEL);
		if (retval < 0) {
...
{
	/* 7.2 removed gpio_export; sysfs export not available */
	return 0;
}
...
{
	struct ts_data *pdata = container_of(t, struct ts_data, poll_timer);
...
static void ts_set_surface(struct ts_data *pdata, unsigned int width, unsigned int height)
{
	struct ts_board *board = pdata->board;
	...
	pdata->input = NULL;
	board->surface_width = width;
	board->surface_height = height;
	ts_configure(pdata);
	ts_register_input_dev(pdata);
...
static void ts_ext_event_handler(struct ts_data *pdata, enum ts_event event, void *data)
{
	...
	case TSEVENT_NOISE_NORMAL:
		...
		break;
	case TSEVENT_SETSURFACE:
		if (data)
			ts_set_surface(pdata, *((u32 *)data) & 0xffff, (*((u32 *)data) >> 16) & 0xffff);
		break;
	default:
...
static int ts_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&pdata->upgrade_lock, WAKE_LOCK_SUSPEND, ATS_WL_UPGRADE_FW);
...
static void ts_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&pdata->upgrade_lock);
