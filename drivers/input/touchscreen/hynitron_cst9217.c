...
#define READ_BUF_LEN            (7 + (MAX_POINTS - 1) * 5 + 1)
...
static void cst9217_report_touch(struct cst9217_ts_data *ts)
{
    ...
    /*
     * CST92 returns one point plus count and trailer in the first seven
     * bytes. Additional points end with a second trailer at D007.
     */
    ret = hyn_i2c_read(ts->client, HYN_REG_BASE, buf, 7);
    if (ret < 0) {
    ...
    print_hex_dump(KERN_INFO, "[HYN_RAW] ", DUMP_PREFIX_NONE, 16, 1, buf, 7, false);
#endif

    if (buf[6] != 0xAB || buf[0] == 0xAB)
        goto out_ack;

    /* This DTS has no virtual-key definition, so never decode key frames. */
    if (buf[5] & 0x80)
        goto out_ack;
    ...
    if (point_num > 1) {
        ret = hyn_i2c_read(ts->client, HYN_REG_BASE + 7, &buf[7],
                           (point_num - 1) * 5 + 1);
        if (ret < 0 || buf[7 + (point_num - 1) * 5] != 0xAB)
            goto out_ack;
    }

    /* Reject the whole frame before reporting any slot from malformed data. */
    for (i = 0; i < point_num; i++) {
        idx = (i == 0) ? 0 : 7 + (i - 1) * 5;
        id = (buf[idx] >> 4) & 0x0F;
        finger_state = buf[idx] & 0x0F;
        event = finger_state >> 1;
        x = ((unsigned int)buf[idx + 1] << 4) |
            ((buf[idx + 3] >> 4) & 0x0F);
        y = ((unsigned int)buf[idx + 2] << 4) |
            (buf[idx + 3] & 0x0F);

        if (id >= MAX_POINTS || test_bit(id, &seen_ids) ||
            (event != 0 && event != (HYN_STAT_DOWN >> 1)) ||
            x > ts->abs_x_max || y > ts->abs_y_max)
            goto out_ack;

        __set_bit(id, &seen_ids);
    }

    if (hyn_i2c_write(ts->client, HYN_REG_BASE, &sync_data, 1) < 0)
        goto out_unlock;
