...
static int smsgc_thread(void *data)
{
	...
	if (rval != 0) {
		pr_err("Smsgc: Failed to open channel %d\n", smsgc->channel);
...
int smsgc_create(u8 dst, u8 channel)
{
	...
	smsgc->txbuf = kzalloc(SMSG_CACHE_NR * sizeof(struct smsg),
						GFP_KERNEL);
	if (!smsgc->txbuf) {
		kfree(smsgc);
		pr_err("Failed to allocate txbuf for smsgc\n");
		return -ENOMEM;
	}
	...
	if (IS_ERR(smsgc->thread)) {
		pr_err("Failed to create kthread: smsgc-%d-%d\n",
				dst, channel);
...
void smsgc_destroy(u8 dst, u8 channel)
{
	struct smsgc_mgr *smsgc;
	u8 ch_index;
...
int smsgc_poll_wait(u8 dst, u8 channel,
		    struct file *filp, poll_table *wait)
{
	...
	if (smsgc->state != SMSGC_STATE_READY) {
		pr_err("smsgc-%d-%d not ready to poll !\n", dst, channel);
		return -ENODEV;
	}
...
int smsgc_status(u8 dst, u8 channel)
{
	struct smsgc_mgr *smsgc;
	u8 ch_index;
...
static int smsgc_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		for (j = 0;  j < SMSG_VALID_CH_NR; j++) {
			...
			seq_printf(m, "smsgc dst %d, channel: %3d, state: %d, txmsgs: %d, cache-rdptr: %d, cache-wrptr: %d\n",
				smsgc->dst, smsgc->channel, smsgc->state,
				smsgc->txmsg_cnt,
				readl(smsgc->rdptr), readl(smsgc->wrptr));
			cnt = 0;
...
int  smsgc_init_debugfs(void *root)
{
	if (!root)
		return -ENXIO;
	debugfs_create_file("smsgc", S_IRUGO,
		(struct dentry *)root, NULL, &smsgc_debug_fops);
	return 0;
}
EXPORT_SYMBOL_GPL(smsgc_init_debugfs);
...
int smsgc_wait_event(struct smsgc_mgr *smsgc, int timeout, char *str)
{
	...
	if (readl(smsgc->wrptr) == readl(smsgc->rdptr)) {
		if (timeout == 0) {
			/* no wait */
			pr_warn("smsgc-%d-%d rxcache is empty!\n",
				smsgc->dst, smsgc->channel);
			rval = -EAGAIN;
		} else if (timeout < 0) {
			/* wait forever */
			rval = wait_event_interruptible(smsgc->rxwait,
				...
				smsgc->state == SMSGC_STATE_IDLE);
			if (rval < 0)
				pr_warn("smsgc-%d-%d %s wait interrupted!\n",
					smsgc->dst, smsgc->channel, str);

			if (smsgc->state == SMSGC_STATE_IDLE) {
				pr_err("smsgc_%s smsgc-%d-%d state is idle!\n",
			...
			if (rval < 0) {
				...
				pr_warn("smsgc-%d-%d %s wait timeout!\n",
					smsgc->dst, smsgc->channel, str);
				rval = -ETIME;
			}
...
int smsgc_recv(u8 dst, struct smsg *msg, int timeout)
{
	struct smsgc_mgr *smsgc;
	ssize_t rval = 0;
	ssize_t rd;
	...
	if (smsgc->state != SMSGC_STATE_READY) {
		pr_err("smsgc-%d-%d not ready to recv!\n",
			smsgc->dst, smsgc->channel);
		return -ENODEV;
	}
...
int smsgc_send(u8 dst, struct smsg *msg, int timeout)
{
	struct smsgc_mgr *smsgc;
	ssize_t rval = 0;
	u8 ch_index;
	...
	if (smsgc->state != SMSGC_STATE_READY) {
		pr_err("smsgc-%d-%d not ready to send!\n",
			smsgc->dst, smsgc->channel);
		return -ENODEV;
	}
	if (!mutex_trylock(&(smsgc->txlock))) {
		pr_info("smsgc-%d-%d send busy!\n",
			smsgc->dst, smsgc->channel);
		return -EBUSY;
	}
	if (smsgc->state == SMSGC_STATE_READY) {
...
static int smsgc_open(struct inode *inode, struct file *filp)
{
	...
	if (smsgc_status(smsgc->init->dst,
			smsgc->init->channel) != 0) {
		pr_err("smsgc %d-%d not ready to open!\n",
		...
		char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (smsgc->state != SMSGC_STATE_READY) {
		pr_err("smsgc-%d-%d not ready to read!\n",
			smsgc->dst, smsgc->channel);
		return -ENODEV;
	}

	if ((count%sizeof(struct smsg)) != 0) {
		pr_warn("smsgc_read %d-%d: count=%d, aligned to 8 bytes, Invalid arguments.\n",
	...
	while (left && (readl(smsgc->wrptr) != readl(smsgc->rdptr))
		...
		if (tail > 0) {
			if ((uintptr_t)buf > TASK_SIZE) {
				unalign_memcpy(buf,
					&(smsgc->caches[rd]),
					(rxmsgs - tail) * sizeof(struct smsg));
				unalign_memcpy(
					buf + (rxmsgs - tail) * sizeof(struct smsg),
					&(smsgc->caches[0]),
					tail * sizeof(struct smsg));
			} else {
				/* handle the user space address */
				if (unalign_copy_to_user((void __user *)buf,
					&(smsgc->caches[rd]),
					(rxmsgs - tail) * sizeof(struct smsg)) ||
				    unalign_copy_to_user((void __user *)(
						buf + (rxmsgs - tail) * sizeof(struct smsg)),
					    &(smsgc->caches[0]),
						tail * sizeof(struct smsg))) {
					pr_err("smsgc-%d-%d read failed to copy to user!\n",
					smsgc->dst, smsgc->channel);
					rval = -EFAULT;
					break;
				}
			...
			if ((uintptr_t)buf > TASK_SIZE) {
				unalign_memcpy(buf, &(smsgc->caches[rd]),
				rxmsgs * sizeof(struct smsg));
			} else {
				/* handle the user space address */
				if (unalign_copy_to_user((void __user *)buf,
					&(smsgc->caches[rd]),
	...
	pr_debug("smsgc_read done: dst=%d, channel=%d, count=%d, read=%d, rval=%d",
		smsgc->dst, smsgc->channel,
		count, count - left * sizeof(struct smsg),
		rval);
		...
		const char __user *buf, size_t count, loff_t *ppos)
{
	...
	if (smsgc->state != SMSGC_STATE_READY) {
		pr_err("smsgc-%d-%d not ready to write!\n",
			smsgc->dst, smsgc->channel);
		return -ENODEV;
	}

	if ((count%sizeof(struct smsg)) != 0) {
		pr_err("smsgc_write %d-%d: count=%d, Invalid arguments.\n",
			smsgc->dst, smsgc->channel, count);
		return -EINVAL;
	}
	left = count/sizeof(struct smsg);
	...
	while (left && smsgc->state == SMSGC_STATE_READY) {
		...
		unalign_copy_from_user(smsgc->txbuf,
			(void __user *)(buf + count - left * sizeof(struct smsg)),
			txmsgs * sizeof(struct smsg));
		for (i = 0; i < txmsgs; i++) {
			/*smsg tx buf should not be full, timeout is ignored*/
			rval = smsg_send(smsgc->dst,
				(struct smsg *)(smsgc->txbuf + i * sizeof(struct smsg)),
				timeout);
			if (rval)
				goto end;
	...
	pr_debug("smsgc_write done: dst=%d, channel=%d, count=%d, sent=%d, rval=%d",
		smsgc->dst, smsgc->channel,
		count, count - left * sizeof(struct smsg), rval);
...
static int smsgc_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node && !init) {
		...
		if (rval) {
			pr_err("Failed to parse smsgc device tree, ret=%d\n",
					rval);
			return rval;
		}
	}
	...
	if (rval != 0) {
		pr_err("Failed to create smsgc: %d\n", rval);
	...
	if (smsgc_dev == NULL) {
		...
		smsgc_destroy_pdata(&init);
		pr_err("Failed to allocate smsgc_device\n");
		return -ENOMEM;
	}
	...
	if (rval != 0) {
		...
		kfree(smsgc_dev);
		smsgc_destroy_pdata(&init);
		pr_err("Failed to alloc smsgc chrdev\n");
		return rval;
	}
	...
	if (rval != 0) {
		...
		smsgc_destroy_pdata(&init);
		pr_err("Failed to add smsgc cdev\n");
		return rval;
	}
...
static void smsgc_remove(struct platform_device *pdev)
{
...
{
	smsgc_class = class_create("smsgc");
