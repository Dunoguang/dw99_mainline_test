...
struct stty_device {
	...
	/* stty state */
	u32			state;
	struct mutex		stat_lock; /* lock for stty status change */
};
...
static void stty_handler (int event, void *data)
{
	...
	pr_debug("gnss stty handler event=%d\n", event);
	...
	case SBUF_NOTIFY_READ:
		do {
			cnt = sbuf_read(stty->pdata->dst,
					...
					STTY_MAX_DATA_LEN,
					0);
			pr_debug("gnss stty handler read data len =%d\n", cnt);
			mutex_lock(&stty->stat_lock);
			if ((stty->state == STTY_STATE_OPEN) && (cnt > 0)) {
				for (i = 0; i < cnt; i++) {
					tty_insert_flip_char(stty->port, buf[i],
							     TTY_NORMAL);
				}
		...
		break;
	default:
		pr_info("gnss stty get invalid event: %d\n", event);
	}
...
static int stty_open(struct tty_struct *tty, struct file *filp)
{
	...
	if (!tty) {
		pr_err("gnss stty open input tty is NULL!\n");
		return -ENOMEM;
	}
	driver = tty->driver;
	stty = (struct stty_device *)driver->driver_state;
	if (!stty) {
		pr_info("gnss stty open input stty NULL!\n");
		return -ENOMEM;
	}

	if (sbuf_status(stty->pdata->dst, stty->pdata->channel) != 0) {
		pr_err("gnss stty_open not ready to open!dst=%d,channel=%d\n",
	...
	pr_debug("gnss stty_open device success!\n");
...
static void stty_close(struct tty_struct *tty, struct file *filp)
{
	...
	if (!tty) {
		pr_info("gnss stty close input tty is NULL!\n");
		return;
	}
	stty = (struct stty_device *)tty->driver_data;
	if (!stty) {
		pr_info("gnss stty close s tty is NULL!\n");
		return;
	}
	...
	pr_debug("gnss stty_close device success !\n");
}
		      ...
		      int count)
{
	...
	pr_debug("gnss stty write count=%d\n", count);
...
static int stty_parse_dt(struct stty_init_data **init, struct device *dev)
{
	...
	ret = of_property_read_u32(np, "sprd,bufid", (u32 *)&pdata->bufid);
	if (ret)
		goto error;
...
static int  stty_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node && !pdata) {
		...
		if (rval) {
			pr_info("failto parse gnss stty device tree, ret=%d\n",
				rval);
			return rval;
		}
	}
	pr_info("gnss stty: device name=%s, dst=%u, channel=%u, bufid=%u\n",
	...
	if (!stty) {
		...
		pr_info("gnss stty Failed to allocate device!\n");
		return -ENOMEM;
	}
	...
	if (rval) {
		...
		stty_destroy_pdata(&pdata, &pdev->dev);
		pr_info("gnss stty driver init error!\n");
		return -EINVAL;
	}
	...
	if (rval) {
		...
		kfree(stty->port);
		devm_kfree(&pdev->dev, stty);
		pr_info("gnss stty regitster notifier failed (%d)\n", rval);
		return rval;
	}

	pr_info("gnss stty_probe init device addr: 0x%p\n", stty);
	platform_set_drvdata(pdev, stty);

	return 0;
}

static void stty_remove(struct platform_device *pdev)
{
	...
	if (rval) {
		pr_info("gnss stty unregitster notifier failed (%d)\n", rval);
		return rval;
	}
