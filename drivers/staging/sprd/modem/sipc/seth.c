...
static int seth_rx_poll_handler(struct napi_struct *napi, int budget)
{
	...
	if (!seth) {
		pr_err("seth_rx_poll_handler no seth device\n");
		return 0;
	}
	...
	while ((budget - skb_cnt) && !blk_ret) {
		...
		if (blk_ret) {
			pr_debug("receive sblock error %d\n", blk_ret);
			continue;
		}
		if (seth->is_rawip)
			skb = dev_alloc_skb(blk.length + ETH_HLEN + NET_IP_ALIGN);
		else
	...
	if (skb_cnt >= 0 && budget > skb_cnt) {
		...
		if (SBLOCK_GET_ARRIVED_COUNT(pdata->dst, pdata->channel) > 0) {
			...
			pr_debug("start rx_timer, jiffies %lu.\n", jiffies);
		}
	}
	return skb_cnt;
}
...
static void seth_tx_ready_handler (void *data)
{
	...
	pr_debug("seth_tx_ready_handler state %0x\n", seth->state);
...
static void seth_tx_open_handler (void *data)
{
	...
	pr_debug("seth_tx_open_handler state %0x\n", seth->state);
	if (seth->state != DEV_ON) {
		seth->state = DEV_ON;
		seth->txstate = DEV_ON;
		if (!netif_carrier_ok(seth->netdev))
...
static void seth_tx_close_handler (void *data)
{
	...
	pr_info("seth_tx_close_handler state %0x\n", seth->state);
...
static void seth_rx_handler (void *data)
{
	...
	if (!seth) {
		pr_err("seth_rx_handler data is NULL.\n");
		return;
	}
...
static void seth_rx_timer_handler(struct timer_list *t)
{
	struct SETH *seth = container_of(t, struct SETH, rx_timer);

	pr_info("rx_timer expried, jiffies %lu.\n", jiffies);
...
static void seth_tx_pre_handler (void *data)
{
	...
	if (seth->txstate != DEV_ON) {
		seth->txstate = DEV_ON;
		pr_info("seth_tx_ready_handler txstate %0x\n", seth->txstate);
...
static void seth_handler (int event, void *data)
{
	...
	case SBLOCK_NOTIFY_OPEN:
		pr_debug("SBLOCK_NOTIFY_OPEN is received\n");
		seth_tx_open_handler(seth);
		break;
	case SBLOCK_NOTIFY_CLOSE:
		pr_debug("SBLOCK_NOTIFY_CLOSE is received\n");
...
static int seth_tx_pkt(void *data, struct sk_buff *skb, int is_ack)
{
	...
	if (ret) {
		pr_info("Get free sblock failed(%d), drop data!\n", ret);
		seth->stats.tx_fifo_errors++;
		return SETH_TX_NO_BLK;
	}

	if (blk.length < skb->len) {
		pr_err("Sblock %d too small, skb %d\n", blk.length, skb->len);
		goto send_fail;
	}
	if (seth->is_rawip)
	...
	if (ret < 0) {
		pr_err("Sblock_send fail, error %d\n", ret);
		goto send_fail;
	}
...
static void seth_tx_flush(struct timer_list *t)
{
	int ret;
	u32 cnt;
	struct SETH *seth = container_of(t, struct SETH, tx_timer);
	struct seth_init_data *pdata = seth->pdata;
	...
	if (ret)
		pr_info("seth tx failed(%d)!\n", ret);
	else
		atomic_set(&seth->txpending, 0);
}

static inline int get_pkt_proto(struct sk_buff *skb) {
	struct iphdr *iph = ip_hdr(skb);
...
static inline struct tcphdr *get_pkt_tcphdr(struct sk_buff *skb)
{
	u8 iph_len;
	struct iphdr *iph = ip_hdr(skb);
...
/*
  ...
  */
static inline bool pkt_need_nodelay(struct sk_buff *skb)
{
...
static inline bool pkt_use_ackpool(struct sk_buff *skb)
{
	struct tcphdr *tcph;
	struct ipv6hdr *ip6h;
	struct iphdr *iph = ip_hdr(skb);
	...
	if (protocol == IPPROTO_TCP) {
		...
		/* we simply consider a tcp ack is a tcp pkt with data_len = 0 */
		if (iph->version == 4) {
...
static int seth_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (seth->state != DEV_ON) {
		pr_err("xmit the state of %s is off\n", dev->name);
...
static int seth_probe(struct platform_device *pdev)
{
	...
	if (pdev->dev.of_node && !pdata) {
		...
		if (ret) {
			pr_err("failed to parse seth device tree, ret=%d\n", ret);
			return ret;
		}
	}
	pr_info("after parse dt, name=%s, dst=%u, channel=%u, blocknum=%u\n",
	...
	netdev = alloc_netdev(sizeof(struct SETH),
		ifname,
		NET_NAME_UNKNOWN,
		seth_setup);
	...
	ret = SBLOCK_REGISTER_NOTIFIER(pdata->dst,
			pdata->channel,
			seth_handler,
			seth);
...
static void seth_remove(struct platform_device *pdev)
{
	...
	netif_napi_del(&seth->napi);
	timer_delete_sync(&seth->rx_timer);
	timer_delete_sync(&seth->tx_timer);
	SBLOCK_DESTROY(pdata->dst, pdata->channel);
	seth_destroy_pdata(&pdata);
...
static int seth_debug_show(struct seq_file *m, void *v)
{
	...
	if (!seth) {
		pr_err("invalid data, seth is NULL\n");
		return -EINVAL;
	}
...
static int seth_debugfs_mknod(void *root, void *data)
{
	...
	debugfs_create_file(pdata->name,
			S_IRUGO,
			(struct dentry *)root,
			data,
			&seth_debug_fops);
...
static int __init seth_debugfs_init(void)
{
	...
	if (!root) {
		pr_err("failed to create seth debugfs dir\n");
		return -ENODEV;
	}
	return 0;
}
