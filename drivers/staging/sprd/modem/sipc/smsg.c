...
static irqreturn_t smsg_irq_handler(void *ptr, void *private)
{
	...
	*/
	...
	wake_lock_timeout(&ch->sipc_wake_lock, HZ / 2);
...
EXPORT_SYMBOL(sipc_get_wakeup_flag);

void sipc_set_wakeup_flag(void)
{
	g_wakeup_flag = 1;
}
EXPORT_SYMBOL(sipc_set_wakeup_flag);

void sipc_clear_wakeup_flag(void)
{
	g_wakeup_flag = 0;
}
EXPORT_SYMBOL(sipc_clear_wakeup_flag);
...
int smsg_ch_wake_unlock(u8 dst, u8 channel)
{
	...
	wake_unlock(&ch->sipc_wake_lock);
	return 0;
}
EXPORT_SYMBOL(smsg_ch_wake_unlock);
...
int smsg_ch_open(u8 dst, u8 channel, int timeout)
{
	...
	wake_lock_init(&ch->sipc_wake_lock,
		       WAKE_LOCK_SUSPEND,
		       ch->wake_lock_name);
...
static int sipc_syscore_suspend(void *data)
{
	int ret = 0;
...
static struct syscore sipc_syscore = {
	.ops = &sipc_syscore_ops,
};

int  smsg_suspend_init(void)
{
	if (!is_wklock_setup) {
		register_syscore(&sipc_syscore);
		is_wklock_setup = 1;
	}
