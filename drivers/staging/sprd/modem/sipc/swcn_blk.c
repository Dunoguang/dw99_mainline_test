	...
	while (!kthread_should_stop()) {
		...
		case SMSG_TYPE_EVENT:
			...
			case SMSG_EVENT_SWCNBLK_SEND:
				wake_up_interruptible_all(&swcnblk->
							  ring->
							  recvwait);
				if (swcnblk->handler)
			...
			case SMSG_EVENT_SWCNBLK_RELEASE:
				wake_up_interruptible_all(&(swcnblk->ring->
							    getwait));
				if (swcnblk->handler)
		   ...
		   void *data)
{
	...
	swcnblk->thread = kthread_create(swcnblk_thread, swcnblk,
			"swcnblk-%d-%d", info->dst, info->channel);
	...
	if (poolhd->txblk_rdptr == poolhd->txblk_wrptr) {
		if (timeout == 0) {
			...
			rval = wait_event_interruptible(
					...
					swcnblk->state == SWCNBLK_STATE_IDLE);
			if (rval < 0)
				pr_info("swcnblk_get wait interrupted!\n");

			if (swcnblk->state == SWCNBLK_STATE_IDLE) {
				pr_err("swcnblk_get swcnblk state is idle!\n");
				rval = -EIO;
			}
		} else {
			/* wait timeout */
			...
			if (rval < 0) {
				...
				pr_info("swcnblk_get wait timeout!\n");
				rval = -ETIME;
			}

			if (swcnblk->state == SWCNBLK_STATE_IDLE) {
				pr_info("swcnblk_get swcnblk state is idle!\n");
				rval = -EIO;
			}
		}
	}
		    ...
		    struct swcnblk_blk *blk, int timeout)
{
	...
	pr_debug("swcnblk_receive: dst=%d, channel=%d, timeout=%d\n",
		 dst, channel, timeout);
	pr_debug("swcnblk_receive: channel=%d, wrptr=%d, rdptr=%d",
		 channel, ringhd->rxblk_wrptr, ringhd->rxblk_rdptr);

	if (ringhd->rxblk_wrptr == ringhd->rxblk_rdptr) {
		if (timeout == 0) {
			/* no wait */
			pr_debug("swcnblk_receive %d-%d is empty!\n",
				 dst, channel);
			rval = -ENODATA;
		} else if (timeout < 0) {
			/* wait forever */
			rval = wait_event_interruptible(ring->recvwait,
							...
							ringhd->rxblk_rdptr);
			if (rval < 0)
				pr_info("swcnblk_receive wait interrupted!\n");

			if (swcnblk->state == SWCNBLK_STATE_IDLE) {
				pr_info("swcnblk_receive swcnblk state is idle!\n");
				rval = -EIO;
			}
			...
			if (rval < 0) {
				...
				pr_info("swcnblk_receive wait timeout!\n");
				rval = -ETIME;
			}
	...
	if (ringhd->rxblk_wrptr != ringhd->rxblk_rdptr &&
		...
		pr_debug("swcnblk_receive: channel=%d, rxpos=%d, addr=%p, len=%d\n",
...
int swcnblk_release(u8 dst, u8 channel, struct swcnblk_blk *blk)
{
	...
	pr_debug("swcnblk_release: dst=%d, channel=%d, addr=%p, len=%d\n",
		 dst, channel, blk->addr, blk->length);
	...
	poolhd->rxblk_wrptr = poolhd->rxblk_wrptr + 1;
	pr_debug("swcnblk_release: addr=%x\n", ring->p_rxblks[rxpos].addr);
	...
	for (i = 0; i < SIPC_ID_NR; i++) {
		for (j = 0;  j < SMSG_VALID_CH_NR; j++) {
			...
			if (!swcnblk)
				continue;
			ring = swcnblk->ring;
			ringhd = (VOLA_SWCNBLK_RING *)(&swcnblk->ring->
						      header->ring);
			poolhd = (VOLA_SWCNBLK_RING *)(&swcnblk->ring->
						      header->pool);
...
static void swcnblk_remove(struct platform_device *pdev)
{
