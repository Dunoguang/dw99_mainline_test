...
void sync_timeline_signal(struct sync_timeline *obj)
{
	...
	trace_staging_sync_timeline(obj);
...
static struct sync_fence *sync_fence_alloc(int size, const char *name)
{
	...
	strscpy(fence->name, name, sizeof(fence->name));
...
int sync_fence_wake_up_wq(wait_queue_entry_t *curr, unsigned int mode,
			  ...
			  struct sync_fence_waiter *waiter)
{
	...
	if (err > 0)
		__add_wait_queue_entry_tail(&fence->wq, &waiter->work);
...
int sync_fence_wait(struct sync_fence *fence, long timeout)
{
	...
	trace_staging_sync_wait(fence, 1);
	for (i = 0; i < fence->num_fences; ++i)
		trace_staging_sync_pt(fence->cbs[i].sync_pt);
	ret = wait_event_interruptible_timeout(fence->wq,
					       atomic_read(&fence->status) <= 0,
					       timeout);
	trace_staging_sync_wait(fence, 0);
...
static void android_fence_release(struct fence *fence)
{
	...
	spin_lock_irqsave(&parent->child_list_lock, flags);
	list_del(&pt->child_list);
	if (WARN_ON_ONCE(!list_empty(&pt->active_list)))
		list_del(&pt->active_list);
	spin_unlock_irqrestore(&parent->child_list_lock, flags);
...
static int sync_fill_pt_info(struct fence *fence, void *data, int size)
{
	...
	if (fence_is_signaled(fence))
		info->status = dma_fence_get_status(fence) >= 0 ? 1 : dma_fence_get_status(fence);
	else
		info->status = 0;
					...
					unsigned long arg)
{
	...
	strscpy(data->name, fence->name, sizeof(data->name));
