...
static struct trusty_irq_state *trusty_irq_is;

static int trusty_irq_cpu_up_cpuhp(unsigned int cpu)
{
	struct trusty_irq_state *is = trusty_irq_is;
...
static int trusty_irq_cpu_down_cpuhp(unsigned int cpu)
{
	struct trusty_irq_state *is = trusty_irq_is;
...
static int trusty_irq_probe(struct platform_device *pdev)
{
	...
	trusty_irq_is = is;
	ret = cpuhp_setup_state_nocalls(CPUHP_AP_ONLINE_DYN, "trusty-irq",
					trusty_irq_cpu_up_cpuhp,
					trusty_irq_cpu_down_cpuhp);
	if (ret < 0) {
		dev_err(&pdev->dev, "cpuhp_setup_state failed %d\n", ret);
		goto err_register_hotcpu_notifier;
	}
	is->cpuhp_state = ret;
	on_each_cpu(trusty_irq_cpu_up, is, 0);

	return 0;

err_on_each_cpu:
	cpuhp_remove_state_nocalls(is->cpuhp_state);
	trusty_irq_is = NULL;
