...
static int sprd_leds_bltc_rgb_probe(struct platform_device *dev)
{
	...
	int ret, i;
	struct gpio_desc *gpio_prechg_led;
	unsigned int pmic_base_addr;

	gpio_prechg_led = devm_gpiod_get_optional(&dev->dev, "prechg-led",
						   GPIOD_OUT_HIGH);
	if (IS_ERR(gpio_prechg_led)) {
		dev_err(&dev->dev, "prechg led gpio request failed, ret %ld\n",
			PTR_ERR(gpio_prechg_led));
	}
...
static void sprd_leds_bltc_rgb_remove(struct platform_device *dev)
{
	struct sprd_leds_bltc_rgb *brgb = platform_get_drvdata(dev);
