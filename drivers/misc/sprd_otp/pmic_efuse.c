...
static int pmic_efuse_lock(void)
{
	...
	ret =
	    hwspin_lock_timeout(pmic_efuse->hw_lock,
					  HWSPINLOCK_TIMEOUT);
	if (ret) {
...
{
	hwspin_unlock(pmic_efuse->hw_lock);
	mutex_unlock(&pmic_efuse_mutex);
}
...
static int sprd_pmic_efuse_probe(struct platform_device *pdev)
{
	...
	{
		int hw_id = of_hwspin_lock_get_id_byname(pdev->dev.of_node,
							"pmic_efuse");
		if (hw_id < 0) {
			dev_err(&pdev->dev, "pmic efuse can not get hardware spinlock.\n");
			return -ENXIO;
		}
		pmic_efuse->hw_lock = hwspin_lock_request_specific(hw_id);
	}
...
static void sprd_pmic_efuse_remove(struct platform_device *pdev)
{
