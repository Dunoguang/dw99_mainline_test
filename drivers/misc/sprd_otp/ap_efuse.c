...
#include <linux/mfd/syscon/sprd/isharkl2/isharkl2_aon_apb_glb.h>
...
static int sprd_ap_efuse_probe(struct platform_device *pdev)
{
	...
	{
		int hw_lock_id = of_hwspin_lock_get_id_byname(pdev->dev.of_node,
							     "ap_efuse");

		if (hw_lock_id < 0) {
			dev_err(&pdev->dev,
				"ap_efuse can not get the hardware spinlock.\n");
			ret = -ENXIO;
			goto clk_fail;
		}
		ap_efuse->hw_lock = hwspin_lock_request_specific(hw_lock_id);
		if (!ap_efuse->hw_lock) {
			dev_err(&pdev->dev,
				"ap_efuse can not request the hardware spinlock.\n");
			ret = -ENXIO;
			goto clk_fail;
		}
	}
...
static void sprd_ap_efuse_remove(struct platform_device *pdev)
{
