...
static void aud_smsg_dump_func(uint32_t is_timeout)
{
	if (dump_func)
...
int aud_smsg_send(uint8_t dst, struct aud_smsg *msg)
{
	...
	if (ipc->dsp_ready == false) {
		pr_err("aud_smsg_send: dsp not ready\n");
		goto send_failed;
	}
...
int aud_smsg_recv(uint8_t dst, struct aud_smsg *msg, int timeout)
{
	...
	if (timeout == 0) {
		...
		if ((ipc->states[msg->channel] == CHAN_STATE_FREE)
			|| (readl_relaxed((void *)ch->wrptr)
			== readl_relaxed((void *)ch->rdptr))) {
			sp_asoc_pr_info("warning %s smsg channel %d is free ipc->dsp_ready %d!\n",
...
static int aud_smsg_debug_show(struct seq_file *m, void *private)
{
	...
	for (i = 0; i < AUD_IPC_NR; i++) {
		...
		seq_printf(m, "txbufAddr: 0x%0x, txbufsize: 0x%0x,txbufrdptr: [0x%0x]=%u, txbufwrptr: [0x%0x]=%u\n",
			   (uint32_t)spc->txbuf_addr,
			   spc->txbuf_size,
			   (uint32_t)spc->txbuf_rdptr,
			   readl_relaxed((void *)spc->txbuf_rdptr),
			   (uint32_t)spc->txbuf_wrptr,
			   readl_relaxed((void *)spc->txbuf_wrptr));
		seq_printf(m, "rxbufAddr: 0x%0x, rxbufsize: 0x%0x,rxbufrdptr: [0x%0x]=%u, rxbufwrptr: [0x%0x]=%u\n",
			  (uint32_t) spc->rxbuf_addr,
			  spc->rxbuf_size,
			 (uint32_t)spc->rxbuf_rdptr,
			 (uint32_t) readl_relaxed((void *)spc->rxbuf_rdptr),
			 (uint32_t) spc->rxbuf_wrptr,
			 (uint32_t)readl_relaxed((void *)spc->rxbuf_wrptr));
...
static int aud_smsg_syscore_suspend(void *data)
{
...
static struct syscore aud_smsg_syscore = {
	.ops = &aud_smsg_syscore_ops,
};

int  aud_smsg_suspend_init(void)
{
	wake_lock_init(&sipc_wake_lock, WAKE_LOCK_SUSPEND, "audio-smsg-sipc");
	register_syscore(&aud_smsg_syscore);
