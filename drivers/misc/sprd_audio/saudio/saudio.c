...
static int saudio_send_common_cmd_ex(uint32_t dst,
	uint32_t channel,
	uint32_t cmd,
	uint32_t subcmd,
	uint32_t reserve1,
...
static int saudio_send_common_cmd(uint32_t dst, uint32_t channel,
		uint32_t cmd, uint32_t subcmd, uint32_t device,
		int32_t timeout)
{
	int result = 0;
	struct sblock blk = { 0 };
	...
	for (stream = 0; stream < SAUDIO_STREAM_MAX; stream++) {
		for (substream = pcm->streams[stream].substream; substream;
			...
			if (addr) {
				...
				pr_debug("saudio_pcm_lib_preallocate_pages_for_all:saudio.c: dmab addr is %x, area is %lx,size is %d",
					(uint32_t) dmab->addr,
					 ...
					 int substreams)
{
	...
	if (err) {
		pr_err("saudio:snd_card_saudio_pcm: prealloc pages error\n");
...
static int saudio_ctrl_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (saudio_snd_notify_modem_clear(saudio)) {
			pr_err(" saudio_ctrl_thread modem error again when notify modem clear\n");
			continue;
		}
...
static int snd_saudio_probe(struct platform_device *devptr)
{
	...
	saudio->thread_id = kthread_create(saudio_ctrl_thread, saudio,
...
static void snd_saudio_remove(struct platform_device *devptr)
{
