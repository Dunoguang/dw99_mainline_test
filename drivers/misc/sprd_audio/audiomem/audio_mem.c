...
int audio_smem_init(uint32_t addr, uint32_t size)
{
...
static uint32_t audio_smem_alloc(uint32_t size)
{
	struct smem_pool *spool = &audio_mem_pool;
...
static void audio_smem_free(uint32_t addr, uint32_t size)
{
	struct smem_pool *spool = &audio_mem_pool;
...
static void audio_mem_remove(struct platform_device *pdev)
{
	if (g_priv_data) {
...
static struct platform_driver audio_mem_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "audio_mem",
		.of_match_table = of_match_ptr(audio_mem_match_table),
	},
