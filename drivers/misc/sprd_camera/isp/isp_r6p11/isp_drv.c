...
static int sprd_isp_stop_nolock(void *isp_handle, int is_post_stop)
{
	...
	for (i = ISP_SCL_PRE; i <= ISP_SCL_CAP; i++) {
		...
		isp_frm_clear(dev, 1 << (i - ISP_SCL_PRE));
	}
...
#ifdef CONFIG_64BIT
	dcache_clean_inval_poc((unsigned long)dev->fmcu_addr_vir,
		    (unsigned long)dev->fmcu_addr_vir + (fmcu_slice->fmcu_num << 3));
#else
...
static int isp_set_statis_buf(struct isp_pipe_dev *dev)
{
	...
	if (statis_module->statis_valid & ISP_STATIS_VALID_BINNING) {
		rtn = isp_set_next_statis_buf(dev->com_idx,
	...
	if (statis_module->statis_valid & ISP_STATIS_VALID_HIST) {
		rtn = isp_set_next_statis_buf(dev->com_idx,
...
static int isp_block_buf_alloc(struct isp_pipe_dev *dev)
{
	...
	if (!isp_k_param->lsc_buf_info.sw_addr) {
		...
		strscpy(isp_k_param->lsc_buf_info.name, LSC_BUF_NAME);
...
int isp_path_cap_with_vid_set_next_frm(struct isp_pipe_dev *dev)
{
	...
	if (isp_buf_queue_read(p_buf_queue, &frame) ||
