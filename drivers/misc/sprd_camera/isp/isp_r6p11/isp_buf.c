	...
	uint8_t off_type)
{
	...
	for (num = 0; num < frm_q_len; num++) {
		...
		pr_info("offline %s[%u] len 0x%zx ISP iova 0x%08x DCAM iova 0x%08x\n",
			off_type == ISP_OFF_BUF_BIN ? "bin" : "full", num,
			ionbuffer->size, ion_buf->addr.yaddr_vir,
			ion_buf->addr.uaddr_vir);
