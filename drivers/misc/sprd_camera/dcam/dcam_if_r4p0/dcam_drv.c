...
static int dcam_raw_path_set_next_frm(enum dcam_id idx)
{
	...
	if (!use_reserve_frame || frame.pfinfo.iova[0] == 0) {
	...
	struct camera_frame *frame)
{
	...
	if (DCAM_PHYADDR_INVALID(frame->yaddr_vir) ||
	    DCAM_PHYADDR_INVALID(frame->uaddr_vir)) {
		pr_err("DCAM%d: invalid full buffer ISP 0x%x DCAM 0x%x\n",
			idx, frame->yaddr_vir, frame->uaddr_vir);
			...
			struct camera_frame *frame)
{
	...
	if (DCAM_PHYADDR_INVALID(frame->yaddr_vir) ||
	    DCAM_PHYADDR_INVALID(frame->uaddr_vir)) {
		pr_err("DCAM%d: invalid bin buffer ISP 0x%x DCAM 0x%x\n",
			idx, frame->yaddr_vir, frame->uaddr_vir);
		rtn = DCAM_RTN_PATH_ADDR_ERR;
		goto exit;
	}
...
static void dcam_frm_clear(enum dcam_id idx)
{
	...
	while (!dcam_frame_dequeue(&raw_path->frame_queue, &frame)) {
		if (frame.pfinfo.mfd[0] && frame.pfinfo.mfd[0] !=
...
static inline unsigned int dcam_convert_path_id_2_path_bit(
	enum dcam_id idx,
				...
				struct camera_frame *frame)
{
	...
	pr_info("DCAM%d: start full ISP iova 0x%x DCAM iova 0x%x\n",
		idx, (frame + 1)->yaddr_vir, (frame + 1)->uaddr_vir);
				...
				struct camera_frame *frame)
{
	...
	*/
	...
	if (idx == DCAM_ID_0)
		...
		addr = DCAM1_IMAGE_DT_VC_CONTROL;
	DCAM_REG_WR(idx, addr, reg_val);
	pr_info("DCAM%d: start bin ISP iova 0x%x DCAM iova 0x%x\n",
		idx, frame->yaddr_vir, frame->uaddr_vir);
...
#define _WIDTH_NO_1_4_BINNING	176
	...
	if (dst.w <= _WIDTH_NO_1_4_BINNING && tmp_ratio == 2) {
		tmp_ratio = 1;
		pr_warn("no 1/4 binning, (%d, %d) -> (%d, %d)\n",
