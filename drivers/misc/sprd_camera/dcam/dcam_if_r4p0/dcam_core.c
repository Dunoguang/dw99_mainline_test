...
/*
 * compat_alloc_user_space was removed from upstream; reimplement the
 * classic semantics: allocate a buffer at the top of the process
 * user address space (below mmap_base).
 */
static void __user *compat_alloc_user_space(unsigned long len)
{
	unsigned long usp = current->mm->mmap_base - len;
...
/*
 * copy_in_user was removed from upstream; implement via kernel bounce.
 */
static inline unsigned long sprd_copy_in_user(void __user *to,
					      const void __user *from, unsigned long n)
{
	...
	while (n) {
		unsigned long chunk = min(n, sizeof(buf));

		if (copy_from_user(buf, from + done, chunk) ||
		    copy_to_user(to + done, buf, chunk))
			break;
		done += chunk;
		n -= chunk;
	}
...
{
	struct camera_dev *dev = container_of(t, struct camera_dev, dcam_timer);
...
#ifdef CONFIG_COMPAT
#define SPRD_IMG_PARM_LEGACY_SIZE		968
#define SPRD_IMG_PARM_LEGACY_PREFIX_SIZE	852
#define SPRD_IMG_STATIS_LEGACY_COMMON_SIZE	44
#define SPRD_IMG_STATIS_LEGACY_FRAME_ID_OFFSET	896
#define SPRD_IMG_STATIS_LEGACY_DAC_INFO_OFFSET	900
#define SPRD_IMG_PARM_LEGACY_JPEGLS_OFFSET	928
#define SPRD_IMG_PARM_LEGACY_EBD_OFFSET		940
#define SPRD_IMG_PARM_LEGACY_RESERVED_OFFSET	952

#define SPRD_IMG_IO_SET_FRM_ID_BASE_LEGACY	0x43c85a04U
#define SPRD_IMG_IO_SET_CROP_LEGACY		0x43c85a05U
#define SPRD_IMG_IO_SET_OUTPUT_SIZE_LEGACY	0x43c85a07U
#define SPRD_IMG_IO_SET_FRAME_ADDR_LEGACY	0x43c85a0aU
#define SPRD_IMG_IO_PATH_FRM_DECI_LEGACY	0x43c85a0bU
#define SPRD_IMG_IO_SET_PATH_SKIP_NUM_LEGACY	0x43c85a1dU

static unsigned int sprd_img_compat_parm_cmd(unsigned int cmd)
{
	switch (cmd) {
	case SPRD_IMG_IO_SET_FRM_ID_BASE_LEGACY:
		return SPRD_IMG_IO_SET_FRM_ID_BASE;
	case SPRD_IMG_IO_SET_CROP_LEGACY:
		return SPRD_IMG_IO_SET_CROP;
	case SPRD_IMG_IO_SET_OUTPUT_SIZE_LEGACY:
		return SPRD_IMG_IO_SET_OUTPUT_SIZE;
	case SPRD_IMG_IO_SET_FRAME_ADDR_LEGACY:
		return SPRD_IMG_IO_SET_FRAME_ADDR;
	case SPRD_IMG_IO_PATH_FRM_DECI_LEGACY:
		return SPRD_IMG_IO_PATH_FRM_DECI;
	case SPRD_IMG_IO_SET_PATH_SKIP_NUM_LEGACY:
		return SPRD_IMG_IO_SET_PATH_SKIP_NUM;
	default:
		return 0;
	}
}

static long sprd_img_k_compat_ioctl(struct file *file, unsigned int cmd,
				    unsigned long arg)
{
	u8 __user *legacy_parm = compat_ptr(arg);
	struct sprd_img_parm __user *native_parm;
	unsigned int native_cmd;

	BUILD_BUG_ON(sizeof(struct sprd_img_parm) != 1008);
	BUILD_BUG_ON(offsetof(struct sprd_img_parm, buffer_count) +
		     sizeof(((struct sprd_img_parm *)0)->buffer_count) !=
		     SPRD_IMG_PARM_LEGACY_PREFIX_SIZE);
	BUILD_BUG_ON(offsetof(struct sprd_img_parm, img_statis_info) != 856);
	BUILD_BUG_ON(offsetof(struct sprd_img_statis_info, frame_id) != 64);
	BUILD_BUG_ON(offsetof(struct sprd_img_statis_info, dac_info) != 68);
	BUILD_BUG_ON(sizeof(struct sprd_img_vcm_dac_info) != 28);
	BUILD_BUG_ON(offsetof(struct sprd_img_parm, jpegls_length) != 960);
	BUILD_BUG_ON(offsetof(struct sprd_img_parm, ebd_ctrl) != 972);
	BUILD_BUG_ON(offsetof(struct sprd_img_parm, reserved) != 992);
	BUILD_BUG_ON(SPRD_IMG_PARM_LEGACY_RESERVED_OFFSET +
		     sizeof(((struct sprd_img_parm *)0)->reserved) !=
		     SPRD_IMG_PARM_LEGACY_SIZE);
	BUILD_BUG_ON(_IOC(_IOC_WRITE, SPRD_IMG_IO_MAGIC, 4,
			  SPRD_IMG_PARM_LEGACY_SIZE) !=
		     SPRD_IMG_IO_SET_FRM_ID_BASE_LEGACY);
	BUILD_BUG_ON(_IOC(_IOC_WRITE, SPRD_IMG_IO_MAGIC, 29,
			  SPRD_IMG_PARM_LEGACY_SIZE) !=
		     SPRD_IMG_IO_SET_PATH_SKIP_NUM_LEGACY);

	native_cmd = sprd_img_compat_parm_cmd(cmd);
	if (!native_cmd)
		return compat_sprd_img_k_ioctl(file, cmd, arg);

	if (!file->f_op || !file->f_op->unlocked_ioctl)
		return -ENOTTY;

	native_parm = compat_alloc_user_space(sizeof(*native_parm));
	if (!native_parm)
		return -EFAULT;

	/* Translate the P tail around fields added to sprd_img_statis_info. */
	if (clear_user(native_parm, sizeof(*native_parm)) ||
	    sprd_copy_in_user(native_parm, legacy_parm,
			 SPRD_IMG_PARM_LEGACY_PREFIX_SIZE) ||
	    sprd_copy_in_user(&native_parm->img_statis_info, legacy_parm +
			 SPRD_IMG_PARM_LEGACY_PREFIX_SIZE,
			 SPRD_IMG_STATIS_LEGACY_COMMON_SIZE) ||
	    sprd_copy_in_user(&native_parm->img_statis_info.frame_id, legacy_parm +
			 SPRD_IMG_STATIS_LEGACY_FRAME_ID_OFFSET,
			 sizeof(native_parm->img_statis_info.frame_id)) ||
	    sprd_copy_in_user(&native_parm->img_statis_info.dac_info, legacy_parm +
			 SPRD_IMG_STATIS_LEGACY_DAC_INFO_OFFSET,
			 sizeof(native_parm->img_statis_info.dac_info)) ||
	    sprd_copy_in_user(native_parm->jpegls_length, legacy_parm +
			 SPRD_IMG_PARM_LEGACY_JPEGLS_OFFSET,
			 sizeof(native_parm->jpegls_length)) ||
	    sprd_copy_in_user(&native_parm->ebd_ctrl, legacy_parm +
			 SPRD_IMG_PARM_LEGACY_EBD_OFFSET,
			 sizeof(native_parm->ebd_ctrl)) ||
	    sprd_copy_in_user(native_parm->reserved, legacy_parm +
			 SPRD_IMG_PARM_LEGACY_RESERVED_OFFSET,
			 sizeof(native_parm->reserved)))
		return -EFAULT;

	pr_info_once("camera ABI: translate sprd_img_parm 968 -> 1008\n");
	return file->f_op->unlocked_ioctl(file, native_cmd,
					   (unsigned long)native_parm);
}
#endif

#define SPRD_IMG_READ_OP_LEGACY_SIZE 192

static int sprd_img_copy_read_op_to_user(char __user *u_data,
		const struct sprd_img_read_op *read_op, uint32_t cmd,
		size_t cnt)
{
	u8 legacy_op[SPRD_IMG_READ_OP_LEGACY_SIZE] = { 0 };
	const size_t parm_offset = offsetof(struct sprd_img_read_op, parm);
	const size_t frame_skip =
		offsetof(struct sprd_img_frm_info, zoom_ratio);
	const size_t capability_skip =
		offsetof(struct sprd_img_path_capability, support_4in1);
	u8 *legacy_parm = legacy_op + parm_offset;
	const u8 *new_parm = (const u8 *)&read_op->parm;

	BUILD_BUG_ON(sizeof(*read_op) != 196);
	BUILD_BUG_ON(offsetof(struct sprd_img_read_op, parm) != 12);
	BUILD_BUG_ON(sizeof(read_op->parm.frame) - sizeof(uint32_t) >
		     SPRD_IMG_READ_OP_LEGACY_SIZE - 12);
	BUILD_BUG_ON(sizeof(read_op->parm.capability) - sizeof(uint32_t) >
		     SPRD_IMG_READ_OP_LEGACY_SIZE - 12);

	if (cnt == sizeof(*read_op))
		return copy_to_user(u_data, read_op, sizeof(*read_op)) ?
			-EFAULT : 0;

	if (cnt != SPRD_IMG_READ_OP_LEGACY_SIZE)
		return -EINVAL;

	/* The P camera ABI predates one u32 in each large union member. */
	memcpy(legacy_op, read_op, parm_offset);
	switch (cmd) {
	case SPRD_IMG_GET_FRM_BUFFER:
		memcpy(legacy_parm, new_parm, frame_skip);
		memcpy(legacy_parm + frame_skip,
		       new_parm + frame_skip + sizeof(read_op->parm.frame.zoom_ratio),
		       sizeof(read_op->parm.frame) - frame_skip -
		       sizeof(read_op->parm.frame.zoom_ratio));
		break;
	case SPRD_IMG_GET_PATH_CAP:
		memcpy(legacy_parm, new_parm, capability_skip);
		memcpy(legacy_parm + capability_skip,
		       new_parm + capability_skip +
		       sizeof(read_op->parm.capability.support_4in1),
		       sizeof(read_op->parm.capability) - capability_skip -
		       sizeof(read_op->parm.capability.support_4in1));
		break;
	default:
		memcpy(legacy_parm, new_parm,
		       sizeof(read_op->parm.reserved));
		break;
	}

	return copy_to_user(u_data, legacy_op, sizeof(legacy_op)) ? -EFAULT : 0;
}
			     ...
			     size_t cnt, loff_t *cnt_ret)
{
	...
	if (cnt != sizeof(struct sprd_img_read_op) &&
	    cnt != SPRD_IMG_READ_OP_LEGACY_SIZE) {
		pr_err("fail to read, cnt %zd read_op %d\n", cnt,
		       (int32_t)sizeof(struct sprd_img_read_op));
		return -EIO;
	}
	if (cnt == SPRD_IMG_READ_OP_LEGACY_SIZE)
		pr_info_once("camera ABI: translate sprd_img_read_op 192 <- 196\n");

	memset(&read_op, 0, sizeof(read_op));
	if (copy_from_user(&read_op, (void __user *)u_data,
			   cnt == SPRD_IMG_READ_OP_LEGACY_SIZE ?
			   offsetof(struct sprd_img_read_op, parm) : cnt)) {
		pr_err("fail to get user info\n");
		return -EFAULT;
	}
	read_cmd = read_op.cmd;
...
read_end:
	copy_ret = sprd_img_copy_read_op_to_user(u_data, &read_op,
						 read_cmd, cnt);
	if (copy_ret)
		ret = copy_ret;
...
#ifdef CONFIG_COMPAT
	.compat_ioctl = sprd_img_k_compat_ioctl,
#endif
...
static void sprd_img_remove(struct platform_device *pdev)
{
	struct camera_group *group = NULL;
