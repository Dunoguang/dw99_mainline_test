					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		flash_ic_driver_reg_write(drv_data->i2c_info, 0x05,
				drv_data->flash_cfg.preflash_level);
		}
					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		if (SPRD_FLASH_LED0 & idx)
			flash_ic_driver_reg_write(drv_data->i2c_info, 0x05,
					drv_data->flash_cfg.torch_level);
		}
					   ...
					   struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		if (SPRD_FLASH_LED0 & idx)
			flash_ic_driver_reg_write(drv_data->i2c_info, 0x03,
					drv_data->flash_cfg.highlight_level);
		}
...
static void sprd_flash_ic_driver_remove(struct i2c_client *client)
{
	struct flash_driver_data *pdata = NULL;
