...
static int sprd_flash_gpio_probe(struct platform_device *pdev)
{
	...
	ret = sprd_flash_register(&flash_gpio_ops, drv_data, SPRD_FLASH_FRONT);
	if (ret < 0)
		goto exit;
exit:
	return ret;
...
static void sprd_flash_gpio_remove(struct platform_device *pdev)
{
}
