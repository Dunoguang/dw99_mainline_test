			...
			struct sprdwl_cmd_gscan_set_config *config)
{
	...
	nla_for_each_nested(attr_bucket, bucket_spec, rem1) {
		ret =  nla_parse(bucket, SPRDWL_ATTR_GSCAN_MAX,
				 nla_data(attr_bucket), nla_len(attr_bucket), NULL, 0);
		if (ret) {
				   ...
				   struct wifi_iface_stat *iface_st)
{
	...
	if (nla_put_u32(reply, SPRDWL_ATTR_LL_STATS_IFACE_BEACON_RX,
			iface_st->beacon_rx))
		goto out_put_fail;
	if (nla_put_u64_64bit(reply, SPRDWL_ATTR_LL_STATS_IFACE_AVERAGE_TSF_OFFSET,iface_st->average_tsf_offset, 0))
		goto out_put_fail;
		...
		for (n = 0; n < vif->priv->gscan_buckets_num; n++) {
			...
			j++) {
				...
				if (nla_put_u64_64bit(reply,
						GSCAN_RESULTS_SCAN_RESULT_TIME_STAMP,p->results[j].ts, 0)) {
					netdev_err(vif->ndev, "failed to put!\n");
					goto out_put_fail;
				}
				if (nla_put(reply,
					  ...
					  struct sprdwl_gscan_result *item)
{
	...
	nla_put_u64_64bit(reply,
		    GSCAN_RESULTS_SCAN_RESULT_TIME_STAMP,item->ts, 0) ||
					...
					const void *data, int len)
{
	...
	if (nla_parse(tb, WLAN_ATTR_SAR_LIMITS_MAX, data, len, NULL, 0)) {
