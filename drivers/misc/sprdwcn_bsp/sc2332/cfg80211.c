...
static int sprdwl_cfg80211_add_key(struct wiphy *wiphy,
...
static int sprdwl_cfg80211_del_key(struct wiphy *wiphy,
				   struct wireless_dev *wdev, int link_id,
				   u8 key_index, bool pairwise,
				   const u8 *mac_addr)
{
...
static int sprdwl_cfg80211_stop_ap(struct wiphy *wiphy,
				  struct net_device *ndev,
...
void sprdwl_scan_done(struct sprdwl_vif *vif, bool abort)
{
	...
	if (priv->scan_vif && priv->scan_vif == vif) {
		...
		spin_lock_bh(&priv->scan_lock);
		if (priv->scan_request) {
			struct cfg80211_scan_info info = { .aborted = abort };
			      ...
			      u16 resp_ie_len, u8 status_code)
{
	...
	if (vif->sm_state == SPRDWL_CONNECTING &&
		...
		cfg80211_roamed(vif->ndev, &info, GFP_KERNEL);
	}
	else {
