...
static void sprdwl_sipc_keep_wakeup(struct sprdwl_intf *intf)
{
	if (time_after(jiffies, intf->wake_last_time)) {
		...
		__pm_wakeup_event(intf->keep_wake,
				  jiffies_to_msecs(intf->wake_timeout));
	}
}
				...
				struct sprdwl_priv *priv)
{
	...
	intf->keep_wake =
		wakeup_source_register(NULL, "sprdwl_keep_wakelock");
	...
	intf->tx_wakelock =
		wakeup_source_register(NULL, "sprdwl_tx_wakelock");
...
err_tx_work:
	wakeup_source_unregister(intf->tx_wakelock);
...
err_tx_list0:
	wakeup_source_unregister(intf->keep_wake);
