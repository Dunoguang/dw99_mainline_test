...
static void  sprd_spi_chipselect(struct spi_device *spi_dev, int is_on)
{
	...
	struct spi_controller	*master = spi_dev->controller;
	int ret;

	spi_chip = spi_controller_get_devdata(master);
	...
	if (is_on) {
		reg_val &= ~(0x1 << (spi_dev->chip_select[0] + 8));
				...
				struct sprd_dma_cfg *c, int rx)
{
	...
	struct spi_controller	*master = spi_chip->bitbang.ctlr;
...
static int sprd_spi_probe(struct platform_device *pdev)
{
	...
	master = spi_alloc_host(&pdev->dev, sizeof(*spi_chip));
	if (!master) {
		dev_err(&pdev->dev,
	...
	spi_chip->reg_base = devm_ioremap(&pdev->dev, regs->start,
			regs->end - regs->start);
	...
	spi_chip->bitbang.ctlr = spi_controller_get(master);
...
static int __exit sprd_spi_remove(struct platform_device *pdev)
{
	...
	struct sprd_spi *spi_chip = spi_controller_get_devdata(master);
