...
{
	struct sprd_adi *sadi = spi_controller_get_devdata(adi_dev->controller);
...
{
	struct spi_controller *adi_ctlr;
	struct sprd_adi *sadi;
	...
	/* alloc spi_master */
	adi_ctlr = spi_alloc_host(&pdev->dev, sizeof(struct sprd_adi));
	if (!adi_ctlr) {
		pr_err("Spi master allocate memory failed!\n");
		return -ENOMEM;
	}

	dev_set_drvdata(&pdev->dev, adi_ctlr);

	/* initialize the spi_master */
	adi_ctlr->dev.of_node = pdev->dev.of_node;
	adi_ctlr->bus_num = ADI_SPI_BUS_NUM;
	/* chip select number according to dt configuration */
	adi_ctlr->num_chipselect = chip_num;
	adi_ctlr->setup = sprd_adi_setup;
	adi_ctlr->flags = SPI_CONTROLLER_HALF_DUPLEX;
	adi_ctlr->bits_per_word_mask = 0;
	adi_ctlr->auto_runtime_pm = true;
	/* default is spi_bitbang_transfer */
	/* adi_ctlr->transfer = spi_bitbang_transfer; */

	/* initialize the sprd_adi structure */
	sadi = spi_controller_get_devdata(adi_ctlr);
	...
	sadi->bitbang.ctlr = spi_controller_get(adi_ctlr);
	if (!sadi->bitbang.ctlr) {
		ret = -ENODEV;
		goto master_alloc_fail;
	}
...
{
	struct spi_controller *adi_ctlr = dev_get_drvdata(&pdev->dev);
	struct sprd_adi *sadi = spi_controller_get_devdata(adi_ctlr);
