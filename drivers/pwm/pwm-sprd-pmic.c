...
static int pwm_pmic_apply(struct pwm_chip *chip, struct pwm_device *pwm,
			   const struct pwm_state *state)
{
	if (state->enabled) {
		int ret;

		ret = pwm_pmic_config(chip, pwm, state->duty_cycle,
				      state->period);
		if (ret)
			return ret;
		pwm_pmic_enable(chip, pwm);
	} else {
		pwm_pmic_disable(chip, pwm);
	}
...
static int pwm_pmic_probe(struct platform_device *pdev)
{
	...
	pr_debug("%s Entering\n", __func__);
	chip = devm_pwmchip_alloc(&pdev->dev, NUM_PWM, 0);
	if (!chip)
		return -ENOMEM;
